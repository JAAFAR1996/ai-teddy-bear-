...
class ServicesConsolidator:
    def __init__(self, project_root: str = "."):
        ...
        self.new_services_structure = {
            "ai_services": {
                ...
                "patterns": ["ai", "openai", "gpt", "llm", "claude", "gemini", "hume", "emotion", "analysis"]
            },
            "audio_services": {
                "path": "src/application/services/audio", 
                "patterns": ["audio", "voice", "speech", "tts", "stt", "sound", "microphone", "speaker"]
            },
            "child_services": {
                "path": "src/application/services/child",
                "patterns": ["child", "kid", "interaction", "learning", "progress", "education"]
            },
            "parent_services": {
                "path": "src/application/services/parent",
                "patterns": ["parent", "dashboard", "control", "report", "monitor", "auth"]
            },
            "device_services": {
                "path": "src/application/services/device",
                "patterns": ["device", "esp32", "hardware", "teddy", "simulator", "connection"]
            },
            "core_services": {
                "path": "src/application/services/core",
                "patterns": ["notification", "messaging", "security", "validation", "utils", "common"]
            }
        }
    ...
    def analyze_current_services(self) -> Dict:
        ...
        services_analysis = {
            ...
            "service_directories": [],
            "files_by_category": {category: [] for category in self.new_services_structure.keys()},
            "unclassified_files": [],
            "complexity_metrics": {}
        }
        ...
        for root, dirs, files in os.walk(self.src_path):
            ...
            for d in dirs:
                if 'service' in d.lower():
                    services_analysis["service_directories"].append(os.path.join(root, d))
            ...
            for file in files:
                if file.endswith('.py') and 'service' in file.lower():
                    ...
                    for category, config in self.new_services_structure.items():
                        if any(pattern in file.lower() for pattern in config["patterns"]):
                            services_analysis["files_by_category"][category].append(str(file_path))
                            classified = True
                            break
        ...
        services_analysis["complexity_metrics"] = {
            ...
            "files_distribution": {cat: len(files) for cat, files in services_analysis["files_by_category"].items()}
        }
...
'''
                ...
                if src_file.exists():
                    ...
                    if target_file.exists():
                        ...
                        while target_file.exists():
                            ...
                            target_file = target_dir / f"{name_parts[0]}_v{name_parts[1]}{name_parts[2]}"
                            counter += 1
        ...
        for file_path in files:
            try:
                ...
                if src_file.exists():
                    ...
                    if target_file.exists():
                        target_file = target_dir / f"{src_file.stem}_consolidated{src_file.suffix}"
    ...
    def execute_consolidation(self, analysis: Dict) -> Dict:
        ...
        if analysis["unclassified_files"]:
            ...
            unclassified_results = self.consolidate_category_services("core_services", analysis["unclassified_files"])
        ...
        results["summary"] = {
            ...
            "structure_improvement": analysis["complexity_metrics"]["complexity_reduction"],
...
def main():
    ...
    try:
        ...
        # عرض التحليل
        print(f"\n📊 نتائج التحليل:")
        print(f"- مجلدات Services حالياً: {analysis['complexity_metrics']['current_service_dirs']}")
        print(f"- مجلدات Services مستهدفة: {analysis['complexity_metrics']['target_service_dirs']}")
        print(f"- التحسن المتوقع: {analysis['complexity_metrics']['complexity_reduction']}")
        ...
        if response.lower() == 'y':
            ...
            print(f"🚀 التحسن المحقق: {analysis['complexity_metrics']['complexity_reduction']}")
