...
class TestUnifiedEncryptionService:
    ...
    async def test_advanced_encryption_levels(self, encryption_service):
        ...
        for level, expiry_days in levels:
            ...
            if encrypted.expires_at:
                ...
                assert abs((encrypted.expires_at - expected_expiry).total_seconds()) < 60
    ...
    async def test_audio_encryption(self, encryption_service):
        ...
        assert encrypted_audio.metadata["child_id"] != child_id  # Should be hashed
    ...
    def test_key_rotation(self, encryption_service):
        ...
        now = datetime.utcnow()
        for i, (key_id, (key, created_at)) in enumerate(encryption_service._key_cache.items()):
            if i < 3:
...
class TestConversationService:
    ...
    async def test_conversation_flow(self, conversation_service):
        ...
        # 2. Add messages
        messages = [
            {"speaker": "child", "text": "مرحبا دبدوب", "timestamp": datetime.utcnow()},
            {"speaker": "teddy", "text": "مرحباً صديقي! كيف حالك؟", "timestamp": datetime.utcnow()},
            {"speaker": "child", "text": "أنا بخير، هل تحكي لي قصة؟", "timestamp": datetime.utcnow()},
            {"speaker": "teddy", "text": "بالطبع! سأحكي لك قصة جميلة...", "timestamp": datetime.utcnow()}
        ]
