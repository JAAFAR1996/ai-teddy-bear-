...
from src.infrastructure.di.enterprise_container import (
    EnterpriseContainer, LifecycleScope, CircularDependencyError,
    DependencyNotFoundError, create_container, inject_dependency
)
from src.infrastructure.messaging.event_driven_architecture import (
    Event, EventType, Command, Query, RedisEventBus, InMemoryCommandBus,
    InMemoryQueryBus, create_event, create_command, create_query
)
from src.infrastructure.plugins.plugin_architecture import (
    PluginManager, PluginType, PluginPermission, PluginManifest,
    create_plugin_manager, create_plugin_manifest
)
from src.infrastructure.microservices.service_orchestrator import (
    ServiceOrchestrator, ServiceInstance, ServiceDefinition,
    LoadBalancingStrategy, ServiceStatus, create_service_instance,
    create_service_definition
)
from src.infrastructure.state.application_state_manager import (
    ApplicationStateManager, StateScope, ThreadSafeStateStore,
    create_state_manager, request_scope, session_scope
)
...
class TestMicroservicesOrchestrator:
    ...
    async def test_load_balancer_round_robin(self):
        ...
        from src.infrastructure.microservices.service_orchestrator import LoadBalancer
...
class TestApplicationStateManager:
    ...
    async def test_request_scope_context(self):
        ...
        async with state_manager.async_request_context("test_request"):
            ...
            await state_manager.set_state("request_key", "request_value", StateScope.REQUEST)
    ...
    async def test_session_scope_context(self):
        ...
        with state_manager.session_context("test_session"):
            ...
            await state_manager.set_state("session_key", "session_value", StateScope.SESSION)
...
class TestSecurity:
    ...
    async def test_plugin_sandboxing(self):
        ...
        from src.infrastructure.plugins.plugin_architecture import PluginSandbox, SecurityError
