...
from src.domain.entities.conversation import (
    ...
    EmotionalState, ConversationMetrics, ContentType
)
...
class TestConversationRepositorySearch:
    ...
    async def test_get_conversations_by_topics(self, conversation_repository):
        ...
        conversations = [
            Conversation(
                child_id='child-1', topics=['math', 'science'],
                messages=[], emotional_states=[]
            ),
            Conversation(
                child_id='child-2', topics=['art', 'creativity'],
                messages=[], emotional_states=[]
            ),
            Conversation(
                child_id='child-3', topics=['math', 'games'],
                messages=[], emotional_states=[]
            )
        ]
    ...
    async def test_find_conversations_with_emotion(self, conversation_repository, sample_conversation):
        ...
        found_conversation = next(
            (conv for conv in happy_conversations if conv.id == created_conversation.id),
            None
        )
