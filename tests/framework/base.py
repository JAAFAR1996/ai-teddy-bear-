...
from typing import Generic, TypeVar, Dict, Any, List, Optional, Callable, Awaitable
import pytest
...
from .validators import ContentSafetyValidator, AgeAppropriateContentGenerator, COPPAComplianceChecker
from .bdd import TestContextBuilder, ActionExecutor
...
class ChildSafetyTestCase(BaseTestCase):
    ...
    async def setup_child_safety(self):
        ...
        self.unsafe_patterns = {
            ...
            "personal_info": [
                ...
                "my email", "my school", "my parent's name"
            ],
            "dangerous_behavior": [
                "don't tell your parents", "keep this secret",
                "meet me at", "send me a photo"
            ]
        }
    ...
    def assert_content_unsafe(self, content: str, age: int, expected_violation: str):
        ...
        assert expected_violation in result.violation_type, \
...
class PerformanceTestCase(BaseTestCase):
    ...
    def assert_performance_within_limits(self, metrics: Dict[str, Any]):
        ...
        if 'avg_latency_ms' in metrics:
            assert metrics['avg_latency_ms'] < self.thresholds['response_time_ms'], \
                f"Average latency {metrics['avg_latency_ms']}ms exceeds threshold"
                
        if 'p95_latency_ms' in metrics:
            assert metrics['p95_latency_ms'] < self.thresholds['p95_latency_ms'], \
                f"P95 latency {metrics['p95_latency_ms']}ms exceeds threshold"
                
        if 'memory_increase_mb' in metrics:
            assert metrics['memory_increase_mb'] < self.thresholds['memory_increase_mb'], \
