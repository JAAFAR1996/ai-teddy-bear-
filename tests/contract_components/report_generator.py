...
def _calculate_overall_results(test_suites: Dict[str, ContractTestSuite]) -> Dict[str, Any]:
    """حساب النتائج الإجمالية"""
    total_tests = sum(
        suite.total_tests for suite in test_suites.values())
    passed_tests = sum(
        suite.passed_tests for suite in test_suites.values())
    failed_tests = sum(
        suite.failed_tests for suite in test_suites.values())
    error_tests = sum(
        suite.error_tests for suite in test_suites.values())
