...
class ConcurrentUserTest:
    ...
    async def simulate_user_session(self, session: aiohttp.ClientSession, user_id: int):
        ...
        try:
            # Login
            async with session.post(f"{self.base_url}/api/v1/auth/login", json={
    ...
    async def run_concurrent_test(self, num_users: int = 50):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if successful > 0:
                avg_duration = sum(r["duration"] for r in results if isinstance(r, dict) and "duration" in r) / successful
            else:
                avg_duration = 0
