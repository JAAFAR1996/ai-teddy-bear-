...
class GitUtils(object):
    ...
    def get_commit_count(repo, *args):
        ...
        if p.wait() != 0:
            raise SystemExit(
                _("%s does not appear to be a valid git repository") % decode(repo)
            )
    ...
    def get_config_settings(repo_working_dir):
        ...
        try:
            output = subproc.check_output(
                "git config --list --null".split(), cwd=repo_working_dir
            )
        ...
        return dict(
            item.split(b"\n", maxsplit=1)
            for item in output.strip().split(b"\0")
            if item
        )
    ...
    def get_blob_sizes(quiet=False):
        ...
        for line in cf.stdout:
            try:
                ...
                sys.stderr.write(
                    _('Error: unexpected `git cat-file` output: "%s"\n') % line
                )
            if not quiet:
    ...
    def print_my_version():
        ...
        contents = re.sub(
            rb'(\("GIT_TEXTDOMAINDIR"\) or ").*"', rb'\1@@LOCALEDIR@@"', contents
        )
...
EXAMPLES
    ...
    For more detailed examples and explanations, see
      https://htmlpreview.github.io/?https://github.com/newren/git-filter-repo/blob/docs/html/git-filter-repo.html#EXAMPLES""")
        ...
        callback.add_argument(
            ...
            metavar="FUNCTION_BODY_OR_FILE",
            help=_(
                "Python code body for processing refnames; see CALLBACKS section below."
            ),
        )
        ...
        sdr.add_argument(
            ...
            action="store_true",
            help=_(
                "This rewrite is intended to remove sensitive data from a "
        ...
        sdr.add_argument(
            ...
            action="store_true",
            help=_(
                "By default, --sensitive-data-removal will trigger a "
    ...
    def sanity_check_args(args):
        ...
        if args.path_changes == None:
            ...
            if args.use_base_name:
                if any(x[0] == "rename" for x in args.path_changes):
                    raise SystemExit(
                        _("Error: --use-base-name and --path-rename are incompatible.")
                    )
        ...
        if args.file_info_callback and (
            ...
            raise SystemExit(
                _(
                    "Error: --file-info-callback is incompatible with "
    ...
    def run(args):
        ...
        if os.path.isdir(reportdir):
            if args.force:
                ...
                sys.stdout.write(
                    _('Error: dir already exists (use --force to delete): "%s"\n')
                    % decode(reportdir)
                )
                sys.exit(1)
...
class FileInfoValueHelper:
    def __init__(self, replace_text, insert_blob_func, source_working_dir):
        ...
        self._cat_file_process = subproc.Popen(
            cmd, stdin=subprocess.PIPE, stdout=subprocess.PIPE, cwd=source_working_dir
        )
...
class RepoFilter(object):
                ...
                if getattr(self, callback_field):
                    raise SystemExit(
                        _(
                            ...
                            "AND pass --%s-callback" % (which_under, which)
                        )
                    )
                if "return " not in code_string and which not in (
                    "blob",
                    "commit",
                    "tag",
                    "reset",
                    ...
                    raise SystemExit(
                        _("Error: --%s-callback should have a return statement") % which
                    )
    ...
    def _run_sanity_checks(self):
        ...
        if self._already_ran:
            ...
            if file_age > 86400:  # file older than a day
                msg = (
                    f"The previous run is older than a day ({decode(ran_path)} already exists).\n"
      ...
      Error: Cannot specify --sensitive-data-removal on a follow-up invocation
        ...
        if self._args.sensitive_data_removal:
            ...
            if self._args.source or self._args.target:
                msg = _(
                    "Error: sensitive data removal is incompatible with --source and --target"
                )
                raise SystemExit(msg)
        ...
        if self._args.replace_refs == "old-default":
            self._args.replace_refs = (
                "update-or-add" if self._already_ran else "update-and-add"
            )
    ...
    def _setup_lfs_orphaning_checks(self):
        ...
        self._file_info_value = FileInfoValueHelper(
            self._args.replace_text, self.insert, source_working_dir
        )
        ...
        self._lfs_object_tracker = LFSObjectTracker(
            self._file_info_value, check_sources, check_targets
        )
        ...
        if self._already_ran:
            ...
            source = True
            self._lfs_object_tracker.find_all_lfs_objects_in_repo(
                source_working_dir, source
            )
    ...
    def sanity_check(refs, is_bare, config_settings):
        ...
        if config_settings.get(b"core.precomposeunicode", b"false") == b"true":
            ...
            if msg:
                raise SystemExit(
                    _(
                        "Aborting: Cannot rewrite history on a character normalizing\n"
        ...
        if (
            ...
            or (
                num_packs == 1
                and num_loose_objects > 0
                and not RepoFilter.loose_objects_are_replace_refs(
                    git_dir, refs, num_loose_objects
                )
            )
        ):
        ...
        if not is_bare:
            ...
            if len(untracked_output) > 0:
                ...
                relevant_uf = [
                    x for x in uf if not x.startswith(b"__pycache__/git_filter_repo.")
                ]
    ...
    def cleanup(self, repo, repack, reset, run_quietly=False, show_debuginfo=False):
            ...
            for mod_type, match_type, path_exp in path_changes:
                if mod_type == "filter" and not wanted:
                    ...
                    if match_type == "match" and filename_matches(match, full_pathname):
                        ...
                        pathname = (
                            full_pathname  # rename incompatible with use_base_name
                        )
                    if match_type == "regex":
    ...
    def _tweak_commit(self, commit, aux_info):
        ...
        self._orig_graph.add_commit_and_parents(
            commit.old_id, orig_parents, commit.original_id
        )
        ...
        parents, new_1st_parent = self._maybe_trim_extra_parents(orig_parents, parents)
        ...
        if (
            ...
            and self._fep
            and (
                prev_1st_parent != parents[0]
                or new_1st_parent
                and new_1st_parent != parents[0]
            )
        ):
            ...
            commit.file_changes = GitUtils.get_file_changes(
                self._repo_working_dir, will_be_1st_commit_hash, commit.original_id
            )
        ...
        if self._file_info_callback:
            ...
            for change in commit.file_changes:
                if change.type != b"D":
                    ...
                    (filename, mode, blob_id) = self._file_info_callback(
                        change.filename,
                        change.mode,
                        change.blob_id,
    ...
    def _read_stash(self):
        ...
        if (
            self._orig_refs
            and b"refs/stash" in self._orig_refs
            and self._args.refs == ["--all"]
        ):
    ...
    def _setup_input(self, use_done_feature):
        if self._args.stdin:
            ...
            skip_blobs = (
                ...
                and (
                    self._args.replace_text is None
                    or self._file_info_callback is not None
                )
    ...
    def _setup_output(self):
        if not self._args.dry_run:
            ...
            fip_cmd = (
                ["git"]
                + location
                + ["-c", "core.ignorecase=false", "fast-import", "--force", "--quiet"]
            )
    ...
    def _migrate_origin_to_heads(self):
        ...
        # data as well.
        if (
            self._args.sensitive_data_removal
            and not self._args.no_fetch
            and not self._already_ran
            and self._config_settings.get(b"remote.origin.mirror", b"false") != b"true"
        ):
            ...
            print(m)
            ret = subproc.call(
                [arg if arg != '""' else "" for arg in cmd.split()],
                cwd=source_working_dir,
            )
    ...
    def _ref_update(self, target_working_dir):
        ...
        refs_to_nuke = [
            x for x in refs_to_nuke if x.startswith(b"refs/") or x == b"HEAD"
        ]
    ...
    def _compute_metadata(self, metadata_dir, orig_refs):
        ...
        if not self._already_ran:
            commit_renames = {old: new for old, new in self._commit_renames.items()}
        else:
            ...
            commit_renames = {
                old: self._commit_renames.get(newish, newish)
        ...
        # runs
        undo_old_commit_renames = dict(
            (v, k) for (k, v) in old_commit_renames.items() if v != deleted_hash
        )
        ...
        } | {old for (old, new) in self._commit_renames.items() if old != new}
        special_changed_commits = {
            old for (old, new) in old_commit_renames.items() if new == deleted_hash
        }
        first_changes = dict()
        for old, new in self._commit_renames.items():
            if old == new:
    ...
    def _handle_lfs_metadata(self, metadata_dir):
      ...
      NOTE: There were LFS Objects Orphaned by this rewrite recorded in
        ...
        with open(orphaned_lfs_path, "bw") as f:
            differences = (
                self._lfs_object_tracker.source_objects.objects
    ...
    def _record_metadata(self, metadata_dir, orig_refs):
        self._flush_renames()
        commit_renames, ref_maps, first_changes = self._compute_metadata(
            metadata_dir, orig_refs
        )
        ...
        if self._args.sensitive_data_removal:
            ...
            print(
                f"NOTE: First Changed Commit(s) is/are:\n  "
    ...
    def run(self):
        ...
        if self._args.sensitive_data_removal:
            lfs_note = ""
            if (
                self._lfs_object_tracker
                and self._lfs_object_tracker.objects_orphaned == True
            ):
                lfs_note = _(" and LFS Objects Orphaned")
            push_command = "git push --force --mirror origin"
            if self._args.no_fetch:
                if self._args.partial:
                    push_command = "git push --force origin " + " ".join(
                        sorted([decode(x) for x in self._changed_refs])
                    )
                else:
