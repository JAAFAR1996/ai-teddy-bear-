...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"] if ENVIRONMENT == "development" else ["https://teddysecurity.ai"],
...
class EmergencyHandler:
    ...
    async def _determine_emergency_action(self, alert: Dict[str, Any]) -> Optional[EmergencyAction]:
        ...
        if 'APIKeyCompromised' in alert_name:
            ...
            return EmergencyAction(
                ...
                priority="critical",
                target="database",
                parameters={"isolate_tables": ["children", "personal_data"], "notify_legal": True}
            )
    ...
    async def _execute_emergency_action(self, action: EmergencyAction) -> Dict[str, Any]:
        ...
        try:
            ...
            if action.action_type == "rotate_api_keys":
                ...
                result = {"status": "unknown_action", "message": f"إجراء غير معروف: {action.action_type}"}
    ...
    async def _rotate_api_keys(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if params.get("notify_users"):
            await self._send_notification("api_key_rotation", {
    ...
    async def _emergency_data_lockdown(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if params.get("notify_legal"):
            await self._send_notification("legal_notification", {
