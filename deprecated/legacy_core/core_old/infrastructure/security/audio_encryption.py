...
class AudioEncryptionManager:
    ...
    async def create_encryption_session(self, device_id: str, user_id: str) -> str:
        ...
        logger.info("Audio encryption session created", 
                   session_id=session_id,
                   device_id=device_id,
                   user_id=user_id)
    ...
    async def encrypt_audio_data(self, session_id: str, audio_data: bytes,
                                sequence_number: int = 0) -> EncryptedAudioPacket:
        ...
        try:
            ...
            logger.debug("Audio data encrypted",
                        session_id=session_id,
    ...
    async def decrypt_audio_data(self, packet: EncryptedAudioPacket) -> bytes:
        ...
        try:
            ...
            logger.debug("Audio data decrypted",
                        session_id=session_id,
            ...
            logger.error("Audio decryption failed",
                        session_id=session_id,
                        error=str(e))
            raise
    ...
    async def encrypt_audio_stream(self, session_id: str, 
                                 audio_chunks: List[bytes]) -> List[EncryptedAudioPacket]:
        ...
        logger.info("Audio stream encrypted",
                   session_id=session_id,
    ...
    async def decrypt_audio_stream(self, packets: List[EncryptedAudioPacket]) -> bytes:
        ...
        logger.info("Audio stream decrypted",
                   packets=len(packets),
