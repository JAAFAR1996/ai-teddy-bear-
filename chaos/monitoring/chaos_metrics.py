...
class ChaosMetricsCollector:
    ...
    async def _collect_system_metrics(self, experiment_id: str):
        ...
        for service_name, endpoint in self.service_endpoints.items():
            try:
                ...
                self.metrics_buffer.append(ChaosMetric(
    ...
    async def _collect_health_snapshot(self, experiment_id: str):
        ...
        throughput = healthy_services * 10.0  # Assume 10 RPS per healthy service
        ...
        snapshot = SystemHealthSnapshot(
            ...
            safety_violations=0  # Will be updated by safety monitoring
        )
    ...
    def generate_experiment_report(self, experiment_id: str) -> Dict[str, Any]:
        ...
        experiment_metrics = [m for m in self.metrics_buffer if m.experiment_id == experiment_id]
        experiment_snapshots = [s for s in self.health_snapshots if s.experiment_id == experiment_id]
        
        if not experiment_metrics and not experiment_snapshots:
            return {"error": f"No metrics found for experiment {experiment_id}"}
        
        # Calculate aggregated metrics
        response_times = [m.metric_value for m in experiment_metrics if m.metric_name == "response_time"]
        memory_usage = [m.metric_value for m in experiment_metrics if m.metric_name == "memory_usage"]
        ...
        for service_name in self.service_endpoints.keys():
            service_metrics = [m for m in experiment_metrics if m.service_name == service_name]
            service_response_times = [m.metric_value for m in service_metrics if m.metric_name == "response_time"]
        ...
        return {
            "experiment_id": experiment_id,
            "duration_minutes": len(experiment_snapshots) / 6,  # Snapshots every 10 seconds
    ...
    def _calculate_experiment_grade(
        self, health_ratio: float, error_rate: float, response_time: float, safety_violations: int
    ) -> str:
    ...
    def export_metrics_to_prometheus(self, experiment_id: str) -> bool:
        ...
        try:
            ...
            for metric in experiment_metrics:
                prometheus_line = (
                    f"chaos_{metric.metric_name}{{"
                    f"experiment_id=\"{metric.experiment_id}\","
                    f"service=\"{metric.service_name}\""
                    f"}} {metric.metric_value} {int(metric.timestamp.timestamp() * 1000)}"
                )
    ...
    def cleanup_old_metrics(self, retention_hours: int = 24):
        ...
        self.health_snapshots = [s for s in self.health_snapshots if s.timestamp > cutoff_time]
        ...
        if cleaned_metrics > 0 or cleaned_snapshots > 0:
            logger.info(f"🧹 Cleaned up {cleaned_metrics} metrics and {cleaned_snapshots} snapshots")
    ...
    def get_real_time_dashboard_data(self, experiment_id: str) -> Dict[str, Any]:
        ...
        current_status = {
            "healthy_services": latest_snapshot.services_healthy if latest_snapshot else 0,
            "total_services": latest_snapshot.services_total if latest_snapshot else 0,
            "health_ratio": (latest_snapshot.services_healthy / latest_snapshot.services_total) if latest_snapshot else 0,
            "avg_response_time": latest_snapshot.avg_response_time if latest_snapshot else 0,
        ...
        for service_name in self.service_endpoints.keys():
            ...
            response_times = [m.metric_value for m in service_metrics if m.metric_name == "response_time"]
