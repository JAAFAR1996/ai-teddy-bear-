...
class ImportsChecker:
    ...
    def check_imports_in_file(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            # البحث عن استدعاءات value_objects.py
    ...
    def check_file_exists(self, import_path: str) -> bool:
        ...
        # تحويل مسار الاستدعاء إلى مسار فعلي
...
"""
        ...
        if domain_dir.exists():
            domains = [d for d in domain_dir.iterdir() if d.is_dir() and not d.name.startswith('__')]
        ...
        if app_dir.exists():
            apps = [d for d in app_dir.iterdir() if d.is_dir() and not d.name.startswith('__')]
            structure_report += f"- عدد Application Domains: {len(apps)}\n"
            
            for app in apps:
                services_count = len(list((app / "services").glob("*.py"))) if (app / "services").exists() else 0
                uc_count = len(list((app / "use_cases").glob("*.py"))) if (app / "use_cases").exists() else 0
...
def main():
    print("=" * 60)
    print("🔍 بدء فحص الاستدعاءات والبنية...")
    print("=" * 60)
    ...
    # طباعة ملخص سريع
    lines = report.split('\n')
    summary_lines = [line for line in lines if 'إجمالي' in line or 'ملفات تحتوي' in line]
