...
class ComprehensiveFixer:
    ...
    def fix_imports_in_file(self, file_path: Path) -> List[str]:
        ...
        try:
            ...
            patterns_to_fix = [
                ...
                (r'from \.use_cases\.(\w+) import', r'from src.application.accessibility.use_cases.\1 import'),
                (r'from \.dto\.(\w+) import', r'from src.application.accessibility.dto.\1 import'),
                (r'from \.\.value_objects\.(\w+) import', r'from src.domain.accessibility.value_objects.\1 import'),
                (r'from \.\.entities\.(\w+) import', r'from src.domain.accessibility.entities.\1 import'),
    ...
    def verify_structure_completeness(self) -> Dict:
        ...
        if domain_dir.exists():
            for domain in domain_dir.iterdir():
                if domain.is_dir() and not domain.name.startswith('__'):
                    entities_count = len(list((domain / "entities").glob("*.py"))) if (domain / "entities").exists() else 0
                    vo_count = len(list((domain / "value_objects").glob("*.py"))) if (domain / "value_objects").exists() else 0
        ...
        # حساب نقاط البنية
        complete_domains = sum(1 for d in verification['domains_created'] if d['complete'])
        total_domains = len(verification['domains_created'])
        large_files = len(verification['large_files_remaining'])
        
        if total_domains > 0:
            verification['structure_score'] = int((complete_domains / total_domains) * 100)
            if large_files == 0:
                verification['structure_score'] = min(verification['structure_score'] + 20, 100)
    ...
    def count_lines_recovered(self) -> Dict:
        ...
        if lines_info['total_lines_original'] > 0:
            lines_info['recovery_percentage'] = int((lines_info['total_lines_new'] / lines_info['total_lines_original']) * 100)
