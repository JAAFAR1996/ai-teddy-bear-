...
class GodClassSplitter:
    ...
    def analyze_python_file(self, file_path: Path) -> Tuple[List[ClassInfo], List[FunctionInfo]]:
            ...
            for node in ast.walk(tree):
                if isinstance(node, ast.ClassDef):
                    ...
                    class_info = ClassInfo(
                        ...
                        size_lines=getattr(node, 'end_lineno', node.lineno + 10) - node.lineno
                    )
                    ...
                    func_info = FunctionInfo(
                        ...
                        size_lines=getattr(node, 'end_lineno', node.lineno + 5) - node.lineno,
                        dependencies=dependencies
                    )
    ...
    def create_split_plan(self, classes: List[ClassInfo], functions: List[FunctionInfo], original_file: Path) -> Dict:
        """إنشاء خطة التقسيم"""
        plan = {
            'original_file': original_file,
            'splits': [],
            'base_module': original_file.stem,
            'package_dir': original_file.parent / original_file.stem.replace('_service', '').replace('_', '_components')
        }
    ...
    def _split_large_class(self, class_info: ClassInfo) -> List[Dict]:
        ...
        for method in class_info.methods:
            if any(keyword in method.lower() for keyword in ['init', 'setup', 'config']):
            ...
            elif any(keyword in method.lower() for keyword in ['process', 'execute', 'run', 'handle']):
                method_groups['processing'].append(method)
            elif any(keyword in method.lower() for keyword in ['save', 'store', 'database', 'persist']):
                method_groups['storage'].append(method)
            elif any(keyword in method.lower() for keyword in ['notify', 'send', 'alert', 'report']):
...
'''
        ...
        for split in plan['splits']:
            if split['type'] == 'class':
                init_content += f"from .{split['filename'][:-3]} import {split['name']}\n"
...
'''
        ...
        facade_file = plan['original_file'].with_name(f"{plan['base_module']}_refactored.py")
