...
class ContentSimilarityAnalyzer:
    ...
    def analyze_service_group(self, group_name: str) -> Dict:
        ...
        results = {"identical": [], "similar": [], "unique": []}
        ...
        for file1 in files:
            for file2 in files:
                ...
                if data1["hash"] == data2["hash"]:
                    ...
                    # فحص التشابه
                    similarity = self.calculate_similarity_percentage(
                    ...
                    if similarity >= 90.0:
                        ...
                        print(
                            f"  🔄 متشابه {similarity:.1f}%: {file1.name} ↔ {file2.name}"
                        )
                        ...
                        print(
                            f"  📝 مراجعة {similarity:.1f}%: {file1.name} ↔ {file2.name}"
                        )
    ...
    def execute_cleanup_actions(self, group_name: str, analysis: Dict) -> Dict:
        ...
        final_delete_dir = (
            self.base_path / "deleted" / "duplicates" / "final_cleanup" / group_name
        )
        final_delete_dir.mkdir(parents=True, exist_ok=True)

        # إنشاء مجلد للدمج
        merge_dir = (
            self.base_path / "deleted" / "duplicates" / "merge_needed" / group_name
        )
        ...
        try:
            ...
            for identical in analysis["identical"]:
                ...
                if (
                    str(file1_path) not in processed_files
                    and str(file2_path) not in processed_files
                ):
...
"""
    ...
    def run_complete_similarity_analysis(self) -> Dict:
        ...
        for group_name in service_groups:
            ...
            all_results[group_name] = {"analysis": analysis, "actions": actions}

        # إنشاء التقرير
        report_content = self.generate_detailed_analysis_report(all_results)
        report_path = (
            self.base_path / "deleted" / "reports" / "CONTENT_SIMILARITY_ANALYSIS.md"
        )
