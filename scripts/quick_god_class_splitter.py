...
class QuickGodClassSplitter:
    ...
    def split_god_class(self, file_path: Path, lines: int) -> Dict[str, str]:
        ...
        try:
            ...
            new_structure = self._create_ddd_structure(
                components, domain_name, file_path.parent
            )
    ...
    def _create_ddd_structure(
        self, components: Dict, domain_name: str, base_path: Path
    ) -> Dict[str, str]:
        """إنشاء بنية DDD"""
        new_files = {}
        ...
        # إنشاء البنية
        structure = {
            "domain/aggregates": components.get("classes", [])[
                :2
            ],  # أول 2 كلاسات كـ aggregates
            "domain/entities": components.get("classes", [])[
                2:4
            ],  # التالية كـ entities
            "domain/value_objects": components.get("dataclasses", []),
            "application/use_cases": [
                f
                for f in components.get("functions", [])
                if "execute" in f or "process" in f
            ][:3],
            "application/services": [
                f
                for f in components.get("functions", [])
                if "validate" in f or "calculate" in f
            ][:3],
        ...
        for path_key, items in structure.items():
            ...
            file_content = self._create_file_content(
                items, components.get("imports", []), domain_name
            )
        ...
        orchestrator_path = (
            domain_path / "application" / "services" / f"{domain_name}_orchestrator.py"
        )
        orchestrator_path.write_text(orchestrator_content, encoding="utf-8")
        new_files[f"application/services/{domain_name}_orchestrator.py"] = (
    ...
    def _create_file_content(
        self, items: List[str], imports: List[str], domain_name: str
    ) -> str:
        """إنشاء محتوى الملف"""
...
'''
    ...
    def run_splitting(self):
        ...
        print(
            f"  Average reduction: {(sum(r['original_lines'] for r in self.report.values()) / len(self.report) / (total_files_created / len(self.report)) if self.report else 0):.0f} lines per file"
        )
