...
async def main():
    ...
    try:
        ...
        from src.domain.value_objects import ChildId, DeviceId, ParentId
        from src.infrastructure.messaging.event_bus_integration import (
            EventDispatchContext, get_event_bus, get_event_dispatcher)
        ...
        print(
            f"   Safety Settings: {child.safety_settings.content_filter_level.value} filtering"
        )
        ...
        for content, emotion, confidence in messages:
            ...
            print(
                f'👶 Child: "{content}" (emotion: {emotion.value}, confidence: {confidence})'
            )
        ...
        print(
            f"✅ Conversation ended after {conversation.get_duration_minutes():.1f} minutes"
        )
        ...
        optimized_profile = child_service.recommend_voice_profile_adjustments(
            child=child, recent_conversations=[conversation]
        )
        ...
        if optimized_profile:
            ...
            print(
                f"   Emotion sensitivity: {optimized_profile.emotion_sensitivity:.2f}"
            )
        ...
        print("\n📈 Event Bus Metrics:")
        print(
            f"   Published events: {metrics['publisher_metrics']['events_published']}"
        )
        ...
        print(
            f"   Average publish time: {metrics['publisher_metrics']['average_publish_time']:.3f}s"
        )

        if "consumer_metrics" in metrics:
            print(
                f"   Consumed events: {metrics['consumer_metrics']['events_consumed']}"
            )
            print(
                f"   Processed events: {metrics['consumer_metrics']['events_processed']}"
            )
            print(
                f"   Processing success rate: {metrics['consumer_metrics']['success_rate']:.2%}"
            )
        ...
        consumer_task = asyncio.create_task(event_bus.start_event_processing())
        ...
        print("\nTroubleshooting:")
        print(
            "1. Ensure Kafka is running: docker-compose -f docker-compose.kafka.yml up -d"
        )
