            ...
            try:
                if group_dir.exists() and not any(group_dir.iterdir()):
                    ...
                    if group_dir.exists():
                        ...
                        print(f"  📁 المجلد ليس فارغاً: {group_dir} ({len(remaining_files)} ملفات)")
                    else:
...
"""
        ...
        for move_info in self.moved_services:
            ...
            new_path = move_info["target"].replace(str(self.base_path), "").replace("\\", ".").replace("/", ".").strip(".")
...
"""
    ...
    def generate_relocation_report(self, validation_results: Dict, 
                                 relocation_results: Dict, 
                                 cleanup_results: Dict) -> str:
...
"""
    ...
    def run_complete_relocation(self) -> Dict:
        ...
        imports_guide_path = self.base_path / "deleted" / "reports" / "IMPORTS_UPDATE_GUIDE.md"
        ...
        report_path = self.base_path / "deleted" / "reports" / "CLEAN_ARCHITECTURE_RELOCATION.md"
