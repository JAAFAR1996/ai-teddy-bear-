...
class CleanArchitectureRelocator:
    ...
    def validate_source_files(self) -> Dict[str, List[str]]:
        ...
        logger.info("🔍 التحقق من وجود الملفات المصدر...")
        validation_results = {
            "found": [],
            "missing": [],
            "total_found": 0,
        ...
        for group_name, group_info in self.relocation_plan.items():
            ...
            if not group_dir.exists():
                logger.info(f"  ❌ مجلد غير موجود: {group_dir}")
            ...
            for service_file in group_info["services"]:
                ...
                if source_path.exists():
                    ...
                    logger.info(f"  ✅ موجود: {service_file} في {group_name}")
                else:
                    validation_results["missing"].append(f"{group_name}/{service_file}")
                    logger.info(f"  ❌ مفقود: {service_file} في {group_name}")
        ...
        logger.info("\n📊 نتائج التحقق:")
        logger.info(f"  ✅ ملفات موجودة: {validation_results['total_found']}")
        logger.info(f"  ❌ ملفات مفقودة: {validation_results['total_missing']}")
    ...
    def create_target_directories(self) -> Dict[str, bool]:
        ...
        for group_name, group_info in self.relocation_plan.items():
            ...
            try:
                ...
                logger.info(f"  ✅ تم إنشاء: {target_dir}")
            ...
            for service_file in group_info["services"]:
                ...
                try:
                    if source_path.exists():
                        ...
                        logger.info(
                            f"  ✅ نُقل: {service_file} → {group_info['target_dir']}"
                        )
                        source_path.unlink()
                        logger.info(f"    🗑️  حُذف الأصل: {source_path}")
                    else:
                        error_info = {
                            "service": service_file,
                            "group": group_name,
                        ...
                        logger.info(f"  ❌ فشل: {service_file} (غير موجود)")
                except Exception as e:
                    error_info = {
                        "service": service_file,
                        "group": group_name,
                    ...
                    logger.info(f"  ❌ فشل: {service_file} - {str(e)}")
        ...
        for group_name in self.relocation_plan.keys():
            ...
            try:
                if group_dir.exists() and not any(group_dir.iterdir()):
                    ...
                    logger.info(f"  🗑️  حُذف المجلد الفارغ: {group_dir}")
                else:
                    ...
                    if group_dir.exists():
                        ...
                        logger.info(f"  ❓ المجلد غير موجود: {group_dir}")
            except Exception as e:
                cleanup_results[group_name] = False
...
"""
    ...
    def run_complete_relocation(self) -> Dict:
        ...
        logger.info("=" * 60)
        logger.info("🎯  CLEAN ARCHITECTURE RELOCATOR")
        logger.info("🏗️  MOVING SERVICES TO CORRECT LOCATIONS")
        logger.info("=" * 60)
        ...
        if validation_results["total_missing"] > 0:
            ...
            logger.info("  💡 سيتم تخطي الملفات المفقودة والمتابعة مع الموجودة")
        ...
        logger.info(f"📝 دليل الاستيرادات: {imports_guide_path}")
        logger.info(f"✅ خدمات منقولة: {relocation_results['total_moved']}")
        logger.info(f"❌ خدمات فشلت: {relocation_results['total_failed']}")
        return {
...
def main():
    ...
    try:
        ...
        if results["relocation"]["total_moved"] > 0:
            logger.info("\n✅ تم النقل بنجاح!")
            logger.info("🎯 البنية الجديدة جاهزة حسب Clean Architecture")
        else:
            logger.info("\n⚠️  لم يتم نقل أي ملفات!")
    except Exception as e:
        logger.info(f"❌ خطأ في عملية إعادة التوزيع: {e}")
        import traceback
