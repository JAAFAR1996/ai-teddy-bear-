...
class AdvancedDirectoriesAnalyzer:
    ...
    def classify_file(self, file_path: Path) -> Tuple[str, bool, bool, bool]:
        ...
        is_test = (
            ...
            name.startswith('conftest') or
            '_test' in name
        )
        
        # ملفات التكوين
        is_config = (
            suffix in ['.json', '.yaml', '.yml', '.ini', '.cfg'] or
            'config' in path_str
        )
        ...
        for dir_name in self.target_dirs:
            ...
            for file_path in dir_path.rglob("*"):
                if file_path.is_file():
                    try:
                        category, is_test, is_config, is_generated = self.classify_file(file_path)
                        
                        # تحديد الملفات المولدة للحذف
                        if is_generated:
    ...
    def move_exact_duplicates(self) -> None:
        ...
        for hash_value, file_paths in self.duplicates.items():
            if len(file_paths) > 1:
                # الاحتفاظ بالملف في tests إذا وُجد، وإلا الأحدث
    ...
    def _choose_primary_file(self, file_paths: List[str]) -> str:
        ...
        for priority_dir in priority_order:
            for file_path in file_paths:
                if file_path.startswith(priority_dir + "/") or file_path.startswith(priority_dir + "\\"):
                    return file_path
        
        # إذا لم توجد أولوية، اختر الأحدث
...
"""
            ...
            merged_content = merge_header
            merged_content += f"# ========== Content from {Path(path1).name} ==========\n"
            merged_content += content1 + "\n\n"
            merged_content += f"# ========== Content from {Path(path2).name} ==========\n"
            merged_content += content2 + "\n"
    ...
    def generate_comprehensive_report(self) -> str:
        """إنشاء تقرير شامل"""
        report_path = self.reports_dir / f"advanced_directories_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    ...
    try:
        ...
        print(f"\n{'='*70}")
        print("📊 تقرير فحص المجلدات المتقدمة")
        print(f"{'='*70}")
