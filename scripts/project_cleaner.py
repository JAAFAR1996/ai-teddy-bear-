...
class ProjectCleaner:
    ...
    def __init__(self, project_root: str = ".", analysis_file: str = "scripts/project_analysis.json"):
        ...
        try:
            ...
            print("❌ لم يتم العثور على تقرير التحليل. يرجى تشغيل project_analyzer.py أولاً")
            sys.exit(1)
    ...
    def remove_trash_files(self, dry_run: bool = False):
        ...
        trash_patterns = [
            '*_old.py', '*_backup.py', '*_temp.py', '*_copy.py', '*_bak.py',
        ...
        for file_info in self.analysis.get('detailed_analysis', []):
            if file_info.get('importance') in ['critical', 'high', 'medium']:
                ...
                if suggested_location and os.path.exists(current_path):
                    ...
                    if not Path(current_path).is_relative_to(Path(suggested_location).parent):
                        if not dry_run:
                            self._move_file_with_imports(current_path, suggested_location)
    ...
    def _backup_and_delete(self, file_path: str):
        ...
        try:
            ...
            backup_path = os.path.join(self.backup_dir, os.path.relpath(file_path, self.project_root))
...
"""
        ...
        if self.report['deleted_files']:
            ...
            for item in self.report['deleted_files'][:10]:  # أول 10 فقط
                report_content += f"| `{Path(item['file']).name}` | {item['reason']} |\n"
        ...
        if self.report['merged_files']:
            ...
            for item in self.report['merged_files'][:10]:  # أول 10 فقط
                ...
                report_content += f"| `{deleted_name}` | `{kept_name}` | {item['type']} |\n"
...
def main():
    ...
    parser.add_argument('--analysis-file', default='scripts/project_analysis.json',
