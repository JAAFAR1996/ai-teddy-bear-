...
class AdvancedFileComparator:
    ...
    def ensure_directories(self) -> None:
        ...
        logger.info(f"[SUCCESS] تم إنشاء المجلدات: {self.deleted_dir}, {self.reports_dir}")
            ...
            for path2 in file_paths[i+1:]:
                if self._files_compatible_for_comparison(path1, path2):
                    ...
                    if similarity >= similarity_threshold:
                        ...
                        logger.info(f"[SIMILAR] ملفات مشابهة: {path1} <-> {path2} ({similarity:.2%})")
    ...
    def move_exact_duplicates(self) -> None:
        ...
        for hash_value, file_paths in self.duplicates.items():
            if len(file_paths) > 1:
                ...
                newest_file = max(file_paths, key=lambda p: self.file_registry[p].modified_time)
                ...
                for file_path in file_paths:
                    if file_path != newest_file:
                        try:
                            ...
                            moved_count += 1
                            logger.info(f"[MOVED] تم نقل الملف المكرر: {file_path} -> {dest_path}")
    ...
    def generate_detailed_report(self) -> str:
        ...
        report_path = self.reports_dir / f"src_deduplication_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
