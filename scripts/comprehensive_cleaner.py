...
class ComprehensiveCleaner:
    ...
    def __init__(
        self, analysis_file: str = "full_project_analysis.json", dry_run: bool = False
    ):
    ...
    def phase1_major_cleanup(self):
        ...
        trash_files = [
            f for f in self.analysis["detailed_analysis"] if f["importance"] == "trash"
        ]
    ...
    def phase2_remove_duplicates(self):
        ...
        exact_duplicates = [
            d for d in self.analysis["duplicate_candidates"] if d["type"] == "exact"
        ]
            ...
            if len(files) > 1:
                ...
                for file_path in files:
                    if file_path != best_file:
                        ...
                        print(
                            f"  🔄 دمج: {Path(file_path).name} -> {Path(best_file).name}"
                        )
    ...
    def _backup_and_delete(self, file_path: Path):
        ...
        try:
            # نسخ احتياطي
            backup_path = Path(self.backup_dir) / file_path.relative_to(
                self.project_root
            )
    ...
    def _remove_print_statements(self, file_path: Path):
        ...
        try:
            ...
            content = re.sub(
                r"^\s*print\s*\(.*?\)\s*$", "", content, flags=re.MULTILINE
            )
...
"""
        ...
        security_warnings = [
            x for x in self.report["phase4_fix"] if x["action"] == "security_warning"
        ]
...
def main():
    ...
    parser.add_argument(
        "--analysis-file", default="full_project_analysis.json", help="مسار ملف التحليل"
    )
    parser.add_argument("--force", action="store_true", help="تشغيل بدون طلب تأكيد")
    ...
    cleaner = ComprehensiveCleaner(
        analysis_file=args.analysis_file, dry_run=args.dry_run
    )
