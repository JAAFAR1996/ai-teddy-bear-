...
def verify_integration_success():
    ...
    domain_count = len([d for d in domain_dir.iterdir() if d.is_dir() and not d.name.startswith('__')])
    app_count = len([d for d in app_dir.iterdir() if d.is_dir() and not d.name.startswith('__')])
