...
class ProjectHealthChecker:
    ...
    def check_required_files(self):
        ...
        for file_path in required_files:
            if Path(file_path).exists():
                ...
                self.checks['structure']['errors'].append(f"Missing required file: {file_path}")
    ...
    def check_directory_structure(self):
        ...
        for dir_path in required_dirs:
            if Path(dir_path).is_dir():
                ...
                self.checks['structure']['failed'] += 1
                self.checks['structure']['errors'].append(f"Missing required directory: {dir_path}")
        ...
        try:
            ...
            if Path('requirements.txt').exists():
                ...
                if requirements:
                    ...
                    self.checks['dependencies']['errors'].append("requirements.txt is empty")
                ...
                for lib in essential_libs:
                    if any(lib in req for req in requirements_lower):
                        ...
                        self.checks['dependencies']['failed'] += 1
                        self.checks['dependencies']['errors'].append(f"Missing essential dependency: {lib}")
            else:
                self.checks['dependencies']['failed'] += 1
                self.checks['dependencies']['errors'].append("requirements.txt not found")
                
        except Exception as e:
            self.checks['dependencies']['failed'] += 1
            self.checks['dependencies']['errors'].append(f"Error reading requirements: {str(e)}")
        ...
        for py_file in python_files:
            try:
                ...
                if 'eval(' in content and 'ast.literal_eval' not in content:
                    ...
                    self.checks['security']['errors'].append(f"Unsafe eval in {py_file}")
                elif 'exec(' in content and '# SECURITY' not in content:
                    self.checks['security']['failed'] += 1
                    self.checks['security']['errors'].append(f"Unsafe exec in {py_file}")
                else:
    ...
    def run_basic_tests(self):
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.checks['tests']['errors'].append("pytest not installed or not working")
        except:
            self.checks['tests']['failed'] += 1
    ...
    def generate_report(self):
        ...
        health_score = (total_passed / total_checks * 100) if total_checks > 0 else 0
