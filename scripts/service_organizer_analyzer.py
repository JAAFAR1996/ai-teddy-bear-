...
class ServiceOrganizerAnalyzer:
    ...
    def categorize_services_by_functionality(self) -> Dict:
        ...
        # تصنيف كل خدمة
        all_services = (self.services["other_services"] + 
        ...
        for service_path in all_services:
            ...
            if any(ai_term in service_name.lower() for ai_term in 
                  ['ai', 'llm', 'gpt', 'ml', 'intelligence']):
                functional_groups["ai_ml"].append(service_path)
                
            elif any(audio_term in service_name.lower() for audio_term in 
                    ['audio', 'voice', 'speech', 'transcription', 'synthesis', 'tts', 'stt']):
                functional_groups["audio_processing"].append(service_path)
                
            elif any(comm_term in service_name.lower() for comm_term in 
                    ['email', 'sms', 'push', 'notification', 'streaming']):
                functional_groups["communication"].append(service_path)
                
            elif any(person_term in service_name.lower() for person_term in 
                    ['personalization', 'personality', 'accessibility']):
                functional_groups["personalization"].append(service_path)
                
            elif any(monitor_term in service_name.lower() for monitor_term in 
                    ['health', 'monitor', 'rate', 'issue_tracker']):
                functional_groups["monitoring"].append(service_path)
                
            elif any(data_term in service_name.lower() for data_term in 
                    ['data', 'memory', 'cache', 'cleanup', 'event_sourcing']):
                functional_groups["data_management"].append(service_path)
                
            elif any(sec_term in service_name.lower() for sec_term in 
                    ['security', 'he_integration', 'moderation']):
                functional_groups["security"].append(service_path)
                
            elif any(parent_term in service_name.lower() for parent_term in 
                    ['parent', 'dashboard', 'report']):
                functional_groups["parent_features"].append(service_path)
                
            elif any(child_term in service_name.lower() for child_term in 
                    ['child', 'interaction', 'story', 'conversation']):
                functional_groups["child_features"].append(service_path)
                
            elif any(infra_term in service_name.lower() for infra_term in 
                    ['external', 'service_registry', 'scheduler', 'base']):
                functional_groups["infrastructure"].append(service_path)
                
            elif any(ui_term in service_name.lower() for ui_term in 
                    ['graphql', 'resolver', 'presentation']):
                functional_groups["ui_presentation"].append(service_path)
                
            elif any(test_term in service_path.lower() for test_term in 
                    ['test', 'simple_', 'edge_ai']):
    ...
    def detect_duplicate_functionalities(self, functional_groups: Dict) -> Dict:
        ...
        if len(ai_services) > 1:
            duplicates["ai_services"] = {
                ...
                "duplicates": [s for s in ai_services if s != self._select_primary_service(ai_services, "ai")],
        ...
        if len(audio_services) > 1:
            duplicates["audio_services"] = {
                ...
                "duplicates": [s for s in audio_services if s != self._select_primary_service(audio_services, "audio")],
        ...
        if len(monitoring_services) > 1:
            duplicates["monitoring_services"] = {
                ...
                "duplicates": [s for s in monitoring_services if s != self._select_primary_service(monitoring_services, "health")],
                "merge_strategy": "unified_monitoring_service"
            }
        
        # فحص خدمات التخزين المؤقت المكررة
        cache_services = [s for s in functional_groups["data_management"] if 'cache' in s.lower()]
        if len(cache_services) > 1:
            duplicates["cache_services"] = {
                "primary": self._select_primary_service(cache_services, "cache"),
                "duplicates": [s for s in cache_services if s != self._select_primary_service(cache_services, "cache")],
    ...
    def _select_primary_service(self, services: List[str], service_type: str) -> str:
        ...
        app_services = [s for s in services if 'src/application/services/' in s and 'test' not in s.lower()]
        if app_services:
    ...
    def _get_clean_arch_location(self, service_path: str) -> str:
        ...
        elif 'external' in service_path.lower() or 'integration' in service_path.lower():
    ...
    def execute_service_organization(self, operations: List[Dict]) -> Dict:
        ...
        for operation in operations:
            try:
                ...
                print(f"  ✅ {operation['group']}: {len(operation['services_to_merge']) + 1} ملفات")
    ...
    def generate_organization_report(self, functional_groups: Dict, duplicates: Dict, results: Dict) -> str:
...
"""
    ...
    def run_complete_organization(self) -> Dict:
        ...
        report_content = self.generate_organization_report(functional_groups, duplicates, results)
        report_path = self.base_path / "deleted" / "reports" / "SERVICE_ORGANIZATION_REPORT.md"
        ...
        print(f"📊 خدمات معالجة: {sum(len(services) for services in functional_groups.values())}")
