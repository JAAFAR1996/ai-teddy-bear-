...
class ExceptionHandlerFixer:
    ...
    def _detect_context(self, code_block: str) -> str:
        ...
        if any(keyword in code_lower for keyword in ["open(", "file", "read", "write"]):
            ...
            for keyword in ["request", "http", "url", "connection"]
        ):
            return "network"
        elif any(
            keyword in code_lower for keyword in ["database", "db", "sql", "query"]
        ):
            return "database"
        elif any(
            keyword in code_lower for keyword in ["json", "yaml", "parse", "decode"]
        ):
            return "parsing"
    ...
    def fix_file(self, analysis: Dict) -> bool:
        ...
        # تطبيق الإصلاحات
        fixes_applied = 0
        for issue in reversed(
            analysis["issues"]
        ):  # من الأسفل للأعلى لتجنب تغيير المواضع
            try:
                match = issue["match"]
    ...
    def fix_project_exceptions(self, target_dirs: List[str] = None) -> Dict:
        ...
        for target_dir in target_dirs:
            ...
            for py_file in python_files:
                ...
                if any(
                    skip in str(py_file) for skip in ["test_", "__pycache__", ".backup"]
                ):
                    continue
