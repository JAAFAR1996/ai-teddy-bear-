...
class EdgeAIDemo:
    ...
    def _generate_demo_data(self) -> Dict[str, Any]:
        ...
        return {
            "audio_samples": {
                ...
                "angry_child": np.random.uniform(-0.4, 0.4, 16000),   # High energy, erratic
    ...
    async def demo_wake_word_detection(self) -> None:
        ...
        try:
            ...
            for sample_name, audio_data in audio_samples.items():
                ...
                start_time = time.time()
                detected, confidence = await self.edge_ai_manager.wake_word_detector.detect_wake_word(audio_data)
    ...
    async def demo_emotion_analysis(self) -> None:
        ...
        try:
            ...
            for sample_name, audio_data in audio_samples.items():
                ...
                features = await self.edge_ai_manager.feature_extractor.extract_features(audio_data)
                print(f"📊 Feature extraction time: {features.extraction_time_ms:.2f}ms")
                
                # Analyze emotion
                start_time = time.time()
                emotion_result = await self.edge_ai_manager.emotion_analyzer.analyze_emotion(features)
                ...
                # Show top 3 emotions
                top_emotions = sorted(emotion_result.emotion_scores.items(), 
    ...
    async def demo_safety_checking(self) -> None:
        ...
        try:
            ...
            mock_features = await self.edge_ai_manager.feature_extractor.extract_features(
            ...
            for sample_name, text in text_samples.items():
                ...
                if safety_result.detected_issues:
                    print(f"🚨 Issues detected: {', '.join(safety_result.detected_issues)}")
        ...
        try:
            ...
            for audio_key, text_key, description in scenarios:
                ...
                result = await self.edge_ai_manager.process_on_edge(
                    self.demo_data["audio_samples"][audio_key],
                    text_samples[text_key]
                )
                total_time = (time.time() - start_time) * 1000
                
                print(f"⏱️  Total processing time: {total_time:.2f}ms")
                print(f"🎯 Wake word detected: {'YES' if result.wake_word_detected else 'NO'}")
                print(f"☁️  Should process cloud: {'YES' if result.should_process_cloud else 'NO'}")
                print(f"🔥 Priority: {result.priority}/10")
                print(f"🔮 Confidence: {result.confidence:.3f}")
                print(f"💻 Device load: {result.device_load:.3f}")
                
                if result.initial_emotion:
                    print(f"😊 Emotion: {result.initial_emotion.primary_emotion} ({result.initial_emotion.confidence:.3f})")
                
                if result.safety_check:
                    print(f"🛡️  Safety: {'PASS' if result.safety_check.passed else 'FAIL'} ({result.safety_check.safety_score:.3f})")
    ...
    async def demo_device_optimization(self) -> None:
        ...
        try:
            ...
            for device_name, specs in device_specs.items():
                ...
                print(f"⚙️  Optimized mode: {device_manager.config.processing_mode.value}")
                print(f"🎙️  Wake word model: {device_manager.config.wake_word_model.value}")
                ...
                print(f"📊 Efficiency rating: {self._calculate_efficiency(processing_time, specs):.1f}/10")
    ...
    def _calculate_efficiency(self, processing_time_ms: float, specs: Dict[str, Any]) -> float:
    ...
    async def demo_integration_service(self) -> None:
        ...
        try:
            ...
            await self.integration_service.configure_for_device(
                "ESP32-S3", 
                self.demo_data["device_specs"]["ESP32-S3"]
            )
            ...
            print(f"   📱 Edge-only responses: {stats['integration_stats']['edge_only_responses']}")
            print(f"   ☁️  Cloud-assisted: {stats['integration_stats']['cloud_assisted_responses']}")
            print(f"   🔄 Hybrid responses: {stats['integration_stats']['hybrid_responses']}")
    ...
    async def demo_performance_benchmarks(self) -> None:
        ...
        try:
            ...
            configs = {
                "Ultra Fast": EdgeModelConfig(processing_mode=EdgeProcessingMode.ULTRA_LOW_LATENCY),
                "Balanced": EdgeModelConfig(processing_mode=EdgeProcessingMode.BALANCED),
                "High Accuracy": EdgeModelConfig(processing_mode=EdgeProcessingMode.HIGH_ACCURACY)
            }
            ...
            for config_name, config in configs.items():
                ...
                print(f"   ⚡ Avg processing time: {stats['processing_stats']['average_processing_time']:.2f}ms")
    ...
    def demo_system_capabilities(self) -> None:
        ...
        print(f"   🎵 Audio Processing Available: {'✅ YES' if AUDIO_PROCESSING_AVAILABLE else '❌ NO'}")
