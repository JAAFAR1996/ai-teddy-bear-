...
try:
    ...
    from src.infrastructure.security.homomorphic_encryption import (
        TENSEAL_AVAILABLE, HEConfig, HEScheme, HomomorphicEncryption,
        ProcessingMode)
...
class HEDemo:
    ...
    def _generate_demo_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expected_emotions": ["happy", "sad", "excited", "calm", "mixed"],
        }
    ...
    async def demo_basic_encryption(self) -> None:
        ...
        try:
            ...
            encrypted_data = await self.he_service.encrypt_voice_features(
                features, child_id
            )
            ...
            print(
                f"⚙️  Processing capabilities: {encrypted_data.processing_capabilities}"
            )
    ...
    async def demo_batch_processing(self) -> None:
        ...
        try:
            ...
            print(
                f"🔄 Preparing batch of {len(self.demo_data['voice_features_samples'])} samples..."
            )

            for i, (features, child_id) in enumerate(
                zip(
            ...
            print(
                f"⚡ Average time per sample: {batch_processing_time/len(results):.2f}ms"
            )
    ...
    async def demo_integration_service(self) -> None:
        ...
        try:
            ...
            with patch(
                "core.infrastructure.security.he_integration_service.AUDIO_PROCESSING_AVAILABLE",
                False,
            ):
            ...
            print(
                f"  ✅ End-to-End Encryption: {report['security_features']['end_to_end_encryption']}"
            )
            print(
                f"  📊 Concurrent Processing: {report['performance']['concurrent_processing']}"
            )
            print(
                f"  🛡️  Privacy by Design: {report['compliance']['privacy_by_design']}"
            )
    ...
    def demo_performance_metrics(self) -> None:
        ...
        try:
            if self.he_service:
                ...
                print(
                    f"    - Security Level: {report['configuration']['security_level']}"
                )
                print(
                    f"    - Poly Modulus Degree: {report['configuration']['poly_modulus_degree']}"
                )

                print(f"  ⚡ Performance:")
                print(
                    f"    - Encryption Time: {report['performance_metrics']['encryption_time_estimate_ms']}"
                )
                print(
                    f"    - Processing Time: {report['performance_metrics']['processing_time_estimate_ms']}"
                )
                print(
                    f"    - Memory Efficient: {report['performance_metrics']['memory_efficient']}"
                )
