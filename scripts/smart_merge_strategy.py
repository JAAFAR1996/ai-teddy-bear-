...
class SmartMergeStrategy:
    ...
    def __init__(self, base_path: str='.'):
        ...
        self.merge_plan = {'ai_services': {'target_file':
            'src/application/services/ai/unified_ai_service.py',
            'files_to_merge': [
            ...
            'special_files': {
            'deprecated/services/ai_services/edge_ai_integration_service.py':
            'src/adapters/edge/',
            'deprecated/services/ai_services/child_domain_service.py':
            'src/domain/services/',
            'deprecated/services/ai_services/email_service.py':
            'src/application/services/communication/',
            'deprecated/services/ai_services/test_ai_service_integration.py':
            'tests/integration/'}}, 'audio_services': {'target_file':
            'src/application/services/core/unified_audio_service.py',
            'files_to_merge': [
            ...
            'special_files': {
            'deprecated/services/audio_services/azure_speech_to_text_service.py'
            : 'src/infrastructure/services/external/',
            'deprecated/services/audio_services/speech_to_text_service.py':
            'src/infrastructure/services/external/',
            'deprecated/services/audio_services/audio_service.py':
            'src/presentation/services/',
            'deprecated/services/audio_services/test_voice_service.py':
            'tests/unit/'}}, 'cache_services': {'target_file':
            'src/infrastructure/services/data/unified_cache_service.py',
            'files_to_merge': [
            'deprecated/services/cache_services/cache_service.py',
            'deprecated/services/cache_services/simple_cache_service.py']},
            'monitoring_services': {'target_file':
            'src/infrastructure/services/monitoring/unified_monitoring_service.py'
            , 'files_to_merge': [
    ...
    def analyze_file_content(self, file_path: Path) ->Dict:
        ...
        try:
            ...
            return {'file_path': str(file_path), 'content': content,
                'classes': classes, 'functions': functions, 'imports':
                imports, 'size': len(content)}
        except Exception as e:
            logger.info(f'خطأ في تحليل {file_path}: {e}')
            return {}
    ...
    def create_unified_service(self, service_group: str) ->str:
        ...
        logger.info(f'🔄 إنشاء خدمة موحدة: {target_file}')
        analyzed_files = []
        ...
        if not analyzed_files:
            return ''
        unified_content = self._generate_unified_content(service_group,
            analyzed_files)
        target_path = self.base_path / target_file
        target_path.parent.mkdir(parents=True, exist_ok=True)
        with open(target_path, 'w', encoding='utf-8') as f:
            f.write(unified_content)
        logger.info(f'  ✅ تم إنشاء: {target_file}')
        return unified_content

    def _generate_unified_content(self, service_group: str, analyzed_files:
        List[Dict]) ->str:
        ...
        unified_class_name = (
            f"Unified{service_name.replace('Services', 'Service')}")
        ...
        for source_file, target_dir in special_files.items():
            try:
                ...
                if source_path.exists():
                    ...
                    logger.info(f'  📁 نقل: {source_path.name} → {target_dir}')
            except Exception as e:
...
"""
    ...
    def execute_smart_merge(self) ->Dict:
        ...
        logger.info('=' * 60)
        logger.info('🔄  SMART MERGE STRATEGY')
        logger.info('🎯  MERGING UNIQUE FILES INTELLIGENTLY')
        logger.info('=' * 60)
        results = {}
        for service_group in self.merge_plan.keys():
            ...
            results[service_group] = {'unified_created': bool(
                unified_content), 'unified_size': len(unified_content),
                'moved_files': special_results['moved_files'], 'errors':
                special_results['errors']}
        report_content = self.generate_merge_report(results)
        report_path = (self.base_path / 'deleted' / 'reports' /
            'SMART_MERGE_REPORT.md')
