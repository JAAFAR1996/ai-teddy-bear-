...
class DocumentationGenerator:
    ...
    def generate_sphinx_docs(self, output_dir: str = 'docs/build'):
        ...
        try:
            ...
            if result.returncode == 0:
                self._logger.info(f"Sphinx documentation generated successfully in {output_dir}")
                return True
            else:
                self._logger.error(f"Sphinx documentation generation failed: {result.stderr}")
                return False
    ...
    def generate_markdown_docs(self, output_dir: str = 'docs/markdown'):
        ...
        try:
            ...
            if result.returncode == 0:
                self._logger.info(f"Markdown documentation generated successfully in {output_dir}")
                return True
            else:
                self._logger.error(f"Markdown documentation generation failed: {result.stderr}")
                return False
        except Exception as e:
    ...
    def generate_api_reference(self, output_dir: str = 'docs/api'):
        ...
        try:
            ...
            result = subprocess.run([
                ...
                '-m', 'src.application', 
                '-m', 'src.domain', 
                '-m', 'src.infrastructure',
                '--render-toc',
                '--output', output_dir
            ], capture_output=True, text=True)
            
            if result.returncode == 0:
                self._logger.info(f"API reference generated successfully in {output_dir}")
                return True
            else:
    ...
    def generate_coverage_report(self, output_dir: str = 'docs/coverage'):
        ...
        try:
            ...
            if result.returncode == 0:
                self._logger.info(f"Coverage report generated successfully in {output_dir}")
                return True
            else:
                self._logger.error(f"Coverage report generation failed: {result.stderr}")
                return False
        except Exception as e:
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='AI Teddy Bear Documentation Generator')
    parser.add_argument('--sphinx', action='store_true', help='Generate Sphinx documentation')
