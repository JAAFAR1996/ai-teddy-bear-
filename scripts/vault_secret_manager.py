...
class VaultSecretManager:
    ...
    def store_api_keys(self, api_keys: Dict[str, str], path: str = "api-keys") -> bool:
        ...
        try:
            ...
            self.logger.info(f"✅ تم تخزين {len(api_keys)} مفتاح API في المسار: {full_path}")
            return True
    ...
    def get_api_key(self, service_name: str, path: str = "api-keys") -> Optional[str]:
    ...
    def rotate_api_key(self, service_name: str, new_key: str, path: str = "api-keys") -> bool:
        """تدوير مفتاح API محدد"""
        try:
            # استرداد المفاتيح الحالية
            current_keys = self.get_all_api_keys(path)
            
            # تحديث المفتاح المحدد
            current_keys[service_name] = new_key
            
            # إعادة تخزين جميع المفاتيح
    ...
    def _encrypt_sensitive_keys(self, keys: Dict[str, str]) -> Dict[str, str]:
        ...
        # هنا يمكن إضافة تشفير إضافي إذا لزم الأمر
        return keys
    
    def _decrypt_sensitive_keys(self, encrypted_keys: Dict[str, str]) -> Dict[str, str]:
        """فك تشفير المفاتيح الحساسة (placeholder)"""
        # هنا يمكن إضافة فك تشفير إذا لزم الأمر
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            status = {
                "vault_authenticated": self.client.is_authenticated(),
...
class TeddyVaultIntegration:
    ...
    def initialize_from_env(self) -> bool:
        ...
        try:
            ...
            if not vault_token:
                # محاولة قراءة من ملف .vault_tokens
...
def main():
    ...
    print("🔐 Vault Secret Manager - اختبار الاتصال")
    print("=" * 50)
    ...
    if teddy_vault.initialize_from_env():
        print("✅ تم تهيئة Vault بنجاح")
        
        # فحص الصحة
        health = teddy_vault.vault_manager.health_check()
        print(f"🏥 فحص الصحة: {health}")
        
        # اختبار استرداد المفاتيح
        try:
            config = teddy_vault.get_service_config()
            print(f"🔑 تم استرداد {len(config)} مفتاح من Vault")
        except Exception as e:
            print(f"❌ خطأ في استرداد المفاتيح: {e}")
    else:
        print("❌ فشل في تهيئة Vault")
        return 1
