...
class AdvancedDuplicateFinder:
        ...
        for i, file1 in enumerate(files_list):
            for j, file2 in enumerate(files_list[i+1:], i+1):
                ...
                if similarity >= threshold:
                    recommendation = self._generate_similarity_recommendation(file1, file2, similarity)
    ...
    def generate_report(self) -> None:
        ...
        exact_duplicates = [g for g in self.duplicate_groups if g.type == "exact"]
        similar_files = [g for g in self.duplicate_groups if g.type == "similar"]
        functional_duplicates = [g for g in self.duplicate_groups if g.type == "functional"]
        service_duplicates = [g for g in self.duplicate_groups if g.type == "service"]
...
"""
    ...
    def _print_summary(self, report: Dict) -> None:
        ...
        print(f"🔄 الملفات المطابقة تماماً: {report['summary']['exact_duplicates']}")
        print(f"🔗 الملفات المشابهة: {report['summary']['similar_files']}")
        print(f"⚙️ التكرار الوظيفي: {report['summary']['functional_duplicates']}")
