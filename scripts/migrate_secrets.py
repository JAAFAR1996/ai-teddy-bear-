...
class MigrationConfig:
    ...
    exclude_patterns: List[str] = field(default_factory=lambda: [
        "*.log", "*.tmp", "node_modules/*", ".git/*", "venv/*", "__pycache__/*"
    ])
...
class AdvancedSecretsMigrator:
    ...
    def __init__(self, config: MigrationConfig):
        ...
        self.secret_patterns = {
            ...
            'secret_key': {
                'pattern': r'["\']?(?i)(?:secret[_-]?key|secretkey)["\']?\s*[:=]\s*["\']([A-Za-z0-9+/=_-]{20,})["\']',
                'confidence': 0.95,
                'type': 'Secret Key'
            },
            'jwt_secret': {
                'pattern': r'["\']?(?i)(?:jwt[_-]?secret|jwtsecret)["\']?\s*[:=]\s*["\']([A-Za-z0-9+/=_%-]{32,})["\']',
                'confidence': 0.98,
                'type': 'JWT Secret'
            },
            'encryption_key': {
                'pattern': r'["\']?(?i)(?:encryption[_-]?key|encryptionkey)["\']?\s*[:=]\s*["\']([A-Za-z0-9+/=_-]{24,})["\']',
                'confidence': 0.95,
                'type': 'Encryption Key'
            },
            'password': {
            ...
            'token': {
                'pattern': r'["\']?(?i)(?:access[_-]?token|accesstoken|bearer[_-]?token|bearertoken)["\']?\s*[:=]\s*["\']([A-Za-z0-9+/=_.-]{20,})["\']',
                'confidence': 0.85,
                'type': 'Access Token'
            },
            'openai_key': {
    ...
    def initialize_vault_client(self) -> bool:
        ...
        try:
            ...
            # إنشاء مسار الأسرار إذا لم يكن موجوداً
    ...
    def _setup_vault_mount(self):
        ...
        try:
            ...
            if mount_path not in mounts['data']:
                ...
                logger.info(f"ℹ️ مسار الأسرار موجود بالفعل: {self.config.vault_mount_path}")
    ...
    def scan_for_secrets(self, root_path: str = ".") -> int:
        ...
        logger.info(f"✅ تم مسح {scanned_files} ملف - اكتُشف {len(self.discovered_secrets)} سر")
    ...
    def _is_valid_secret(self, value: str, secret_type: str) -> bool:
        ...
        # استبعاد القيم الوهمية والأمثلة
        dummy_patterns = [
            'your_', 'example_', 'test_', 'demo_', 'sample_',
            'placeholder', 'changeme', 'secret_here', '123456',
    ...
    def _extract_key_name(self, line: str, pattern_name: str) -> str:
        ...
        # محاولة استخراج اسم المفتاح من السياق
        key_match = re.search(r'["\']?(\w+(?:[_-]\w+)*)["\']?\s*[:=]', line)
        if key_match:
    ...
    def _generate_vault_path(self, secret: SecretItem) -> str:
        """إنشاء مسار هرمي في Vault"""
        # استخراج اسم المشروع من مسار الملف
    ...
    def _update_file_with_vault_reference(self, secret: SecretItem, migration_results: Dict):
    ...
    def generate_migration_report(self, migration_results: Dict, backup_dir: str) -> str:
        ...
        # حفظ التقرير
        report_file = Path(f'secrets_migration_report_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json')
...
def main():
    ...
    parser.add_argument('--vault-url', default='http://localhost:8200', help='عنوان Vault')
    ...
    parser.add_argument('--dry-run', action='store_true', help='تشغيل تجريبي بدون تعديلات')
