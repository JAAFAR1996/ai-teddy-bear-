            ...
            for filename in files:
                ...
                if file_hash:
                    ...
                    if filepath.suffix == '.py':
                        file_info['features'] = self.analyze_python_file(filepath)
    ...
    def print_summary(self):
        ...
        if self.duplicates:
            ...
            for i, duplicate_group in enumerate(report['duplicates'], 1):
                ...
                print(f"  الحجم الإجمالي: {duplicate_group['total_size']:,} bytes")
                print(f"  المساحة المهدورة: {duplicate_group['wasted_space']:,} bytes")
                print("  الملفات:")
                ...
                if 'features_comparison' in duplicate_group:
                    ...
                    if comp['identical_features']:
                        ...
                        for diff in comp['differences']:
                            print(f"    - {diff['file']}: اختلاف في {diff['feature']}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='البحث عن الملفات المكررة في مشروع AI Teddy Bear')
    parser.add_argument('path', nargs='?', default='.', help='مسار المشروع (افتراضي: المجلد الحالي)')
    parser.add_argument('-o', '--output', default='duplicate_files_report.json', help='ملف التقرير')
    parser.add_argument('-t', '--threshold', type=int, default=80, help='نسبة التشابه للملفات المتشابهة (افتراضي: 80%)')
