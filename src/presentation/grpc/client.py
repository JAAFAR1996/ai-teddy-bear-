...
class GRPCClient:
    ...
    async def process_audio(self, audio_data: bytes, format: str = "wav", sample_rate: int = 16000):
        ...
        # Mock gRPC call
        request = {"audio_data": audio_data, "format": format, "sample_rate": sample_rate}

        # Simulate gRPC response
        return {"transcription": "مرحبا كيف حالك؟", "emotions": ["happy"], "confidence": 0.85}
    ...
    async def generate_ai_response(self, message: str, child_id: str, context: dict = None):
        ...
        request = {"message": message, "child_id": child_id, "context": context or {}, "response_type": "general"}

        # Simulate gRPC response
        return {"response_text": f"مرحبا! تحدثت عن {message}", "emotion": "friendly", "confidence": 0.9}
    ...
    async def stream_ai_response(self, message: str, child_id: str):
        ...
        for i, chunk in enumerate(chunks):
            yield {"text": chunk, "is_final": i == len(chunks) - 1}
