...
try:
    ...
    logger.warning(
        "⚠️ librosa not available. Install with: pip install librosa")
...
try:
    ...
    logger.warning(
        "⚠️ noisereduce not available. Install with: pip install noisereduce")
...
class AudioProcessingEngine:
    ...
    audio_data: np.ndarray,
     processing_level: str = "auto") -> tuple:
        ...
        try:
            ...
            if processing_level in [
    "medium", "high"] and NOISEREDUCE_AVAILABLE:
                processed_audio = self._reduce_noise(
                    processed_audio, processing_level)
                processing_info["steps_applied"].append("noise_reduction")

            if processing_level in ["medium", "high"] and SCIPY_AVAILABLE:
                processed_audio = self._enhance_voice(
                    processed_audio, processing_level)
    ...
    def _reduce_noise(self, audio: np.ndarray, level: str) -> np.ndarray:
        ...
        try:
            ...
            logger.warning(
    "Noise reduction failed, using original",
     error=str(e))
            return audio
    ...
    def _enhance_voice(self, audio: np.ndarray, level: str) -> np.ndarray:
        ...
        try:
            ...
            b_speech, a_speech = butter(
                2, [speech_low, speech_high], btype='band')
            ...
            logger.warning(
    "Voice enhancement failed, using original",
     error=str(e))
            return audio
    ...
    def _advanced_processing(self, audio: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            logger.warning(
    "Advanced processing failed, using original",
     error=str(e))
            return audio
    ...
    original: np.ndarray,
     processed: np.ndarray) -> dict:
        ...
        try:
            ...
            original_dynamic_range = np.max(
                np.abs(original)) - np.min(np.abs(original))
            processed_dynamic_range = np.max(
                np.abs(processed)) - np.min(np.abs(processed))
