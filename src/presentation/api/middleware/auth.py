...
class AuthConfig(BaseModel):
    ...
    jwt_secret_name: str = Field(
        default="jwt_secret", description="JWT secret name in vault"
    )
    api_key_name: str = Field(
        default="teddy_api_key", description="API key name in vault"
    )
    token_expiry_hours: int = Field(
        default=24, description="Token expiry in hours")
    max_failed_attempts: int = Field(
        default=5, description="Max failed auth attempts")
...
class AuthManager:
    ...
    async def get_jwt_secret(self) -> str:
        ...
        if not secret:
            raise SecurityException(
                f"JWT secret '{self.config.jwt_secret_name}' not found"
            )
    ...
    async def get_api_key(self) -> str:
        ...
        if not secret:
            raise SecurityException(
                f"API key '{self.config.api_key_name}' not found")
    ...
    def record_failed_attempt(self, identifier: str):
        ...
        self._failed_attempts[identifier] = self._failed_attempts.get(
            identifier, 0) + 1
        ...
        if self._failed_attempts[identifier] >= self.config.max_failed_attempts:
            ...
            self._lockout_times[identifier] = (
                asyncio.get_event_loop().time() + lockout_duration
            )
            logger.warning(
                f"🔒 Account {identifier} locked out for {self.config.lockout_duration_minutes} minutes"
            )
...
def require_parent_auth(f) -> Any:
    ...
    async def decorated_function(*args, **kwargs) -> Any:
        try:
            ...
            if not auth_manager.check_rate_limit(client_ip):
                logger.warning(
                    f"🚨 Rate limit exceeded for parent auth IP: {client_ip}"
                )
                return jsonify({"error": "Rate limit exceeded"}), 429
            ...
            try:
                ...
                if not parent_id:
                    auth_manager.record_failed_attempt(client_ip)
                    logger.warning(
                        f"🚨 Missing parent_id in token from IP: {client_ip}"
                    )
                ...
                auth_manager.record_successful_attempt(client_ip)
                logger.info(
                    f"✅ Valid parent authentication for ID: {parent_id} from IP: {client_ip}"
                )
...
def require_child_auth(f) -> Any:
    ...
    async def decorated_function(*args, **kwargs) -> Any:
        try:
            ...
            if not auth_header or not auth_header.startswith("Bearer "):
                logger.warning(
                    "🚨 Missing or invalid Authorization header for child auth"
                )
                return jsonify({"error": "Authentication required"}), 401
            ...
            if not auth_manager.check_rate_limit(client_ip):
                logger.warning(
                    f"🚨 Rate limit exceeded for child auth IP: {client_ip}")
                return jsonify({"error": "Rate limit exceeded"}), 429
            ...
            try:
                ...
                if not child_id:
                    auth_manager.record_failed_attempt(client_ip)
                    logger.warning(
                        f"🚨 Missing child_id in token from IP: {client_ip}")
                    return jsonify({"error": "Invalid token"}), 401

                request.child_id = child_id
                auth_manager.record_successful_attempt(client_ip)
                logger.info(
                    f"✅ Valid child authentication for ID: {child_id} from IP: {client_ip}"
                )
                ...
                auth_manager.record_failed_attempt(client_ip)
                logger.warning(
                    f"🚨 Invalid child token from IP: {client_ip}: {e}")
                return jsonify({"error": "Invalid token"}), 401
