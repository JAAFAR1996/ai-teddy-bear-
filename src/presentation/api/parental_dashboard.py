...
class SimpleDashboardService:
    ...
    def get_interaction_summary(self, child_id: str) -> Dict:
        ...
        summary = {
            ...
            "speech_analysis": {"concerns": self.analytics.get_speech_concerns(child_id)},
    ...
    def generate_recommendations(self, child_id: str) -> List[str]:
        ...
        if emotion_stability < 0.5:
            recommendations.append("لاحظنا تقلبات في مزاج الطفل. يُنصح بقضاء وقت إضافي معه والتحدث عن مشاعره.")

        # توصيات النطق
        if speech_concerns:
            recommendations.append(
                f"تم رصد بعض التحديات في النطق: {', '.join(speech_concerns)}. " "قد يكون من المفيد استشارة أخصائي نطق."
            )
        else:
    ...
    def generate_emotion_chart(self, child_id: str, days: int = 7) -> str:
        ...
        fig = px.bar(
            ...
            color_discrete_map={"happy": "#4CAF50", "calm": "#2196F3", "excited": "#FF9800"},
        )
    ...
    def generate_progress_chart(self, child_id: str) -> str:
        ...
        fig = go.Figure(data=[go.Bar(x=games, y=scores, marker_color=["#4CAF50", "#2196F3", "#FF9800"])])

        fig.update_layout(title="تقدم الطفل في الألعاب", xaxis_title="نوع اللعبة", yaxis_title="الدرجة", height=400)
