...
from ..aggregates.child_aggregate import SafetyViolation, ConversationLimitExceeded
...
class ChildDomainService:
    ...
    def conduct_comprehensive_safety_assessment(
        ...
        requires_parent_review = (
            overall_safety_score < 0.7 or
            child.safety_violations_count >= 2 or
            child.escalation_count > 0
        )
    ...
    def recommend_voice_profile_adjustments(
        ...
        new_emotion_sensitivity = max(0.1, min(1.0, 
            current_profile.emotion_sensitivity + emotion_sensitivity_adjustment))
        
        # Only recommend changes if they're significant
        if (abs(new_pitch - current_profile.pitch) > 0.05 or
            abs(new_speed - current_profile.speed) > 0.03 or
            abs(new_emotion_sensitivity - current_profile.emotion_sensitivity) > 0.05):
    ...
    def _assess_conversation_patterns_safety(self, conversations: List[Conversation]) -> float:
        ...
        low_engagement_count = sum(1 for conv in conversations 
                                 if conv.child_engagement_score < 0.3)
