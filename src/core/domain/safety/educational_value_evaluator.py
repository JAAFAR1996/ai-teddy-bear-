...
class EducationalValueEvaluator:
    ...
    def _calculate_educational_score(
        ...
        educational_keywords = [
            "learn", "study", "teach", "show", "explain", "discover",
            "explore", "practice", "try", "experiment", "question"
        ]
        
        educational_content = sum(1 for keyword in educational_keywords 
        ...
        interactive_content = sum(1 for keyword in interactive_keywords 
                                if keyword in content_lower)
    ...
    def _generate_learning_recommendations(
        ...
        for category, skills in expected_skills.items():
            covered_skills = [skill for skill in skill_mastery.keys() 
                            if skill.endswith(category)]
            
            if len(covered_skills) < len(skills) * 0.5:
                recommendations.append(f"Focus more on {category} development")
        
        # Suggest skill advancement
        mastered_skills = [skill for skill, level in skill_mastery.items() 
                         if level == "mastered"]
