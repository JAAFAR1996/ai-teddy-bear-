...
class InteractiveStoryEngine:
    ...
    async def start_interactive_story(self, child_name: str, friends: List[str], age: int, 
                                    story_type: StoryType = None, device_id: str = None) -> Dict[str, Any]:
        """بدء قصة تفاعلية"""
        
        # اختيار القصة المناسبة
        if story_type:
            suitable_stories = [s for s in self.stories.values() 
    ...
    async def process_story_choice(self, story_state: Dict, choice_id: str, 
    ...
    def analyze_behavioral_patterns(self, child_name: str, device_id: str, 
        ...
        relevant_logs = [
            log for log in self.choice_logs
    ...
    def _generate_parent_recommendations(self, patterns: List[BehavioralPattern], 
                                       behavioral_averages: Dict) -> List[str]:
