...
class AdvancedBehavioralAnalyzer:
    ...
    async def analyze_voice_with_context(self, audio_data: bytes, 
                                       story_context: Optional[Dict] = None,
                                       child_name: str = None,
                                       device_id: str = None) -> VoiceAnalysis:
    ...
    def _combine_voice_and_text_analysis(self, voice_features: Dict, text_analysis: Dict) -> Dict[str, Any]:
        ...
        voice_confidence = 1.0 - (voice_features["pause_frequency"] * 0.5 + 
    ...
    def _apply_story_context(self, analysis: Dict, story_context: Dict) -> Dict[str, Any]:
        ...
        if (story_context.get("requires_courage") and 
            VoicePattern.ANXIOUS in analysis["detected_patterns"]):
            analysis["context_notes"] = "أظهر قلقاً في موقف يتطلب الشجاعة"
        
        # إذا كان السياق اجتماعي والطفل أظهر تردد
        if (story_context.get("social_situation") and 
            VoicePattern.HESITANT in analysis["detected_patterns"]):
            analysis["context_notes"] = "تردد في الموقف الاجتماعي"
        
        return analysis
    
    async def _update_psychological_profile(self, child_name: str, device_id: str, 
                                          voice_analysis: VoiceAnalysis):
        """تحديث الملف النفسي للطفل"""
    ...
    async def check_for_behavioral_concerns(self, child_name: str, device_id: str):
        ...
        recent_analyses = [
            analysis for analysis in self.voice_analyses[-30:]  # آخر 30 تحليل
    ...
    async def _check_repeated_anxiety(self, child_name: str, device_id: str, 
                                    analyses: List[VoiceAnalysis]):
        """فحص القلق المتكرر"""
        
        anxiety_count = sum(1 for analysis in analyses[-7:] 
    ...
    async def _check_low_confidence(self, child_name: str, device_id: str, 
                                  analyses: List[VoiceAnalysis]):
        """فحص انخفاض الثقة"""
    ...
    def get_child_psychological_report(self, child_name: str, device_id: str) -> Dict[str, Any]:
        ...
        if len(profile.confidence_trend) >= 5:
            ...
            earlier_avg = sum(profile.confidence_trend[-10:-5]) / 5 if len(profile.confidence_trend) >= 10 else recent_avg
        ...
        recent_alerts = [
            alert for alert in self.behavioral_alerts
            if alert.child_name == child_name and alert.device_id == device_id
        ...
        return {
            "child_name": child_name,
            "device_id": device_id,
            "confidence_level": sum(profile.confidence_trend) / len(profile.confidence_trend) if profile.confidence_trend else 0.5,
    ...
    def mark_alert_as_read(self, alert_id -> Any: str) -> Any:
