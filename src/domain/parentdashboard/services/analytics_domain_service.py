...
from ..models.analytics_models import (
    AnalyticsData, ConversationLog, LearningProgress, 
    UsageMetrics, AnalyticsFilter
)
...
class AnalyticsDomainService:
    ...
    def calculate_analytics(
        ...
        if filter_criteria:
            logs = [log for log in logs if filter_criteria.applies_to_log(
    ...
    def _calculate_learning_progress(
        ...
        educational_conversations = sum(
            1 for log in logs 
            if set(log.topics or []) & educational_topics
        )
    ...
    def _calculate_duration_score(
        ...
        appropriate_sessions = sum(
            1 for log in logs 
            if 5 <= (log.duration_seconds / 60) <= 30
        )
