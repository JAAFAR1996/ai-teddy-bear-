...
from sqlalchemy import Column, String, Integer, Time, Boolean, ForeignKey, JSON
...
class AccessScheduleType(Enum):
    ...
    def get_default_schedule(self) -> List[Dict[str, Any]]:
        ...
        if self == AccessScheduleType.ALWAYS:
            ...
            return [
                {'day': day, 'start_hour': 15, 'start_minute': 0, 
            ...
            return [
                {'day': day, 'start_hour': 9, 'start_minute': 0,
...
class ParentalControl:
    ...
    def should_alert_for_topic(self, topic: str) -> bool:
        ...
        return (
            self.alert_settings.get('content_moderation', True) and
            not self.is_topic_allowed(topic)
        )
...
class AccessSchedule(Base):
    ...
    def is_access_allowed_now(self) -> bool:
        ...
        return (
            current_day == self.day_of_week and
