...
class SearchFilters:
    ...
    def get_active_filter_count(self) -> int:
        ...
        count = 0
        if self.name_query: count += 1
        if self.age_range: count += 1
        if self.languages: count += 1
        if self.interests: count += 1
        if self.has_special_needs is not None: count += 1
        if self.communication_style: count += 1
        if self.learning_level: count += 1
        if self.family_code: count += 1
        if self.parent_id: count += 1
        if self.interaction_time_filter: count += 1
        if self.cultural_background: count += 1
        return count
