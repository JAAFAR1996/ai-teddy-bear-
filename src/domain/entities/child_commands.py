...
from ...domain.value_objects import ChildId, ParentId, DeviceId
...
class RegisterChildCommandHandler(CommandHandler):
    ...
    async def handle(self, command: RegisterChildCommand) -> CommandResult:
        ...
        try:
            ...
            return CommandResult(
                success=False,
                message=f"Child registration failed: {str(e)}"
            )
...
def register_child_command_handlers():
    ...
    command_bus.register_handler(
        ...
        UpdateChildProfileCommandHandler()
    )
    
    command_bus.register_handler(
        ReportSafetyViolationCommand,
        ReportSafetyViolationCommandHandler()
    )
