...
class Message(BaseModel):
    ...
    def extract_questions(self) -> List[str]:
        ...
        questions = [s.strip() for s in sentences if '?' in s or s.strip().startswith(('What', 'Who', 'Where', 'When', 'Why', 'How', 'Is', 'Are', 'Can', 'Will'))]
        return questions
...
class Conversation(BaseModel):
    ...
    def calculate_educational_score(self) -> float:
        ...
        edu_content_count = sum(
            ...
            if m.metadata.educational_content is not None
        )
        if self.messages:
    ...
    def calculate_quality_score(self) -> float:
        ...
        positive_emotions = sum(
            1 for state in self.emotional_states 
