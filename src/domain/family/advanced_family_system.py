...
class AdvancedFamilySystem:
    ...
    def create_family_profile(self, family_name: str, parent_name: str, 
                            children_info: List[Dict], settings: Dict = None) -> str:
    ...
    def add_scheduled_message(self, family_id: str, child_name: str, device_id: str,
                            message_type: MessageType, content: str, 
                            scheduled_time: time, days_of_week: List[int],
                            parent_name: str) -> str:
    ...
    def add_time_restriction(self, family_id: str, child_name: str, device_id: str,
                           restriction_type: TimeRestrictionType, 
                           parent_name: str, reason: str,
                           start_time: time = None, end_time: time = None,
                           daily_limit_minutes: int = None,
                           days_of_week: List[int] = None) -> str:
        """إضافة قيد زمني جديد"""
    ...
    async def check_scheduled_messages(self) -> List[Dict]:
        ...
        for message in self.scheduled_messages:
            ...
            if (message.last_sent and 
                message.last_sent.date() == today):
                continue
    ...
    def check_time_restrictions(self, device_id: str, child_name: str) -> Dict[str, Any]:
        ...
        child_restrictions = [
            r for r in self.time_restrictions
            ...
            if restriction.restriction_type == TimeRestrictionType.BEDTIME_RESTRICTION:
                ...
                if restriction.start_time and restriction.end_time:
                    if restriction.start_time > restriction.end_time:
                        ...
                        if (current_time >= restriction.start_time or 
                ...
                # نافذة زمنية محددة
                if (restriction.start_time and restriction.end_time and
                    not (restriction.start_time <= current_time <= restriction.end_time)):
                    violation = True
    ...
    def generate_family_comparison_report(self, family_id: str, 
                                        children_data: Dict[str, Dict]) -> ChildComparison:
    ...
    def update_family_settings(self, family_id: str, new_settings: Dict) -> bool:
        ...
        if "daily_interaction_limit" in new_settings:
            for restriction in self.time_restrictions:
                if (restriction.restriction_type == TimeRestrictionType.DAILY_LIMIT and
                    any(member.name == restriction.child_name 
    ...
    def create_custom_encouragement_message(self, child_name: str, achievement: str,
                                          personal_traits: List[str]) -> str:
