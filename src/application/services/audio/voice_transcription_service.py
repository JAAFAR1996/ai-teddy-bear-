...
from src.domain.audio.models import (
    TranscriptionRequest, ProviderConfig, 
...
class TranscriptionExecutor:
    ...
    async def execute(
        ...
        try:
            ...
            logger.error(f"Transcription failed with {provider.name}: {str(e)}")
            return None
    
    async def _transcribe_whisper(self, request: TranscriptionRequest) -> Optional[str]:
        ...
        try:
            # Process audio file
            audio_files = await VoiceAudioProcessor.process_audio_file(request.audio_path)
            
            # Run transcription in executor to avoid blocking
            loop = asyncio.get_event_loop()
            result = await loop.run_in_executor(
                None, 
                self.resources.whisper_model.transcribe,
                audio_files["wav"]
            )
            ...
            if result and 'text' in result:
                return VoiceAudioProcessor.clean_transcription_text(result['text'])
    ...
    async def _transcribe_azure(self, request: TranscriptionRequest) -> Optional[str]:
        ...
        try:
            ...
            audio_config = speechsdk.audio.AudioConfig(filename=audio_files["wav"])
    ...
    async def _transcribe_fallback(self, request: TranscriptionRequest) -> Optional[str]:
...
class TranscriptionService(BaseProviderService[TranscriptionRequest]):
    ...
    async def process(self, audio_data: str, language: str = "Arabic") -> Optional[str]:
