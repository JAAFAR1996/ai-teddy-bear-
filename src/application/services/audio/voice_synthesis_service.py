...
class SynthesisExecutor:
    ...
    async def execute(
        ...
        try:
            if provider.provider_type == ProviderType.ELEVENLABS:
                ...
                logger.warning(
                    f"Unknown synthesis provider: {provider.provider_type}")
                return None
    ...
    async def _synthesize_elevenlabs(
            self, request: SynthesisRequest) -> Optional[str]:
        ...
        try:
            ...
            voice_settings = self._get_elevenlabs_voice_settings(
                request.emotion)
...
class SynthesisService(BaseProviderService[SynthesisRequest]):
    ...
    async def synthesize(
        ...
        # Generate cache key
        cache_key = self.cache_manager.generate_synthesis_key(
            text, emotion, language)

        # Create request
        request = SynthesisRequest(
            text=text, emotion=emotion, language=language, cache_key=cache_key
        )
