...
class ConversationService(BaseService[Conversation]):
    ...
    async def archive_conversation(
        ...
        result = await self._repository.archive_conversation(
            conversation_id,
            reason
        )
        await self._log_operation(
    ...
    async def get_flagged_content(
        ...
        return await self._repository.get_flagged_content(
            start_date,
            end_date
        )
    ...
    async def clean_old_conversations(self) -> int:
        ...
        for conv in old_conversations:
            await self.archive_conversation(
                conv.id,
                f"Automated cleanup - Age > {self.content_retention_days} days"
            )
            count += 1
    ...
    async def get_similar_conversations(
        ...
        return await self._repository.get_similar_conversations(
            conversation_id,
            limit
        )
