...
from src.domain.exceptions.base import (
    ErrorContext, InappropriateContentException, 
    ParentalConsentRequiredException, ValidationException,
    AuthenticationException, ExternalServiceException,
    AgeInappropriateException, QuotaExceededException
)
from src.infrastructure.decorators.exception_handler import (
    handle_exceptions, with_retry, with_circuit_breaker,
    child_safe, validate_input, authenticated, RetryConfig
)
from src.infrastructure.monitoring.metrics import (
    track_request_duration, MetricsCollector
)
...
class ChildInteractionServiceV2:
    ...
    @handle_exceptions(
        (InappropriateContentException, lambda e: {
            "error": "Content filtered",
            "safe": True,
            "message": "تم تصفية المحتوى لحماية الطفل"
        }),
        (ParentalConsentRequiredException, lambda e: {
            "error": "Parent approval needed",
            "action": "notify_parent",
            "details": {"action": e.action, "reason": e.reason}
        }),
        (ValidationException, lambda e: {
            "error": f"Invalid input: {str(e)}",
            "retry": True,
            "field": e.field_name
        }),
        (QuotaExceededException, lambda e: {
    ...
    @handle_exceptions(
        (AuthenticationException, lambda e: {
