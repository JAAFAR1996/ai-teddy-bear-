...
class ARVRPreferencesManager:
    ...
    def get_recommended_ar_categories(self, child_id: str, top_n: int = 3) -> list:
        ...
        # ترتيب حسب النقاط
        sorted_categories = sorted(ar_prefs.items(), key=lambda x: x[1], reverse=True)
    ...
    def get_recommended_vr_themes(self, child_id: str, top_n: int = 3) -> list:
        ...
        # ترتيب حسب النقاط
        sorted_themes = sorted(vr_prefs.items(), key=lambda x: x[1], reverse=True)
    ...
    def get_preferences_summary(self, child_id: str) -> Dict:
        ...
        return {
            "top_ar_category": self.get_recommended_ar_categories(child_id, 1)[0] if prefs["preferred_ar_categories"] else "educational",
            "top_vr_theme": self.get_recommended_vr_themes(child_id, 1)[0] if prefs["preferred_vr_themes"] else "educational",
            "optimal_duration": prefs["optimal_session_duration"],
            "requires_comfort_adjustments": bool(prefs["comfort_settings"]),
