...
from .moderation_helpers import ModerationRequest, ModerationContext, ConditionalDecomposer
...
class ModerationServiceRefactored:
    ...
    async def check_content(
        ...
        if context.use_cache:
            cached = self.cache_manager.get(mod_request.content, mod_request.age, mod_request.language)
            if cached:
                return cached
        ...
        if not local_result.is_safe:
            ...
            self.cache_manager.set(mod_request.content, mod_request.age, mod_request.language, response)
            return response
        
        # Safe response
        safe_response = self.result_processor.create_safe_response("Passed all checks")
        self.cache_manager.set(mod_request.content, mod_request.age, mod_request.language, safe_response)
