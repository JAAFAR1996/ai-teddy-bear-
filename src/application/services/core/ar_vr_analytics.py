...
class ARVRAnalytics:
    ...
    def _analyze_progress_trends(self, sessions: List[Dict]) -> Dict:
        ...
        for session in sessions:
            session_progress = sum(session.get("learning_progress", {}).values())
    ...
    def compare_children_performance(self, children_data: Dict[str, List[Dict]]) -> Dict:
        ...
        for child_id, sessions in children_data.items():
            ...
            aggregated_stats[f"child_{len(aggregated_stats) + 1}"] = {
                "session_count": len(sessions),
                "avg_engagement": child_stats.get("engagement_patterns", {}).get("average_engagement", 0),
                "total_progress": child_stats.get("progress_trends", {}).get("total_progress", 0),
            }
    ...
    def _generate_comparison_insights(self, stats: Dict) -> List[str]:
        ...
        best_engagement = max(stats.values(), key=lambda x: x["avg_engagement"])
        worst_engagement = min(stats.values(), key=lambda x: x["avg_engagement"])
        ...
        # متوسط عدد الجلسات
        avg_sessions = sum(child["session_count"] for child in stats.values()) / len(stats)
