...
class ModerationLocalCheckers:
    ...
    def _load_word_lists(self) -> None:
        ...
        self.logger.info(
            f"Loaded {len(self.whitelist)} safe words, {len(self.blacklist)} banned words"
        )
    ...
    def _init_nlp_models(self) -> None:
        ...
        if not SPACY_AVAILABLE or not TRANSFORMERS_AVAILABLE:
            self.logger.warning(
                "NLP libraries not available, using basic checks only")
            self.nlp = None
            self.sentiment_analyzer = None
            self.toxicity_classifier = None
            return
    ...
    async def check_whitelist_blacklist(
    ...
    async def check_with_rule_engine(
        self, request: ModerationRequest
    ) -> ModerationResult:
        ...
        try:
            ...
            return ModerationResult(
                is_safe=max_severity
                in [ModerationSeverity.SAFE, ModerationSeverity.LOW],
    ...
    async def check_with_nlp_models(
        self, request: ModerationRequest
    ) -> ModerationResult:
    ...
    async def _perform_nlp_analysis(
        ...
        # 1. تحليل المشاعر
        sentiment_category, sentiment_score = self._analyze_sentiment(
            request.content)
        ...
        # 3. فحص السمية
        toxicity_category, toxicity_score = self._check_toxicity(
            request.content)
    ...
    async def check_context_appropriate(
        self, request: ModerationRequest, context: List[Message]
    ) -> ModerationResult:
        """🗨️ فحص مناسبة السياق"""
        if not context:
    ...
    async def check_age_specific_content(
        self, request: ModerationRequest
    ) -> ModerationResult:
        ...
        if not ModerationLookupTables.is_age_appropriate(
            ContentCategory.VIOLENCE, request.age
        ):
            if any(word in content_lower for word in violence_words):
    ...
    def _analyze_sentiment(
        self, content: str
    ) -> Tuple[Optional[ContentCategory], float]:
        """😊 تحليل المشاعر"""
        ...
        try:
            ...
            if sentiment[
                "label"
            ] == "NEGATIVE" and ConditionalDecomposer.is_score_above_threshold(
                sentiment["score"], 0.8
            ):
    ...
    def _check_toxicity(
            self, content: str) -> Tuple[Optional[ContentCategory], float]:
        """☣️ فحص السمية"""
        ...
        try:
            ...
            if toxicity[
                "label"
            ] == "TOXIC" and ConditionalDecomposer.is_score_above_threshold(
                toxicity["score"], 0.7
            ):
    ...
    def _extract_topics_from_context(
            self, messages: List[Message]) -> List[str]:
