...
from .models import (
    ...
    ProcessingResult,
    AudioBuffer as AudioBufferConfig,
)
...
class AudioProcessor:
    ...
    def __init__(
        self,
        input_buffer_config: AudioBufferConfig,
        output_buffer_config: AudioBufferConfig,
    ):
    ...
    async def process_audio_input(
        ...
        try:
            ...
            self._update_processing_stats(
                processing_time, processing_result.success)
    ...
    async def _check_buffer_ready_for_processing(self) -> ProcessingResult:
        ...
        if buffer_size < chunk_size:
            return ProcessingResult.error_result(
                f"Buffer not ready: {buffer_size} < {chunk_size}", metadata={
                    "buffer_size": buffer_size, "required_size": chunk_size}, )

        return ProcessingResult.success_result(
            {"buffer_ready": True, "buffer_size": buffer_size, "chunk_size": chunk_size}
        )
    ...
    async def _process_audio_chunk(
        ...
        try:
            ...
            if not audio_chunk:
                return ProcessingResult.error_result(
                    "No audio data available in buffer"
                )
    ...
    async def _apply_audio_processing(
        self, audio_data: bytes, request: AudioProcessingRequest
    ) -> Optional[bytes]:
        """
    ...
    def _update_processing_stats(self, processing_time: float, success: bool):
        ...
        self.logger.debug(
            f"Audio processing completed in {processing_time:.3f}s")
    ...
    async def get_output_buffer_data(
            self, size: Optional[int] = None) -> bytes:
    ...
    async def clear_buffers(self) -> ProcessingResult:
        ...
        try:
            ...
            return ProcessingResult.error_result(
                f"Failed to clear buffers: {str(e)}")
    ...
    def get_processing_stats(self) -> Dict[str, Any]:
        ...
        avg_processing_time = (
            ...
            if self.processed_audio_count > 0
            else 0
        )
        ...
        try:
            ...
            async for chunk in audio_stream:
                request = AudioProcessingRequest(
                    audio_data=chunk, session_id="stream", format=AudioFormat.WAV)
                ...
                if result.success:
                    ...
                    self.logger.warning(
                        f"Failed to process stream chunk: {result.error_message}"
                    )

            return ProcessingResult.success_result(
                {"stream_processed": True, "processed_chunks": processed_chunks}
            )
    ...
    async def optimize_buffers(self) -> ProcessingResult:
        ...
        try:
            ...
            if input_stats["write_count"] > 0:
                avg_chunk_size = input_stats["total_bytes"] / \
                    input_stats["write_count"]
                if avg_chunk_size < self.input_buffer.config.chunk_size * 0.5:
                    optimizations.append(
                        "Consider reducing chunk size for better efficiency"
                    )
            ...
            return ProcessingResult.error_result(
                f"Buffer optimization failed: {str(e)}"
            )

    async def process_audio(
            self,
            request: AudioProcessingRequest) -> ProcessingResult:
