    ...
    async def check_input_content(self, text: str) -> ProcessingResult:
        ...
        if not self.moderation_service:
            return ProcessingResult.success_result(
                {"allowed": True, "reason": "No moderation service"}
            )
        ...
        try:
            ...
            if not moderation_result.get("allowed", True):
                ...
                return ProcessingResult.error_result(
                    "عذراً، لا يمكنني الإجابة على هذا السؤال. هل يمكنك طرح سؤال آخر؟", metadata={
...
class SessionContextBuilder:
    ...
    def build_conversation_history(
        ...
        try:
            ...
            # Get recent messages
            session_history = self.session_manager.session_history.get(
                session_id, [])
            ...
            self.logger.debug(
                f"Built conversation history with {len(history)} messages"
            )
            return history
...
class LLMProcessor:
    ...
    async def process_llm_request(
            self, request: LLMRequest) -> ProcessingResult:
        """
    ...
    def _build_conversation_context(self, request: LLMRequest):
        ...
        try:
            ...
            # Build history
            history = self.context_builder.build_conversation_history(
                request.session_id
            )
    ...
    async def _generate_llm_response(
        self, conversation, request: LLMRequest
    ) -> ProcessingResult:
    ...
    async def _check_output_moderation(
            self, response_text: str) -> ProcessingResult:
    ...
    async def _handle_llm_error(
        self, request: LLMRequest, error_message: str
    ) -> ProcessingResult:
        """Handle LLM generation errors with retry logic"""
        # Check if should retry
        if self.retry_handler.should_retry(
            request.retry_count, Exception(error_message)
        ):
            self.retry_attempts += 1
            self.logger.info(
                f"Retrying LLM request (attempt {request.retry_count + 1})"
            )
    ...
    async def _log_successful_interaction(
            self, request: LLMRequest, response: str):
