...
class WebSocketManager:
        ...
        try:
            ...
            async def server():
                async with websockets.serve(
                    self.handle_client_connection, self.config.host, self.config.port
                ):
                    self.logger.info(
    ...
    async def handle_client_connection(
        ...
        try:
            ...
            self.logger.error(
                f"Error handling client connection {session_id}: {e}")
    ...
    async def _process_raw_message(
        ...
        try:
            ...
            message = WebSocketMessage(
                type=message_type, data=data, session_id=session_id
            )
    ...
    async def _route_message(
        ...
        if handler:
            ...
            await self._send_error_message(
                websocket, f"Unknown message type: {message.type}"
            )
    ...
    async def _unregister_connection(
        ...
        self.logger.info(
            f"Client unregistered: {session_id} (remaining: {len(self.active_connections)})"
        )
    ...
    async def broadcast_message(
        self, message: WebSocketMessage, exclude_session: Optional[str] = None
    ) -> ProcessingResult:
        ...
        self.logger.info(
            f"Broadcast completed: {successful_sends} successful, {failed_sends} failed"
        )
    ...
    async def send_ping_to_client(
        ...
        ping_message = WebSocketMessage(
            type="ping", data={"message": "ping"}, session_id=session_id
        )
