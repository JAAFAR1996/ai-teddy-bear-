...
class TTSProviderManager:
    ...
    def get_available_providers(self) -> list:
        ...
        return [
            provider
            for provider, available in self._provider_availability.items()
            if available
        ]

    def get_fallback_provider(
            self,
            preferred: TTSProvider) -> Optional[TTSProvider]:
...
class ElevenLabsTTSProcessor:
    ...
    async def synthesize(
            self,
            request: TextToSpeechRequest) -> ProcessingResult:
        ...
        try:
            ...
            self.logger.debug(
                f"ElevenLabs synthesis successful: {len(audio_bytes)} bytes"
            )
...
class GTTSProcessor:
    ...
    async def synthesize(
        ...
        try:
            ...
            lang = (
                request.language
                if request.language in ["ar", "en", "es", "fr"]
                else "ar"
            )
            ...
            self.logger.debug(
                f"gTTS synthesis successful: {len(audio_bytes)} bytes")
    ...
    async def synthesize(
        ...
        try:
            ...
            if result.reason == speechsdk.ResultReason.SynthesizingAudioCompleted:
                ...
                self.logger.debug(
                    f"Azure synthesis successful: {len(audio_bytes)} bytes"
                )
...
class TextToSpeechService:
    ...
    def __init__(
        ...
        if elevenlabs_api_key and self.provider_manager.is_provider_available(
            TTSProvider.ELEVENLABS
        ):
            self.processors[TTSProvider.ELEVENLABS] = ElevenLabsTTSProcessor(
                elevenlabs_api_key
            )
        ...
        if (
            azure_api_key
            and azure_region
            and self.provider_manager.is_provider_available(TTSProvider.AZURE)
        ):
            self.processors[TTSProvider.AZURE] = AzureTTSProcessor(
        ...
        self.logger.info(
            f"TTS Service initialized with {len(self.processors)} providers"
        )

    async def synthesize_speech(
            self, request: TextToSpeechRequest) -> ProcessingResult:
        """
    ...
    async def _perform_synthesis(
        self, provider: TTSProvider, request: TextToSpeechRequest
    ) -> ProcessingResult:
        ...
        if result.success:
            ...
            self.logger.warning(
                f"Synthesis failed with {provider}: {result.error_message}"
            )
    ...
    async def test_provider(self, provider: TTSProvider) -> ProcessingResult:
        ...
        test_request = TextToSpeechRequest(
            text="This is a test message", provider=provider, language="en"
        )
