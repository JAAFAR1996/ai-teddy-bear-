...
from ...domain.entities.conversation import Content, ContentType, Conversation, InteractionType, Message
...
class ConversationService(BaseService[Conversation]):
    ...
    async def start_conversation(self, child: Child, initial_message: Optional[str] = None) -> Conversation:
        ...
        conversation = Conversation(
            child_id=child.id,
            metadata={"language": child.preferences.language, "interaction_style": child.preferences.interaction_style},
        )

        if initial_message:
            conversation.add_message(
                content=initial_message, content_type=ContentType.TEXT, interaction_type=InteractionType.CHILD_TO_AI
            )

        await self._log_operation("start_conversation", conversation.id, {"child_id": str(child.id)})
    ...
    async def add_child_message(
        ...
        await self._log_operation(
            "add_child_message", conversation_id, {"content_type": content_type.value, "emotion": emotion}
        )
    ...
    async def get_conversation_context(self, conversation_id: UUID, message_limit: int = 5) -> str:
        """Get recent conversation context for AI processing."""
        return await self._repository.get_conversation_context(conversation_id, message_limit)

    async def analyze_emotion_trends(self, conversation_id: UUID) -> List[Tuple[datetime, str]]:
    ...
    async def get_child_conversations(
        ...
        return await self._repository.find_by_child(child_id, start_date, end_date, limit)
    ...
    async def archive_conversation(self, conversation_id: UUID, reason: str) -> bool:
        ...
        await self._log_operation("archive_conversation", conversation_id, {"reason": reason})
        return result
    ...
    async def search_conversations(
        ...
        return await self._repository.find_by_content(keyword, content_type, start_date, end_date)
    ...
    async def clean_old_conversations(self) -> int:
        ...
        old_conversations = await self._repository.find_by_content(keyword="", end_date=cutoff_date)

        for conv in old_conversations:
            await self.archive_conversation(conv.id, f"Automated cleanup - Age > {self.content_retention_days} days")
            count += 1

        await self._log_operation(
            "clean_old_conversations", details={"cleaned_count": count, "cutoff_date": cutoff_date.isoformat()}
        )
        return count

    async def get_similar_conversations(self, conversation_id: UUID, limit: int = 5) -> List[Conversation]:
