...
class ExportService:
            ...
            if conversations:
                ...
                for conv in conversations[:20]:  # Limit to 20 for PDF size
                    row = [
                        ...
                        conv.get('summary', '')[:100] + '...' if len(conv.get('summary', '')) > 100 else conv.get('summary', '')
                    ]
    ...
    async def _export_analytics_excel(
        ...
        with pd.ExcelWriter(buffer, engine='xlsxwriter') as writer:
            # Summary sheet
            summary_data = {
                'Metric': ['Total Conversations', 'Total Duration (minutes)', 'Average Session (minutes)', 'Quality Score'],
                'Value': [
