...
class CleanupExecutionService:
    ...
    async def execute_cleanup(self, targets: List[CleanupTarget], report: CleanupReport, session_manager):
        ...
        try:
            ...
            for table_name in deletion_order:
                if table_name in targets_by_table:
                    deleted_count = await self._delete_table_records(
                        table_name, targets_by_table[table_name], report, session_manager
                    )
            ...
            self.logger.info(f"Cleanup completed: {report.total_records_deleted} records deleted")

        except Exception as e:
            report.add_error(f"Error executing cleanup: {str(e)}")
            self.logger.error(f"Error executing cleanup: {e}")

    def _group_targets_by_table(self, targets: List[CleanupTarget]) -> Dict[str, List[CleanupTarget]]:
    ...
    async def _delete_table_records(
        self, table_name: str, targets: List[CleanupTarget], report: CleanupReport, session_manager
    ) -> int:
    ...
    async def _delete_files(self, file_targets: List[CleanupTarget], report: CleanupReport) -> int:
        """حذف الملفات"""
        ...
        try:
                ...
                if cache_dir.exists():
                    for file_path in cache_dir.rglob("*"):
                        if file_path.is_file():
                            ...
                            if file_age.total_seconds() > 86400 and file_size < 1024 * 1024:  # 1MB
            ...
            for file_path in orphaned_files:
                try:
                    ...
                    self.logger.warning(f"Could not delete orphaned file {file_path}: {e}")
    ...
    def _update_report_counts(self, table_name: str, deleted_count: int, report: CleanupReport):
