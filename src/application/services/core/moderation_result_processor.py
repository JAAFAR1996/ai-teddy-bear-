...
class ModerationResultProcessor:
    ...
    def aggregate_results(
            self,
            results: List[ModerationResult]) -> ModerationResult:
        """📋 تجميع نتائج متعددة"""
        if not results:
        ...
        categories_and_scores = self._merge_categories_and_scores(
            valid_results)
    ...
    def format_response(
        self, result: ModerationResult, request: ModerationRequest
    ) -> Dict[str, Any]:
        """📝 تنسيق الاستجابة النهائية"""
        ...
        response = {
            ...
            "categories": [
                cat.value for cat in result.flagged_categories],
            "confidence": self._calculate_confidence(result),
            "reason": self._get_rejection_reason(result) if not allowed else None,
            "alternative_response": (
                self._get_alternative_response(result) if not allowed else None),
            "timestamp": (
                result.timestamp.isoformat() if hasattr(
                    result,
                    "timestamp") else None),
        }
    ...
    def _determine_overall_safety(
            self, results: List[ModerationResult]) -> bool:
    ...
    def _merge_categories_and_scores(
        self, results: List[ModerationResult]
    ) -> Dict[str, Any]:
        ...
        return {"categories": list(set(all_categories)), "scores": all_scores}
    ...
    def _get_rejection_reason(self, result: ModerationResult) -> str:
        ...
        try:
            return ModerationLookupTables.get_rejection_reason(
    ...
    def _get_alternative_response(self, result: ModerationResult) -> str:
        """💬 الحصول على رد بديل"""
        try:
            return ModerationLookupTables.get_alternative_response(
                result.flagged_categories
            )
        except Exception as e:
    ...
    def create_unsafe_response(
        self, reason: str, categories: List[ContentCategory]
    ) -> Dict[str, Any]:
        """❌ إنشاء رد غير آمن"""
        return {
            "allowed": False,
            ...
            "confidence": 0.9,
            "reason": reason,
            "alternative_response": ModerationLookupTables.get_alternative_response(
                categories
            ),
        }
