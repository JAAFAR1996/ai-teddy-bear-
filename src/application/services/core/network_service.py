...
class CloudNetworkService:
    ...
    async def handle_device_connection(
        ...
        try:
            ...
            logger.error(
                f"Connection handling failed for {device_id}: {str(e)}")
            raise
    ...
    async def handle_device_disconnection(self, device_id: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Disconnection handling failed for {device_id}: {str(e)}")
            return False

    async def send_message_to_device(
        self, device_id: str, message: NetworkMessage
    ) -> bool:
        """
        ...
        try:
            if device_id not in self.active_connections:
                logger.warning(
                    f"Cannot send message to offline device: {device_id}")
                return False

            # In real implementation, this would send via WebSocket/HTTP
            logger.info(
                f"Sending message to {device_id}: {message.message_type}")
    ...
    async def broadcast_message(
        self, message: NetworkMessage, exclude_devices: List[str] = None
    ) -> int:
        """
    ...
    def get_connection_status(
            self, device_id: str) -> Optional[Dict[str, Any]]:
