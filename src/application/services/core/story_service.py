...
from src.domain.story_generator import PersonalizedStoryGenerator, StoryTheme
...
class StoryService:
    ...
    def __init__(self, config):
        self.story_generator = PersonalizedStoryGenerator(openai_key=config["OPENAI_API_KEY"])
        self.voice_engine = MultiVoiceTTS(elevenlabs_key=config["ELEVENLABS_API_KEY"])

    async def create_and_narrate_story(self, child_id: str, current_emotion: str, story_type: str = "auto") -> Dict:
        """إنشاء وسرد قصة مخصصة"""

        # جلب ملف الطفل (افتراضي)
        profile = {"name": "أحمد", "age": 7, "interests": ["الحيوانات", "المغامرات"]}
        ...
        return {"story_id": story_data["id"], "audio_file": audio_file, "text": story_data["story"]}
