...
class ContentAnalyzer:
    ...
    def analyze_content(
        self, content: str, age: int = 10, language: str = "en"
    ) -> Dict[str, Any]:
        """
    ...
    def _check_age_appropriateness(
            self, content: str, age: int) -> Dict[str, Any]:
    ...
    def _create_safe_result(
        self, reason: str, confidence: float = 0.9, valid: bool = True
    ) -> Dict[str, Any]:
    ...
    def update_whitelist(self, words: List[str], action: str = "add") -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Whitelist updated: {action}ed {len(words)} words")
            return True
    ...
    def update_blacklist(self, words: List[str], action: str = "add") -> bool:
        ...
        try:
            ...
            self.logger.info(
                f"Blacklist updated: {action}ed {len(words)} words")
            return True
        except Exception as e:
    ...
    def get_analysis_stats(self) -> Dict[str, Any]:
        ...
        return {
            "whitelist_size": len(
                self.whitelist), "blacklist_size": len(
                self.blacklist), "age_rules_count": len(
                self.age_rules), "personal_info_patterns": len(
                    self.personal_info_patterns), "harmful_patterns": {
                        k: len(v) for k, v in self.harmful_patterns.items()}, }
