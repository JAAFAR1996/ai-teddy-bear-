...
class ModerationStatistics:
    ...
    def record_moderation_result(
        ...
        try:
            ...
            self._add_to_history(
                user_id, session_id, content_length, is_safe,
                severity, categories, result.get("confidence", 0.0), processing_time_ms
            )
    ...
    def _update_user_stats(self, user_id: str, is_safe: bool, categories: List[str]) -> None:
        ...
        for category in categories:
            user_data["categories"][category] = user_data["categories"].get(category, 0) + 1
    ...
    def get_general_stats(self) -> Dict[str, Any]:
        ...
        safe_rate = (self.stats["safe_content"] / total_requests * 100) if total_requests > 0 else 0
        
        return {
            "total_requests": total_requests,
            "safe_requests": self.stats["safe_content"],
            "blocked_requests": self.stats["blocked_content"],
            "safe_rate_percent": round(safe_rate, 2),
            ...
            "uptime_hours": round(self.performance_metrics["uptime_seconds"] / 3600, 2),
    ...
    def get_category_breakdown(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_common_category": max(self.category_stats.items(), key=lambda x: x[1])[0] if self.category_stats else "none"
        }
    ...
    def get_user_stats(self, user_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "users_with_blocked_content": sum(
                1 for user_data in self.user_stats.values()
    ...
    def get_hourly_trends(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_requests_in_period": sum(data["total"] for data in recent_hours.values())
        }
    
    def get_recent_activity(self, limit: int = 100) -> List[Dict[str, Any]]:
        """Get recent moderation activity"""
        recent_entries = self.recent_entries[-limit:] if limit > 0 else self.recent_entries
    ...
    def _calculate_error_rate(self) -> float:
        ...
        return round(
            (self.performance_metrics["total_errors"] / total_requests) * 100, 2
        )
