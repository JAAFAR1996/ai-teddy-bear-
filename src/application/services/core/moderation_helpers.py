...
from .moderation import ContentCategory, ModerationSeverity, ModerationResult
...
class ModerationStateMachine:
    ...
    def _build_transition_table(self) -> Dict[Tuple[ModerationState, ModerationEvent], ModerationState]:
        """🗺️ جدول التحولات - Lookup Table بدلاً من الشروط"""
        return {
            # من البداية
            (ModerationState.STARTING, ModerationEvent.START): ModerationState.VALIDATING,
            
            # التحقق من صحة البيانات
            (ModerationState.VALIDATING, ModerationEvent.VALIDATE): ModerationState.CHECKING_CACHE,
            (ModerationState.VALIDATING, ModerationEvent.FAIL): ModerationState.FAILED,
            
            # فحص Cache
            (ModerationState.CHECKING_CACHE, ModerationEvent.CACHE_HIT): ModerationState.COMPLETED,
            (ModerationState.CHECKING_CACHE, ModerationEvent.CACHE_MISS): ModerationState.LOCAL_CHECK,
            
            # الفحص المحلي
            (ModerationState.LOCAL_CHECK, ModerationEvent.LOCAL_UNSAFE): ModerationState.GENERATING_RESPONSE,
            (ModerationState.LOCAL_CHECK, ModerationEvent.LOCAL_SAFE): ModerationState.AI_CHECK,
            
            # فحص AI
            (ModerationState.AI_CHECK, ModerationEvent.AI_UNSAFE): ModerationState.GENERATING_RESPONSE,
            (ModerationState.AI_CHECK, ModerationEvent.AI_SAFE): ModerationState.GENERATING_RESPONSE,
            (ModerationState.AI_CHECK, ModerationEvent.AI_FAILED): ModerationState.GENERATING_RESPONSE,
            
            # إنتاج النتيجة
            (ModerationState.GENERATING_RESPONSE, ModerationEvent.GENERATE): ModerationState.COMPLETED,
            
            # الإكمال
            (ModerationState.COMPLETED, ModerationEvent.COMPLETE): ModerationState.COMPLETED,
        }
...
class ConditionalDecomposer:
    ...
    @staticmethod
    def has_risky_categories(categories: List[ContentCategory], risky_list: List[ContentCategory]) -> bool:
        """⚠️ شرط مبسط: هل يحتوي على تصنيفات خطيرة؟"""
        return any(cat in risky_list for cat in categories)
    
    @staticmethod
    def should_use_ai_check(content_length: int, local_result_safe: bool, ai_enabled: bool) -> bool:
        """🤖 شرط مبسط: هل نحتاج فحص AI؟"""
        return (
            ai_enabled and 
            local_result_safe and 
            content_length > 20
        )
    
    @staticmethod
    def should_alert_parent(severity: ModerationSeverity, user_violations_count: int) -> bool:
        """👨‍👩‍👧‍👦 شرط مبسط: هل نحتاج تنبيه الوالدين؟"""
        return (
            severity in [ModerationSeverity.HIGH, ModerationSeverity.CRITICAL] or
            user_violations_count >= 3
        )
