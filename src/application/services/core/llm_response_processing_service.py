...
class LLMResponseProcessingService:
    ...
    def __init__(self, llm_factory, moderation_service=None, parent_dashboard=None):
        self.llm_factory = llm_factory
        self.moderation_service = moderation_service
        self.parent_dashboard = parent_dashboard
        self.logger = logging.getLogger(self.__class__.__name__)

    async def process_llm_request(self, text: str, session_id: str = None, retry_count: int = 0, session_manager=None) -> str:
...
class LLMRequestProcessor:
    ...
    def __init__(self, service, text: str, session_id: str = None, retry_count: int = 0, session_manager=None):
    ...
    async def _generate_response(self, context: Dict) -> Dict:
        ...
        context["llm_response"] = await self._generate_llm_response(context["conversation"])
        return {"continue": True}
    ...
    def _build_conversation_context(self) -> Conversation:
        ...
        try:
            ...
            self.service.logger.error(f"Error building conversation context: {e}")
    ...
    def _add_recent_messages_to_history(self, history: List[Message]) -> None:
        ...
        recent_messages = self.session_manager.get_recent_messages(self.session_id, limit=5)
    ...
    def _convert_message_to_history_entry(self, msg: Dict) -> Optional[Message]:
        """Extract function: Convert session message to conversation Message"""
        # Table lookup pattern instead of nested if-elif
        message_type_mapping = {
            'user_audio': lambda content: Message(role='user', content=content),
            'assistant': lambda content: Message(role='assistant', content=content)
        }
    ...
    async def _log_interaction_to_services(self, response: str) -> None:
        ...
        try:
            ...
            if self.session_id and self.service.parent_dashboard and self.session_manager:
    ...
    async def _handle_error(self, error: Exception) -> str:
        ...
        if self.retry_count < 2:
            ...
            if any(err in error_str for err in retryable_errors):
                self.service.logger.info(f"Retrying LLM request (attempt {self.retry_count + 1})")
                await asyncio.sleep(1)  # Brief delay
                
                # Create new processor with incremented retry count
                retry_processor = LLMRequestProcessor(
                    self.service, self.text, self.session_id, 
                    self.retry_count + 1, self.session_manager
                )
