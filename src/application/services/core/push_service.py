...
class PushService:
    ...
    def _load_config(self) -> Any:
        ...
        try:
            ...
            self.apns_bundle_id = push_config.get(
    ...
    async def send_batch_notifications(
            self, notifications: List[Dict]) -> Dict[str, int]:
        """
    ...
    async def _send_fcm_notification(
        ...
        try:
            ...
            if not self.fcm_server_key or self.fcm_server_key == "your_fcm_key":
                self.logger.info(
                    "FCM notification simulated (no server key configured)",
                    device_id=device_id)
                return True
    ...
    async def _send_apns_notification(
        ...
        try:
            ...
            if not self.apns_key_id or self.apns_key_id == "your_apns_key":
                self.logger.info(
                    "APNs notification simulated (no key configured)",
                    device_id=device_id)
                return True
    ...
    async def _simulate_notification(
            self,
            device_id: str,
            notification: Dict) -> bool:
...
if __name__ == "__main__":
    ...
    async def test_push():
        ...
        logger.error(
            f"Simple push test: {'✅ Success' if success else '❌ Failed'}")
        ...
        logger.error(
            f"Data push test: {'✅ Success' if success else '❌ Failed'}")

        # اختبار إرسال مجموعة
        notifications = [{"device_id": "device_1",
                          "notification": {"title": "Batch 1",
                                           "body": "First notification"},
                          "platform": "android"},
                         {"device_id": "device_2",
                          "notification": {"title": "Batch 2",
                                           "body": "Second notification"},
                          "platform": "ios"}]

        stats = await push_service.send_batch_notifications(notifications)
        logger.info(
            f"Batch push test: {stats['sent']}/{stats['total']} sent successfully")
