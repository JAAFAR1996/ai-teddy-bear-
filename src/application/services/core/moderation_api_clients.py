...
class ModerationAPIClients:
    ...
    def _init_azure_client(self) -> None:
        ...
        try:
            ...
            if key and endpoint:
                ...
                self.logger.info(
                    "Azure Content Safety client initialized successfully")
            else:
                self.logger.warning(
                    "Azure Content Safety credentials not found")
        except Exception as e:
    ...
    def _init_google_client(self) -> None:
        ...
        try:
            if getattr(self.config.api_keys, "GOOGLE_CLOUD_CREDENTIALS", None):
                ...
                self.logger.info(
                    "Google Cloud NLP client initialized successfully")
            else:
    ...
    async def check_with_openai(
            self, request: ModerationRequest) -> ModerationResult:
        ...
        try:
            response = await self.openai_client.moderations.create(
                model="text-moderation-stable", input=request.content
            )
    ...
    def _process_openai_result(self, result) -> ModerationResult:
        ...
        for (
            openai_category,
            our_category,
        ) in ModerationLookupTables.OPENAI_CATEGORY_MAPPING.items():
    ...
    async def check_with_azure(
            self,
            request: ModerationRequest) -> ModerationResult:
        ...
        for result_type, category in severity_mapping.items():
            if hasattr(response, result_type):
                ...
                if result.severity > 0:
                    ...
                    # تحويل إلى 0-1
                    confidence_scores[category] = result.severity / 6
    ...
    async def check_with_google(
        ...
        try:
            ...
            return self._process_google_result(
                sentiment_response, entities_response)

        except Exception as e:
            self.logger.error(f"Google moderation error: {e}")
            return self._create_error_result("Google", str(e))

    def _process_google_result(
        self, sentiment_response, entities_response
    ) -> ModerationResult:
        """📊 معالجة نتيجة Google"""
        categories = []
        confidence_scores = {}
