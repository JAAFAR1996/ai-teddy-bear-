...
class WebSocketConnectionService:
    ...
    async def start_server(self, message_handler):
        ...
        async def server():
            async with websockets.serve(
                lambda ws, path: self.handle_client_connection(ws, path, message_handler),
                self.host,
                self.port
            ):
    ...
    async def handle_client_connection(self, websocket: WebSocketServerProtocol, path: str, message_handler):
    ...
    async def send_welcome_message(self, websocket: WebSocketServerProtocol, session_id: str):
    ...
    async def send_json_message(self, websocket: WebSocketServerProtocol, data: dict):
    ...
    async def send_error_message(self, websocket: WebSocketServerProtocol, error_message: str):
    ...
    async def broadcast_message(self, data: dict):
        ...
        for connection in self.active_connections.copy():  # Copy to avoid modification during iteration
            task = asyncio.create_task(self._safe_send_to_connection(connection, data))
    ...
    async def _safe_send_to_connection(self, connection: WebSocketServerProtocol, data: dict):
    ...
    async def connect_to_elevenlabs(self, api_key: str, voice_id: str, model_id: str = "eleven_multilingual_v2"):
    ...
    async def handle_elevenlabs_reconnect(self, api_key: str, voice_id: str, model_id: str):
        ...
        self.logger.info(f"Reconnecting to ElevenLabs in {delay}s (attempt {self.reconnect_attempts})")
