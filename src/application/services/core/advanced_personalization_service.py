...
class AdvancedPersonalizationService:
    ...
    def analyze_personality_from_interactions(self, child_id: str, interactions: List[Dict]) -> ChildPersonality:
        ...
        # تحديث مستوى الفضول
        unique_topics = len(set(interaction.get('topic', '') for interaction in interactions if interaction.get('topic')))
        personality.curiosity_level = min(1.0, unique_topics / 10)  # 10 مواضيع مختلفة = فضول كامل
        
        # تحديث مستوى الإبداع
        creative_activities = sum(1 for interaction in interactions 
    ...
    def track_content_performance(self, child_id -> Any: str, content -> Any: Dict, performance_data -> Any: Dict) -> Any:
        ...
        for content_item in self.content_performance[child_id]:
            if (content_item.content_type == adaptive_content.content_type and 
                content_item.topic == adaptive_content.topic and
                content_item.difficulty_level == adaptive_content.difficulty_level):
        ...
        if existing_content:
            ...
            existing_content.engagement_score = (existing_content.engagement_score + adaptive_content.engagement_score) / 2
    ...
    def _get_personality_summary(self, personality: ChildPersonality) -> Dict:
        ...
        return {
            ...
            "curiosity_level": "عالي" if personality.curiosity_level > 0.6 else "متوسط" if personality.curiosity_level > 0.3 else "منخفض",
            "creativity_level": "عالي" if personality.creativity_level > 0.6 else "متوسط" if personality.creativity_level > 0.3 else "منخفض"
        }
    ...
    def _analyze_engagement_patterns(self, child_id: str) -> Dict:
        ...
        best_time = max(patterns.attention_patterns.items(), key=lambda x: x[1]) if patterns.attention_patterns else ("morning", 0)
    ...
    def _get_optimization_suggestions(self, personality: ChildPersonality, patterns: InteractionPattern) -> List[str]:
        ...
        best_time = max(patterns.attention_patterns.items(), key=lambda x: x[1])[0] if patterns.attention_patterns else None
        if best_time:
