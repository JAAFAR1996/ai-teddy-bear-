        ...
        if "openai" not in self._cached_keys:
            try:
                if self.secrets_manager:
                    ...
                    self._cached_keys["openai"] = os.getenv(
                        "OPENAI_API_KEY", "")
        ...
        if "anthropic" not in self._cached_keys:
            try:
                if self.secrets_manager:
                    ...
                    self._cached_keys["anthropic"] = os.getenv(
                        "ANTHROPIC_API_KEY", "")
            except Exception as e:
    ...
    async def get_azure_content_safety_key(self) -> str:
        ...
        if "azure_content_safety" not in self._cached_keys:
            try:
                ...
                self.logger.error(
                    f"❌ Failed to get Azure Content Safety key: {e}")
                self._cached_keys["azure_content_safety"] = ""
...
class ModerationService:
    ...
    async def check_content(
        ...
        try:
            ...
            service_result = self._convert_analysis_to_service_format(
                analysis_result)
            ...
            return self._create_safe_response(
                "Processing error - content allowed")
...
def create_moderation_request(
        ...
        user_id: Optional[str] = None,
        age: int = 10,
        language: str = "en") -> ModerationRequest:
