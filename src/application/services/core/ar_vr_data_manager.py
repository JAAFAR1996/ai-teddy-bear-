...
class ARVRDataManager:
    ...
    def save_ar_experiences(
            self, experiences: Dict[str, ARExperience]) -> bool:
    ...
    def save_vr_environments(
            self, environments: Dict[str, VREnvironment]) -> bool:
    ...
    def save_child_preferences(
            self, child_preferences: Dict[str, Dict]) -> bool:
        """حفظ تفضيلات الأطفال"""
        try:
    ...
    def get_data_statistics(self) -> Dict:
        ...
        try:
            ...
            if sessions_file.exists():
                with open(sessions_file, "r", encoding="utf-8") as f:
                    ...
                    stats["total_sessions"] = sum(
                        len(sessions) for sessions in sessions_data.values()
                    )
            ...
            total_size = sum(
                f.stat().st_size for f in self.data_dir.glob("*.json"))
        ...
        try:
            ...
            for child_id, sessions in sessions_data.items():
                ...
                sessions_data[child_id] = [
                    ...
                    if datetime.fromisoformat(session.get("start_time", ""))
                    > cutoff_date
                ]
