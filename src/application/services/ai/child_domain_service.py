...
class ChildDomainService:
    ...
    def _assess_parent_involvement_and_review(
        ...
        requires_parent_review = (
            ...
            or child.safety_violations_count >= 2
            or child.escalation_count > 0
        )

        if requires_parent_review and "parent review" not in [
    ...
    def _calculate_voice_adjustments(
        ...
        if (
            "frustrated" in emotional_patterns
            and emotional_patterns["frustrated"] > 0.3
        ):
    ...
    def _apply_voice_adjustments(
        ...
        new_emotion_sensitivity = max(
            ...
            min(
                1.0,
                current_profile.emotion_sensitivity + emotion_sensitivity_adjustment,
            ),
        )
    ...
    def validate_child_aggregate_consistency(self, child: Child) -> List[str]:
        ...
        if child.voice_profile and not child.voice_profile.is_age_appropriate(
            child.age
        ):
            violations.append(
        ...
        if child.safety_settings:
            if (
                child.safety_settings.max_session_minutes
    ...
    def _assess_topic_age_appropriateness(self, topic: str, age: int) -> float:
        ...
        # Get age-appropriate keywords for this age
    ...
    def _find_age_keywords(self, age: int) -> Optional[Dict[str, List[str]]]:
        ...
        age_appropriateness_map = self._get_age_appropriateness_map()
    ...
    def _analyze_conversation_history_compatibility(
        self, topic: str, conversations: List[Conversation]
    ) -> float:
    ...
    def _assess_emotional_compatibility(
        self, topic: str, current_emotion: str
    ) -> float:
    ...
    def _assess_time_appropriateness(
        ...
        if 6 <= hour <= 11:  # Morning
            ...
            if any(
                word in topic_lower
                for word in ["play", "games", "adventure", "learning"]
            ):
                return 1.0
    ...
    def _assess_conversation_patterns_safety(
