...
from ....domain.emotion.models import (
    EmotionResult, EmotionTrend, ChildEmotionProfile
)
...
class EmotionHistoryService:
    ...
    async def get_emotional_stability_score(
        ...
        try:
            ...
            positive_ratio = (
                emotion_counts['happy'] + emotion_counts['calm']
            ) / total
    ...
    def _calculate_emotion_trend(
        ...
        significance = (
            "high" if abs(change) > 0.3 else
            "medium" if abs(change) > 0.1 else
            "low"
        )
    ...
    def _identify_emotional_triggers(self, emotions: List[EmotionResult]) -> List[str]:
        ...
        negative_emotions = [
            e for e in emotions 
            if e.primary_emotion in ['sad', 'angry', 'scared']
        ]
        ...
        for hour, hour_emotion_list in hour_emotions.items():
            negative_ratio = sum(
                1 for e in hour_emotion_list 
                if e in ['sad', 'angry', 'scared']
    ...
    def _identify_positive_indicators(self, emotions: List[EmotionResult]) -> List[str]:
        ...
        positive_count = sum(
            1 for e in emotions 
            if e.primary_emotion in ['happy', 'calm', 'curious']
        )
    ...
    def _identify_risk_factors(self, emotions: List[EmotionResult]) -> List[str]:
        ...
        concerning_indicators = [
            'short response', 'hesitation', 'quiet voice', 'withdrawal'
        ]
