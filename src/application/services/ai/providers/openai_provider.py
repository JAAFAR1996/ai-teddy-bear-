...
""",
        ...
        logger.info(
            f"OpenAI provider initialized with model: {self.default_model}")
    ...
    def _create_system_prompt(self, request: AIRequest) -> str:
        ...
        # Add safety rules
        safety_rules = "\\n".join(
            f"- {rule}" for rule in self.child_prompts["safety_rules"]
        )
        base_prompt += f"\\n\\nSafety Rules:\\n{safety_rules}"
    ...
    def _select_model(self, request: AIRequest) -> str:
        ...
        if (
            len(request.message) > 100
            or request.context.get("complexity", "normal") == "high"
        ):
            model = "gpt-4"

        # Use GPT-3.5 for simple requests to save costs
        elif (
            len(request.message) < 50
            and request.context.get("complexity", "normal") == "low"
        ):
            model = "gpt-3.5-turbo"
    ...
    def _process_openai_response(
        self, response: Dict[str, Any], request: AIRequest, model: str
    ) -> AIResponse:
    ...
    def _calculate_confidence(
            self, message: str, response: Dict[str, Any]) -> float:
    ...
    async def _check_response_safety(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            flagged_categories = [cat for cat,
                                  flagged in categories.items() if flagged]
...
Primary emotion:"""
            ...
            primary_emotion = emotion_mapping.get(
                emotion_text, EmotionType.NEUTRAL)
...
Category:"""
    ...
    def estimate_cost(self, request: AIRequest) -> float:
        ...
        config = self.model_configs.get(
            model, self.model_configs["gpt-3.5-turbo"])

        # Estimate tokens (rough approximation)
        estimated_tokens = len(request.message.split()) * \
