...
class BaseAIProvider(ABC):
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                return await asyncio.wait_for(
                    self._make_request(request), timeout=self.timeout
                )
    ...
    async def _post_process_response(
        self, response: AIResponse, request: AIRequest
    ) -> AIResponse:
    ...
    async def _create_error_response(
        self, request: AIRequest, error: str
    ) -> AIResponse:
    ...
    def _update_success_metrics(
        self, processing_time: float, response: AIResponse
    ) -> None:
    ...
    def get_health_status(self) -> HealthStatus:
        ...
        time_since_check = (
            datetime.utcnow() -
            self.last_health_check).total_seconds()
