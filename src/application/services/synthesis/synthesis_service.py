...
class ModernSynthesisService:
    ...
    async def initialize(
        self, credentials: Optional[SynthesisServiceCredentials] = None
    ) -> None:
        ...
        try:
            ...
            logger.info(
                f"🚀 Synthesis service initialized with providers: {available_providers}"
            )
    ...
    async def _initialize_providers(
        self, credentials: SynthesisServiceCredentials
    ) -> None:
        ...
        for provider_type, provider in self.providers.items():
            try:
                ...
                if success:
                    logger.info(
                        f"✅ {provider.get_provider_name()} provider initialized"
                    )
                else:
                    logger.warning(
                        f"⚠️ {provider.get_provider_name()} provider not available"
                    )
            except Exception as e:
                logger.error(
                    f"❌ Failed to initialize {provider.get_provider_name()}: {e}"
                )
    ...
    async def synthesize_stream(
        ...
        try:
            # إعداد سياق التركيب
            context = await self._prepare_synthesis_context(
                text, emotion, character_id, language
            )
    ...
    async def synthesize_audio(
        ...
        try:
            ...
            logger.debug(
                f"🎯 Synthesized in {time.time() - start_time:.2f}s: '{text[:50]}...'"
            )
            return audio_data
    ...
    async def _prepare_synthesis_context(
        ...
        """إعداد سياق التركيب"""
        character = await self.character_manager.select_character(
            character_id, language
        )
        voice_settings = character.emotional_settings.get(
            emotion, character.emotional_settings[EmotionalTone.FRIENDLY]
        )
    ...
    async def _execute_streaming_synthesis(
    ...
    async def _fallback_stream_synthesis(
            self, text: str) -> AsyncIterator[bytes]:
    ...
    def _update_performance_stats(
        ...
        self.performance_monitor.update_synthesis_stats(
            provider=provider, processing_time=processing_time, success=success
        )
