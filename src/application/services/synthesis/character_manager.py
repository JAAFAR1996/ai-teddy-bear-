...
class VoiceCharacterManager:
        ...
        if language:
            for character in self.voice_characters.values():
                if character.language == language:
                    logger.debug(f"Selected character by language: {character.id}")
                    return character
        
        # Return current or default character
        default_character = self.current_character or self.voice_characters.get("teddy_en")
    ...
    def _create_fallback_character(self) -> VoiceCharacter:
        ...
        basic_settings = {
            EmotionalTone.FRIENDLY: VoiceSettings(stability=0.5, similarity_boost=0.5, style=0.5)
        }
    ...
    def remove_character(self, character_id: str) -> bool:
        ...
        if character_id in self.voice_characters:
            ...
            logger.warning(f"Character ID not found for removal: {character_id}")
            return False
