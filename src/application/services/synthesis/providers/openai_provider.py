...
class OpenAIProvider(BaseSynthesisProvider):
    ...
    async def synthesize_audio(
        ...
        try:
            ...
            logger.debug(
                f"OpenAI synthesis completed: {len(audio_data)} bytes")
            return audio_data
    ...
    async def synthesize_stream(
        ...
        try:
            ...
            try:
                response = await self.client.audio.speech.create(
                    model="tts-1", voice=context.character.voice_id, input=context.text
                )
                audio_data = response.content
    ...
    async def health_check(self) -> dict:
        ...
        try:
            ...
            response = await self.client.audio.speech.create(
                model="tts-1", voice="alloy", input="Test"  # Default voice
            )
