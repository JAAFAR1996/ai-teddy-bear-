...
class StreamingAudioBuffer:
    ...
    def __init__(self, config: SynthesisConfig):
        ...
        logger.debug(
            f"Audio buffer initialized with chunk_size={config.chunk_size}")
    ...
    async def write(self, audio_chunk: bytes) -> None:
        ...
        async with self.lock:
            ...
            logger.debug(
                f"Written {len(audio_chunk)} bytes, total: {self.total_bytes}")
    ...
    async def get_stats(self) -> dict:
        ...
        async with self.lock:
            return {
                ...
                "buffer_utilization": (
                    len(self.buffer) / self.config.buffer_size
