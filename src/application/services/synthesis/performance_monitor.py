...
class PerformanceMonitor:
    ...
    def update_synthesis_stats(
        ...
        try:
            ...
            self.stats["provider_response_times"][provider_name].append(
                processing_time)

            # Keep only last 100 response times per provider
            if len(self.stats["provider_response_times"][provider_name]) > 100:
                self.stats["provider_response_times"][provider_name] = self.stats[
                    "provider_response_times"
                ][provider_name][-100:]

            logger.debug(
                f"Updated stats for provider {provider_name}: {processing_time:.3f}s"
            )
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            avg_processing_time = (
                ...
                self.stats["total_syntheses"] if self.stats["total_syntheses"] > 0 else 0)
            ...
            for provider, usage_count in self.stats["provider_usage"].items():
                ...
                provider_metrics[provider] = {
                    ...
                    usage_count if usage_count > 0 else 0,
                    "avg_response_time": (
                        sum(response_times) /
    ...
    def get_provider_health(self, provider: VoiceProvider) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                f"Failed to get provider health for {provider.value}: {e}")
    ...
    def track_operation_end(
        ...
        self.update_synthesis_stats(
            provider, processing_time, audio_duration, success)

        logger.debug(
            f"Completed tracking operation: {operation_id} ({processing_time:.3f}s)"
        )
