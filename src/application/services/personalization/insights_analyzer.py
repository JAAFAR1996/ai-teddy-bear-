...
class PersonalizationInsightsAnalyzer:
    ...
    def get_personalization_insights(
        ...
        try:
            return {
                ...
                "learning_style_analysis": self._determine_learning_style(
    ...
    def _get_personality_summary(self, personality: ChildPersonality) -> Dict:
        ...
        return {
            ...
            "creativity_level": self._get_level_description(
                personality.creativity_level
            ),
    ...
    def _determine_learning_style(
        ...
        dominant_style = (
            max(learning_prefs.items(), key=lambda x: x[1])
            if learning_prefs
            else ("storytelling", 0.5)
        )
    ...
    def _analyze_engagement_patterns(
            self, patterns: InteractionPattern) -> Dict:
        ...
        total_attention = (
            sum(patterns.attention_patterns.values())
            if patterns.attention_patterns
            else 1
        )
        time_distribution = (
    ...
    def _analyze_content_preferences(
        ...
        return {
            ...
            "average_engagement": np.mean(
                [c.engagement_score for c in content_performance]
            ),
        }
    ...
    def _get_optimization_suggestions(
        ...
        if personality.attention_span < 10:
            suggestions.append(
                "تقليل مدة الأنشطة لتناسب فترة الانتباه القصيرة")
        ...
        if personality.conscientiousness < 0.4:
            suggestions.append(
                "تقسيم المهام الكبيرة إلى خطوات صغيرة مع مكافآت متكررة")

        if personality.neuroticism > 0.6:
            suggestions.append(
                "التركيز على الأنشطة المهدئة وتجنب المحتوى المثير للقلق")

        # اقتراحات بناءً على الأنماط
        if len(patterns.preferred_activities) < 3:
            suggestions.append(
                "تجربة أنواع أنشطة متنوعة لاكتشاف تفضيلات جديدة")
    ...
    def _identify_child_strengths(
            self, personality: ChildPersonality) -> List[str]:
    ...
    def _calculate_learning_efficiency(
        ...
        efficiency += ((personality.curiosity_level +
                        personality.creativity_level) / 2 * 0.4)
    ...
    def _get_dominant_response_pattern(
            self, patterns: InteractionPattern) -> str:
    ...
    def _calculate_engagement_consistency(
            self, patterns: InteractionPattern) -> float:
        """حساب ثبات الانخراط"""
    ...
    def _analyze_difficulty_performance(
        ...
        for content in content_performance:
            difficulty_scores[content.difficulty_level].append(
                content.engagement_score)
    ...
    def _get_developmental_priority(
            self, personality: ChildPersonality) -> str:
    ...
    def _get_skill_recommendations(
        ...
        if len(patterns.favorite_topics) > 0:
            recommendations.append(
                f"تعميق المعرفة في {patterns.favorite_topics[0]}")
    ...
    def _calculate_success_metrics(
        ...
        return {
            ...
            "high_engagement_percentage": len(
                [
                    s for s in engagement_scores if s > 0.7]) /
            len(engagement_scores) *
            100,
        }

    def _analyze_interaction_trends(
            self, patterns: InteractionPattern) -> Dict:
        """تحليل اتجاهات التفاعل"""
        return {
            "activity_diversity": len(
                patterns.preferred_activities),
            "topic_diversity": len(
                patterns.favorite_topics),
    ...
    def _calculate_response_consistency(
        ...
        proportions = [
            count /
            total_responses for count in patterns.response_patterns.values()]
