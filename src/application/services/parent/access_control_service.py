...
from ..models.control_models import (
    ParentalControl, AccessSchedule, AccessScheduleType,
    TimeUsageStats
)
...
class AccessControlService:
    ...
    def check_access_allowed(
        ...
        for schedule in schedules:
            if (schedule.enabled and 
                ...
                schedule.start_time <= current_time_only <= schedule.end_time):
                return True, None
    ...
    def should_trigger_time_warning(
        self,
        usage_stats: TimeUsageStats
    ) -> bool:
    ...
    def should_block_access_time_limit(
        ...
        return (
            usage_stats.is_daily_limit_exceeded() or
            usage_stats.is_session_limit_exceeded()
        )
    ...
    def _get_next_allowed_time(
        ...
        for schedule in schedules:
            if (schedule.enabled and 
                schedule.day_of_week == current_day and
                schedule.start_time > current_time_only):
