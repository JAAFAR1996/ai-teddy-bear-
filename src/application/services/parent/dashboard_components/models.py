...
class ChildProfileValidator:
    ...
    def validate_interests_and_language(
        self, interests: List[str], language: str
    ) -> None:
...
class InteractionLogValidator:
    ...
    def validate_required_fields(
        self, user_id: str, child_message: str, assistant_message: str
    ) -> None:
        """Validate required string fields"""
        self.validator.validate_non_empty_string(user_id, "user_id")
        self.validator.validate_non_empty_string(
            child_message, "child_message")
        self.validator.validate_non_empty_string(
            assistant_message, "assistant_message")

    def validate_message_length(
        self, child_message: str, assistant_message: str
    ) -> None:
        ...
        if len(assistant_message) > 10000:
            raise ValueError(
                "assistant_message cannot exceed 10000 characters")
...
class ChildProfileData:
    ...
    def __post_init__(self):
        ...
        profile_validator.validate_basic_fields(
            self.parent_id, self.name, self.age)
        profile_validator.validate_age_constraints(self.age)
        profile_validator.validate_interests_and_language(
...
class InteractionLogData:
    ...
    def __post_init__(self):
        ...
        self.timestamp = interaction_validator.set_default_timestamp(
            self.timestamp)
...
class ExportRequest:
    ...
    def __post_init__(self):
        ...
        if self.data_type not in valid_data_types:
            raise ValueError(
                f"data_type must be one of: {', '.join(valid_data_types)}")
...
class AlertRequest:
    ...
    def __post_init__(self):
        ...
        ValidationService.validate_non_empty_string(
