...
class AccessControlAlertsService:
    ...
    def __init__(self, orchestrator, alert_service, notification_service, access_control_service):
    ...
    async def check_access_allowed(self, child_id: str) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            if not allowed:
                self.logger.info(f"Access denied for child {child_id}: {reason}")
            else:
            ...
            self.logger.error(f"Failed to check access for child {child_id}: {e}")
    ...
    async def update_parental_controls(
        ...
        try:
            ...
            if not self._validate_parental_controls(controls):
                self.logger.warning(f"Invalid parental controls for child {child_id}")
                return False
            
            success = await self.orchestrator.update_parental_controls(child_id, controls)
            
            if success:
                self.logger.info(f"Successfully updated parental controls for child {child_id}")
                
                # Create audit log for control changes
                await self._log_control_changes(child_id, controls)
            else:
                self.logger.warning(f"Failed to update parental controls for child {child_id}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error updating parental controls for child {child_id}: {e}")
            return False
    ...
    async def set_access_schedule(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully set {schedule_type} schedule for child {child_id}")
            else:
                self.logger.warning(f"Failed to set schedule for child {child_id}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error setting access schedule for child {child_id}: {e}")
            return False
    ...
    async def create_alert(self, alert_request: AlertRequest) -> bool:
        ...
        try:
            ...
            if alert:
                ...
                self.logger.warning(f"Failed to create alert for child {alert_request.child_id}")
                return False
    ...
    async def get_active_alerts(self, parent_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(alerts)} active alerts for parent {parent_id}")
            return alerts
            
        except Exception as e:
            self.logger.error(f"Failed to get active alerts for parent {parent_id}: {e}")
            return []
    ...
    async def acknowledge_alert(self, alert_id: str, parent_id: str) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Alert {alert_id} acknowledged by parent {parent_id}")
            else:
    ...
    async def dismiss_alert(self, alert_id: str, parent_id: str) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Alert {alert_id} dismissed by parent {parent_id}")
            else:
    ...
    async def get_alert_history(
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(alerts)} historical alerts for parent {parent_id}")
            return alerts
            
        except Exception as e:
            self.logger.error(f"Failed to get alert history for parent {parent_id}: {e}")
            return []
    ...
    async def update_notification_preferences(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Updated notification preferences for parent {parent_id}")
            else:
                self.logger.warning(f"Failed to update preferences for parent {parent_id}")
    ...
    async def _log_control_changes(self, child_id: str, controls: Dict[str, Any]) -> None:
    ...
    async def _apply_access_schedule(
        ...
        try:
            ...
            self.logger.info(f"Applied access schedule for child {child_id}: {len(schedules)} rules")
            return True
    ...
    async def _send_alert_notification(
        ...
        try:
            ...
            # Don't raise - notification failure shouldn't prevent alert creation
    ...
    async def _get_parent_id_for_child(self, child_id: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get parent ID for child {child_id}: {e}")
            return None
