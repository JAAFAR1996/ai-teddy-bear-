...
class ConversationSessionService:
    ...
    def __init__(self, session_service, content_analysis_service, cache_service):
    ...
    async def start_conversation_session(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(f"Started conversation session for user {user_id}")
    ...
    async def end_conversation_session(self, session_id: str):
        ...
        try:
            ...
            if conversation_log:
                ...
                self.logger.warning(f"No conversation log found for session {session_id}")
    ...
    async def get_active_sessions(self, child_id: str) -> list:
        ...
        try:
            ...
            self.logger.debug(f"Found {len(active_sessions)} active sessions for child {child_id}")
            return active_sessions
            
        except Exception as e:
            self.logger.error(f"Failed to get active sessions for child {child_id}: {e}")
            return []
    ...
    async def get_session_history(
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(sessions)} sessions for child {child_id}")
            return sessions
            
        except Exception as e:
            self.logger.error(f"Failed to get session history for child {child_id}: {e}")
            return []
    ...
    async def get_session_details(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if session_details:
                self.logger.debug(f"Retrieved details for session {session_id}")
            else:
            ...
            self.logger.error(f"Failed to get session details {session_id}: {e}")
            return None
    ...
    async def update_session_metadata(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully updated metadata for session {session_id}")
            else:
                self.logger.warning(f"Failed to update metadata for session {session_id}")
    ...
    async def get_real_time_session_status(self, child_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Failed to get real-time status for child {child_id}: {e}")
            return {"is_active": False, "error": str(e)}
    
    async def _analyze_interaction_content(self, interaction_data: InteractionLogData):
        ...
        try:
            ...
            assistant_analysis = self.content_analysis_service.analyze_assistant_response(
                interaction_data.assistant_message,
                interaction_data.child_message
            )
    ...
    async def _process_session_alerts(self, conversation_log):
        ...
        try:
            ...
            # Don't raise - alert processing failure shouldn't block session ending
