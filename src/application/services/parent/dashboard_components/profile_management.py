...
class ProfileManagementService:
    ...
    async def create_parent_account(
        ...
        try:
            ...
            self.logger.error(f"Failed to create parent account for {email}: {e}")
            raise
    ...
    async def update_child_profile(
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully updated child profile: {child_id}")
            else:
                self.logger.warning(f"Failed to update child profile: {child_id}")
    ...
    async def delete_child_profile(self, child_id: str) -> bool:
        ...
        try:
            ...
            if success:
                self.logger.info(f"Successfully deactivated child profile: {child_id}")
            else:
                self.logger.warning(f"Failed to deactivate child profile: {child_id}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error deactivating child profile {child_id}: {e}")
            return False
    ...
    async def get_children_for_parent(self, parent_id: str):
        ...
        try:
            ...
            self.logger.debug(f"Found {len(active_children)} active children for parent {parent_id}")
            return active_children
            
        except Exception as e:
            self.logger.error(f"Failed to get children for parent {parent_id}: {e}")
            raise
    ...
    async def update_parent_settings(
        ...
        try:
            ...
            if not filtered_settings:
                self.logger.warning(f"No valid settings to update for parent {parent_id}")
                return False
            ...
            if success:
                self.logger.info(f"Successfully updated parent settings: {parent_id}")
            else:
                self.logger.warning(f"Failed to update parent settings: {parent_id}")
            
            return success
            
        except Exception as e:
            self.logger.error(f"Error updating parent settings {parent_id}: {e}")
            return False
