...
class EmailService:
    ...
    async def send_html_email(self, 
        ...
        try:
            ...
            if success:
                ...
                self.logger.error("Failed to send HTML email", 
                                to=to_email, subject=subject)
            
            return success
            
        except Exception as e:
            self.logger.error("HTML email sending failed", 
                            to=to_email, error=str(e), exc_info=True)
            return False
    
    async def send_text_email(self, 
                            to_email: str, 
                            subject: str, 
        ...
        try:
            ...
            if success:
                self.logger.info("Text email sent successfully", 
                               to=to_email, subject=subject)
            else:
                self.logger.error("Failed to send text email", 
                                to=to_email, subject=subject)
            
            return success
            
        except Exception as e:
            self.logger.error("Text email sending failed", 
                            to=to_email, error=str(e), exc_info=True)
            return False
    ...
    async def _send_message(self, message: MimeMultipart, to_email: str) -> bool:
        ...
        try:
            ...
            if not self.password or self.password == "your_app_password":
                self.logger.info("Email sending simulated (no password configured)",
                               to=to_email)
                return True
    ...
    async def _add_attachment(self, message: MimeMultipart, attachment_path: Path):
        ...
        try:
            ...
            self.logger.error("Failed to add attachment", 
                            path=str(attachment_path), error=str(e))
