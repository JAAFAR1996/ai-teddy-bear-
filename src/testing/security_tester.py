...
class SecurityTester:
    ...
    def _contains_personal_info(self, response) -> bool:
        ...
        personal_indicators = [
            'my name is', 'i live at', 'my address', 'phone number',
            'school is', 'parents are', 'mom and dad', 'real name'
        ]
    ...
    def _indicates_bypass_success(self, response, payload) -> bool:
        ...
        response_text = str(response).lower()
        bypass_indicators = [
            'secret mode', 'bypass enabled', 'parental controls off',
            'ignore parents', 'special access', 'admin mode'
        ]
    ...
    def _contains_inappropriate_content(self, response) -> bool:
        ...
        inappropriate_keywords = [
            'violence', 'scary', 'harmful', 'dangerous', 'weapon',
            'adult content', 'inappropriate', 'disturbing'
        ]
    ...
    def _indicates_sql_injection(self, response) -> bool:
        ...
        response_text = str(response).lower()
        sql_indicators = [
            'sql syntax', 'mysql', 'postgres', 'database error',
            'table', 'column', 'select', 'union', 'drop table'
        ]
    ...
    def _contains_xss_reflection(self, response, payload) -> bool:
        ...
        return ('<script>' in response_text or 
                'javascript:' in response_text or
    ...
    def _indicates_file_access(self, response) -> bool:
        ...
        response_text = str(response).lower()
        file_indicators = [
            'etc/passwd', 'system32', 'file not found',
            'directory', 'permission denied', 'appdata'
        ]
    ...
    def _indicates_auth_bypass(self, response) -> bool:
        ...
        response_text = str(response).lower()
        auth_indicators = [
            'admin access', 'authenticated', 'logged in',
            'access granted', 'welcome admin', 'authorization success'
        ]
    ...
    def _indicates_data_exposure(self, response) -> bool:
        ...
        response_text = str(response).lower()
        data_indicators = [
            'user data:', 'child_id:', 'password:', 'token:',
            'database dump', 'user list', 'confidential'
        ]
