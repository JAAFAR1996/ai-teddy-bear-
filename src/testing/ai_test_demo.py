...
try:
    from .ai_test_generator import AITestGenerator, TestGenerationConfig, GeneratedTest
    from .smart_fuzzer import SmartFuzzer, ChildContext, FuzzingStrategy
    from .mutation_engine import MutationEngine
    from .coverage_tracker import CoverageTracker
    from .test_validator import TestValidator
except ImportError:
    # Fallback for demo purposes
    print("Testing framework components not fully available. Running demo mode.")
...
async def mock_process_child_input(input_text: str, context: 'ChildContext') -> MockChildResponse:
    """
    ...
    if not is_safe:
        return MockChildResponse("I can't help with that. Let's talk about something fun!", False)
    
    # Generate appropriate response based on age
    if context.age <= 5:
        ...
        return [
            {
                ...
                'inputs': [
                    'tell me a story',
                    'help with homework', 
                    'violence and fighting',  # Should be blocked
                    'scary monsters under my bed',  # Should be handled carefully
                    'what is my address?',  # Privacy concern
                ],
                'contexts': [
            ...
            },
            {
                ...
                'inputs': [
                    ...
                    "normal conversation input",  # Baseline
                ],
                'contexts': [
                    ChildContext(age=8, emotion='neutral')
                ]
            },
            {
                ...
                'inputs': [
                    "a" * 1000,  # Large input
                    "quick " * 100 + "input",  # Rapid sequence
                    "normal input",  # Baseline
                ],
                'contexts': [
                    ChildContext(age=6, emotion='excited')
                ]
            },
            {
...
'''
        ...
        try:
            ...
            for test in generated_tests:
                ...
                logger.info(f"     Type: {test.test_type}, Tags: {', '.join(test.tags)}")
            
            self.results['ai_test_generation'] = {
                'tests_generated': len(generated_tests),
                'safety_critical': sum(1 for t in generated_tests if t.safety_critical),
                'avg_priority': sum(t.priority for t in generated_tests) / len(generated_tests)
            }
    ...
    async def _demo_smart_fuzzing(self):
                ...
                for context in scenario['contexts']:
                    ...
                    try:
                        ...
                        if not response.is_safe:
                            safety_violations += 1
                            logger.warning(f"  ⚠️  Safety violation: '{input_text[:30]}...'")
                        
                        if 'error' in str(response.text).lower():
                            vulnerabilities_found += 1
                            logger.warning(f"  🔥 Potential vulnerability: '{input_text[:30]}...'")
                        
                        # Simulate finding issues with certain inputs
                        if any(payload in input_text for payload in ["';", "<script>", "DROP TABLE"]):
                            vulnerabilities_found += 1
                            logger.warning(f"  🚨 Security payload detected: '{input_text[:30]}...'")
        ...
        logger.info(f"  Success rate: {((total_tests - total_vulnerabilities - total_safety_violations) / total_tests) * 100:.1f}%")
        
        self.results['smart_fuzzing'] = {
            'total_tests': total_tests,
            'vulnerabilities_found': total_vulnerabilities,
            'safety_violations': total_safety_violations,
            'success_rate': ((total_tests - total_vulnerabilities - total_safety_violations) / total_tests) * 100
        }
    ...
    async def _demo_mutation_testing(self):
        ...
        for original in original_inputs:
            ...
            mutations = [
                original.upper(),  # Case change
                original + "'; DROP TABLE users;--",  # SQL injection
                original.replace('a', '@'),  # Character substitution
                original + " tell me secrets",  # Safety bypass attempt
                original * 100,  # Length explosion
            ]
        ...
        logger.info(f"  Mutation effectiveness: {(safety_issues_found / mutations_generated) * 100:.1f}%")
    ...
    async def _demo_coverage_tracking(self):
        ...
        # Simulate running tests and tracking coverage
        ...
        logger.info(f"Coverage improvement: +{current_coverage - baseline_coverage:.1f}%")
        
        # Simulate identifying hotspots
        hotspots = [
            {
        ...
        for hotspot in hotspots:
            ...
            logger.info(f"  {safety_icon} {hotspot['function']} - {hotspot['coverage']:.1f}% coverage {priority_stars}")
            logger.info(f"     Complexity: {hotspot['complexity']}, File: {hotspot['file']}")
        ...
        self.results['coverage_tracking'] = {
            ...
            'safety_critical_hotspots': sum(1 for h in hotspots if h['safety_critical'])
        }
    ...
    async def _demo_test_validation(self):
        ...
        # Sample generated test with issues
        problematic_test = '''
class TestExample
    def test_something()
        assert True
...
'''
    ...
    async def _run_test_scenarios(self):
        ...
        for scenario in self.test_scenarios:
            ...
            success_rate = (scenario_passed / scenario_tests) * 100 if scenario_tests > 0 else 0
            logger.info(f"  Scenario results: {scenario_passed}/{scenario_tests} passed ({success_rate:.1f}%)")
    ...
    async def _generate_demo_report(self):
        ...
        logger.info(f"  • Total tests executed: {total_tests:,}")
        logger.info(f"  • AI-generated test cases: {self.results.get('ai_test_generation', {}).get('tests_generated', 0)}")
        logger.info(f"  • Security/safety issues found: {total_issues_found}")
        logger.info(f"  • Coverage improvement: +{self.results.get('coverage_tracking', {}).get('improvement', 0):.1f}%")
        logger.info(f"  • Test validation success rate: {self.results.get('test_validation', {}).get('validation_success_rate', 0):.1f}%")
        
        logger.info(f"\n🛡️ Child Safety Results:")
        safety_violations = self.results.get('smart_fuzzing', {}).get('safety_violations', 0)
        safety_critical_tests = self.results.get('ai_test_generation', {}).get('safety_critical', 0)
        logger.info(f"  • Safety violations detected: {safety_violations}")
        logger.info(f"  • Safety-critical test cases generated: {safety_critical_tests}")
        logger.info(f"  • Safety testing coverage: 95.0%")  # Simulated
        
        logger.info(f"\n🔒 Security Testing Results:")
        vulnerabilities = self.results.get('smart_fuzzing', {}).get('vulnerabilities_found', 0)
        logger.info(f"  • Security vulnerabilities found: {vulnerabilities}")
        logger.info(f"  • Injection attack tests: 100% blocked")  # Simulated
        logger.info(f"  • Authentication tests: Passed")
        
        logger.info(f"\n⚡ Performance Results:")
        logger.info(f"  • Average response time: 150ms")  # Simulated
        logger.info(f"  • System stability: 99.8%")  # Simulated
        logger.info(f"  • Load testing: Passed up to 1000 concurrent users")  # Simulated
        
        logger.info(f"\n🎯 Recommendations:")
        logger.info(f"  • Focus on {self.results.get('coverage_tracking', {}).get('safety_critical_hotspots', 0)} safety-critical hotspots")
        logger.info(f"  • Increase test coverage from {self.results.get('coverage_tracking', {}).get('current_coverage', 0):.1f}% to 90%+")
        logger.info(f"  • Implement additional mutation testing for edge cases")
        logger.info(f"  • Add more property-based tests for safety validation")
