...
class ServiceHealthMonitor:
    ...
    async def _check_service_health(self, service_name: str, instance: ServiceInstance):
        ...
        try:
            ...
            health_check_result = await asyncio.wait_for(
                instance.registration.health_check(),
    ...
    async def _attempt_service_recovery(self, service_name: str, instance: ServiceInstance):
        ...
        try:
            ...
            logger.error("Service recovery failed", service=service_name, error=str(e))
            instance.status = ServiceStatus.UNHEALTHY
    ...
    def get_health_status(self, service_name: str) -> Dict[str, Any]:
        ...
        if service_name in self.container._instances:
            ...
            success_rate = (sum(history) / len(history) * 100) if history else 0
...
class AsyncServiceResolver:
    ...
    async def resolve(self, service_name: str, context: Optional[Dict[str, Any]] = None) -> Any:
    ...
    async def _resolve_service(self, service_name: str, context: Optional[Dict[str, Any]]) -> Any:
...
class EnterpriseContainer:
    ...
    async def get_service(self, service_name_or_type: Union[str, Type[T]], context: Optional[Dict[str, Any]] = None) -> T:
    ...
    async def _create_service_instance(self, service_name: str, registration: ServiceRegistration) -> Any:
    ...
    logger.error(f"Error in operation: {e}", exc_info=True)                        # Parameter not available, use default if provided
            ...
            logger.error("Service creation failed", service=service_name, error=str(e))
            raise
    ...
    async def health_check(self, service_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        healthy_services = sum(1 for status in health_statuses.values() 
    ...
    async def _dispose_service_instance(self, service_name: str, instance: ServiceInstance):
        ...
        try:
            ...
            logger.error("Service disposal error", service=service_name, error=str(e))
