...
class AsyncDatabaseOptimizer:
    ...
    async def get_read_replica_session(
        ...
        replica = self._select_replica(replica_name)
    ...
    async def execute_optimized_query(
        ...
        if use_cache:
            cached_data = self._check_cache(cache_key, cache_ttl)
        ...
        try:
            async with self.get_primary_session() as session:
                ...
                DB_QUERY_DURATION.labels(
                    operation="select",
                    table="unknown").observe(duration)
                DB_QUERY_TOTAL.labels(
                    operation="select", status="success").inc()
    ...
    async def _check_replica_health(self, replica: ReadReplica):
        """Checks the health and replication lag of a single read replica."""
        if replica.engine:
            try:
                async with replica.engine.begin() as conn:
...
async def create_database_optimizer(
        config: Dict[str, Any]) -> AsyncDatabaseOptimizer:
