...
class KafkaTopics:
    ...
    # Child Events Topics
    CHILD_REGISTERED = TopicConfig(
        name="child.registered",
        partitions=3,
        retention_ms=2592000000  # 30 days
    )
    
    CHILD_PROFILE_UPDATED = TopicConfig(
        name="child.profile-updated",
        partitions=3,
        retention_ms=2592000000  # 30 days
    )
    ...
    CHILD_SAFETY_VIOLATION = TopicConfig(
        ...
        partitions=3,
        retention_ms=7776000000  # 90 days - important for compliance
    )
    
    CHILD_MILESTONE_ACHIEVED = TopicConfig(
        name="child.milestone-achieved",
        partitions=3,
        retention_ms=31536000000  # 365 days - long-term tracking
    )
    ...
    CONVERSATION_ENDED = TopicConfig(
    ...
    CONVERSATION_ESCALATED = TopicConfig(
        ...
        partitions=3,
        retention_ms=7776000000  # 90 days - critical events
    )
    
    MESSAGE_RECEIVED = TopicConfig(
        name="message.received",
        partitions=6,  # High volume expected
        retention_ms=604800000  # 7 days
    )
    
    RESPONSE_GENERATED = TopicConfig(
    ...
    EMOTION_DETECTED = TopicConfig(
        name="emotion.detected",
        partitions=3,
        retention_ms=2592000000  # 30 days for analysis
    )
    
    # Analytics Topics
    USAGE_STATS = TopicConfig(
        name="analytics.usage-stats",
        partitions=3,
    ...
    # Dead Letter Queue
    DLQ_FAILED_EVENTS = TopicConfig(
        name="dlq.failed-events",
        partitions=1,
        retention_ms=2592000000  # 30 days
    )
...
class KafkaMonitoringConfig:
    ...
    def get_jmx_opts(self) -> str:
        ...
        return (f"-Dcom.sun.management.jmxremote "
