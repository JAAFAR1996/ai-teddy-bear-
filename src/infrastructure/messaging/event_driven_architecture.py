...
class IEventStore(ABC):
    ...
    async def read(
            self,
            stream_name: str,
            from_position: int = 0) -> List[Event]:
...
class RedisEventStore(IEventStore):
    ...
    def _redis_data_to_event(self, data: Dict[str, bytes]) -> Event:
        ...
        metadata = EventMetadata(
            ...
            causation_id=(
                data[b"causation_id"].decode() if data[b"causation_id"] else None
            ),
...
class RedisEventBus(IEventBus):
    ...
    async def publish(self, event: Event) -> None:
        ...
        logger.info(
            f"📢 Published event {event.metadata.event_id} to {channel}")
...
class SagaManager:
    ...
    async def start_saga(self, saga_id: str,
...
class EventSourcingManager:
    ...
    async def save_events(
        ...
        if len(current_events) + \
                len(events) >= 100:  # Snapshot every 100 events
    ...
    async def load_aggregate(
        ...
        if aggregate_id in self.snapshots:
            ...
            events = await self.event_store.read(
                f"aggregate:{aggregate_id}", snapshot["version"]
            )
            for event in events:
...
class DeadLetterQueue:
    ...
    async def add_failed_event(self, event: Event, error: Exception) -> None:
        ...
        logger.warning(
            f"⚠️ Event {event.metadata.event_id} moved to DLQ: {error}")
...
def create_event_store(
        redis_url: str = "redis://localhost:6379") -> IEventStore:
...
def create_command(
    data: Dict[str, Any], user_id: Optional[str] = None, **kwargs
) -> Command:
    """Create command"""
...
def create_query(
    parameters: Dict[str, Any], user_id: Optional[str] = None, **kwargs
) -> Query:
    """Create query"""
