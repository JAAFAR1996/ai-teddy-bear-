...
class CloudServerLauncher:
    ...
    def create_control_panel(self, parent) -> Any:
        ...
        self._create_quick_action_buttons(control_frame)
    ...
    def create_api_tab(self, notebook) -> Any:
        ...
        endpoints_frame = tk.LabelFrame(
            api_frame, text="🎯 API Endpoints Status", font=(
                "Arial", 12, "bold"))
    ...
    def update_system_stats(self) -> Any:
        ...
        try:
            ...
            self.root.after(
                0, lambda: self.cpu_percent_label.config(
                    text=f"{cpu_percent:.1f}"))

            # Memory Usage
            memory = psutil.virtual_memory()
            self.root.after(
                0, lambda: self.memory_percent_label.config(
            ...
            self.root.after(
                0, lambda: self.disk_percent_label.config(
...
"""
    ...
    def run(self) -> Any:
        ...
        try:
            ...
            print("👋 Cloud Server Launcher shutting down...")
        finally:
            if self.server_running:
                self.stop_server()
