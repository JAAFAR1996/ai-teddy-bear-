...
from .event_store import EventStore, get_event_store, StoredEvent
from .snapshot_store import SnapshotStore, get_snapshot_store, Snapshot
...
class EventSourcingRepositoryImpl(EventSourcingRepository):
    ...
    async def _load_from_snapshot(self, stream_id: str) -> Optional[T]:
        ...
        events = await self.event_store.load_events(
            stream_id=stream_id,
            from_version=snapshot.metadata.version + 1
        )
