...
class HumeSpeechEmotionAnalyzer:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        logger.error(
            f"   API Status: {'✅ Ready' if self.api_key else '❌ No API Key'}"
        )
    ...
    async def analyze_child_voice(
        ...
        try:
            if not self.api_key:
                logger.warning(
                    "⚠️ No HUME API key provided, using fallback analysis")
    ...
    async def _analyze_with_hume(self, audio_path: str) -> Dict[str, Any]:
        ...
        if HUME_AVAILABLE:
            try:
                ...
                logger.error(f"⚠️ HUME SDK error, trying direct API: {e}")

        # استخدام API مباشر كـ fallback
        return await self._analyze_with_direct_api(audio_path)

    async def _analyze_with_direct_api(
            self, audio_path: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"{self.base_url}/batch/jobs", headers=headers, data=files
            ) as response:
                if response.status == 200:
                    ...
                    raise Exception(
                        f"HUME API error {response.status}: {error_text}")

    async def _wait_for_results(
        self, session: aiohttp.ClientSession, job_id: str, headers: Dict[str, str]
    ) -> Dict[str, Any]:
        ...
        while attempt < max_attempts:
            async with session.get(
                f"{self.base_url}/batch/jobs/{job_id}", headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def _download_results(
        ...
        async with session.get(
            f"{self.base_url}/batch/jobs/{job_id}/predictions", headers=headers
        ) as response:
            if response.status == 200:
                return await response.json()
            else:
                raise Exception(
                    f"❌ Results download failed: {response.status}")

    def _process_hume_predictions(
    ...
    async def _convert_to_child_emotion(
        ...
        try:
            self._process_hume_predictions(hume_results, child_emotion)

            child_emotion.dominant_emotion = self._get_dominant_emotion(
                child_emotion)
            child_emotion.confidence = self._calculate_confidence(
                child_emotion)
            child_emotion.emotional_intensity = self._calculate_intensity(
                child_emotion)
            ...
            logger.error(f"⚠️ Error converting HUME results: {e}")
    ...
    async def _add_developmental_analysis(
        self, emotion: ChildVoiceEmotion, audio_data: bytes, child_age: int
    ) -> ChildVoiceEmotion:
        ...
        emotion.pitch_variation = voice_characteristics.get(
            "pitch_variation", 0.5)
        emotion.voice_quality = voice_characteristics.get("quality", "clear")

        # تحليل مستوى الانتباه
        emotion.attention_level = self._calculate_attention_level(
            emotion, voice_characteristics
        )
    ...
    def _adjust_for_age(
        self, emotion: ChildVoiceEmotion, age: int
    ) -> ChildVoiceEmotion:
        ...
        age_group = (
            ...
            if age <= 4
                ...
                "5-6"
                if age <= 6
                else "7-8" if age <= 8 else "9-10" if age <= 10 else "11+"
            )
        )
    ...
    def _apply_context(
        self, emotion: ChildVoiceEmotion, context: Dict[str, Any]
    ) -> ChildVoiceEmotion:
    ...
    def _calculate_attention_level(
        self, emotion: ChildVoiceEmotion, voice_characteristics: Dict[str, Any]
    ) -> float:
        """حساب مستوى الانتباه"""
        ...
        clarity_score = 1.0 if voice_characteristics.get(
            "quality") == "clear" else 0.6
        energy_score = voice_characteristics.get("energy", 0.5)
        stability_score = 1.0 - \
            voice_characteristics.get("pitch_variation", 0.5)

        # الفضول يشير إلى انتباه
        curiosity_bonus = emotion.curiosity * 0.3

        attention = (
            clarity_score + energy_score + stability_score + curiosity_bonus
        ) / 4.0
        return min(attention, 1.0)

    def _calculate_communication_clarity(
        self, voice_characteristics: Dict[str, Any], child_age: int
    ) -> float:
        """حساب وضوح التواصل"""
        ...
        quality_score = 1.0 if voice_characteristics.get(
            "quality") == "clear" else 0.7
    ...
    def _get_positive_indicators(
            self, emotion: ChildVoiceEmotion) -> List[str]:
        """Generates positive developmental indicators."""
        indicators = []
    ...
    def _get_attention_indicators(
            self, emotion: ChildVoiceEmotion) -> List[str]:
        """Generates indicators that may need attention."""
        indicators = []
    ...
    def _get_age_specific_indicators(
        self, emotion: ChildVoiceEmotion, child_age: int
    ) -> List[str]:
        """Generates age-specific developmental indicators."""
        indicators = []
    ...
    def _generate_developmental_indicators(
        ...
        positive_indicators = self._get_positive_indicators(emotion)
        attention_indicators = self._get_attention_indicators(emotion)
        age_specific_indicators = self._get_age_specific_indicators(
            emotion, child_age)

        return positive_indicators + attention_indicators + age_specific_indicators
    ...
    async def get_child_emotion_summary(
        self, emotions_history: List[ChildVoiceEmotion]
    ) -> Dict[str, Any]:
        ...
        # المشاعر المهيمنة
        dominant_emotions = [
            emotion.dominant_emotion for emotion in emotions_history]
        ...
        return {
            ...
            "recommendations": self._generate_recommendations(
                avg_emotions, most_common
            ),
        }

    def _calculate_stability(
            self,
            emotions_history: List[ChildVoiceEmotion]) -> float:
    ...
    def _generate_recommendations(
        self, avg_emotions: Dict[str, float], most_common: str
    ) -> List[str]:
