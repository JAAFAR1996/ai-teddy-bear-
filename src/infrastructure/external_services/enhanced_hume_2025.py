...
class EnhancedHumeIntegration:
    ...
    return {
        ...
        return {'error': 'No historical data found', 'sessions': 0}
            ...
            daily_summaries[date_key]['dominant_emotions'].append(
        ...
        for day_data in daily_summaries.values():
            if day_data['emotions']:
                day_dominant = max(
                    day_data['emotions'], key=day_data['emotions'].get)
        ...
        if daily_dominant_emotions:
            ...
            stability_score = daily_dominant_emotions.count(
                most_common_daily) / len(daily_dominant_emotions)
        else:
            stability_score = 0.5
