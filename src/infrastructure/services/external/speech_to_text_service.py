    ...
    def __init__(self, db_path, model_size='medium'):
        ...
        async def transcribe_audio(
            ...
            if not self.whisper_model:
                self.logger.error(
                    "Whisper model not loaded, cannot transcribe.")
                return None

            self.logger.info(
                f"Starting transcription for file: {audio_file_path}")
            try:
                result = self.whisper_model.transcribe(
                    audio_file_path, language="ar")
    ...
    async def transcribe_directory(self, directory_path: str, user_id=None, conversation_id=None, language=None):
        ...
        for filename in os.listdir(directory_path):
            if filename.endswith(('.wav', '.mp3', '.ogg', '.flac')):
                ...
                text = await self.transcribe_audio(full_path, user_id=user_id, conversation_id=conversation_id, language=language)
