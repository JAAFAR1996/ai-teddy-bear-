...
class DependencyOptions:
    """Optional parameters for dependency registration."""
...
class LifecycleManager:
    ...
    async def _cleanup_scope(self, scope_instances: Dict[str, Any]):
        """Helper to cleanup instances in a specific scope."""
    ...
    async def cleanup(self):
        ...
        with self._lock:
            await self._cleanup_scope(self.singletons)
            await self._cleanup_scope(self.request_scoped)
            await self._cleanup_scope(self.session_scoped)
...
class EnterpriseContainer(containers.DeclarativeContainer):
    ...
    def register(
        ...
        with self._lock:
            ...
            metadata = DependencyMetadata(
                ...
                cleanup_method=options.cleanup_method,
            ...
            self.dependency_graph.add_dependency(
                interface_name, metadata.dependencies, metadata
            )

            # Create provider
            if options.factory:
                provider = AsyncProvider(options.factory)
            else:
            ...
            logger.info(
                f"📦 Registered {interface_name} with scope {scope.value}")

    def register_singleton(
        self, interface: Type[T], implementation: Type[T], **kwargs
    ) -> None:
        ...
        self.register(
            ...
            options=DependencyOptions(**kwargs),
        )
            ...
            for interface_name, metadata in self.registrations.items():
                if metadata.scope == LifecycleScope.SINGLETON:
                    try:
                        ...
                        logger.error(
                            f"❌ Failed to initialize {interface_name}: {e}")
    ...
    async def _initialize_dependency(
        self, interface_name: str, metadata: DependencyMetadata
    ):
        ...
        if metadata.async_init:
            ...
            if metadata.factory:
                ...
                if hasattr(
                        instance,
                        "initialize") and asyncio.iscoroutinefunction(
                        instance.initialize):
    ...
    async def get(self, interface: Type[T]) -> T:
        ...
        if interface_name not in self.registrations:
            raise DependencyNotFoundError(
                f"Dependency {interface_name} not registered")
        ...
        for param_name, param in sig.parameters.items():
            ...
            if param_type != inspect.Parameter.empty:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Could not resolve dependency {param_name}: {e}")
        ...
        for interface_name, metadata in self.registrations.items():
            if tag in metadata.tags:
                try:
                    ...
                    logger.warning(
                        f"⚠️ Could not get {interface_name} with tag {tag}: {e}"
                    )
        ...
        for interface_name, metadata in self.registrations.items():
            try:
                ...
                if metadata.health_check:
                    health_status[interface_name] = metadata.health_check(
                        instance)
                ...
                logger.error(
                    f"❌ Health check failed for {interface_name}: {e}")
    ...
    async def shutdown(self) -> None:
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(
                    f"✅ Container '{self.name}' shut down successfully")
            except Exception as e:
...
def register_services(
    container: EnterpriseContainer, services: Dict[Type, Type]
) -> None:
