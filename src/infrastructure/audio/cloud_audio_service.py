...
class CloudAudioService:
    ...
    async def _save_temp_audio(self, audio_data: bytes, device_id: str) -> Path:
    ...
    async def _generate_ai_response(self, message: str, device_id: str) -> Dict[str, str]:
    ...
    async def _generate_fallback_response(self, message: str) -> Dict[str, str]:
        """Generate fallback response when AI APIs are unavailable"""
        message_lower = message.lower()
        
        if any(word in message_lower for word in ['hello', 'hi', 'مرحبا']):
            text = "Hello my little friend! I'm so happy to talk with you today! 🧸💕"
        elif any(word in message_lower for word in ['play', 'game', 'العب']):
            text = "Let's play together! Would you like me to tell you a story or sing a song? 🎵"
        elif any(word in message_lower for word in ['story', 'قصة']):
            text = "Once upon a time, there was a magical teddy bear who loved to make children smile... ✨"
        elif any(word in message_lower for word in ['sad', 'upset', 'حزين']):
            text = "Aww, don't be sad little one. I'm here for you always. Let's think of happy things! 🌈"
        else:
    ...
    async def _generate_speech(self, text: str) -> Optional[str]:
        ...
        try:
            if ELEVENLABS_AVAILABLE and os.getenv('ELEVENLABS_API_KEY'):
                ...
                logger.warning("No TTS service available - returning text only")
                return None
    ...
    async def get_device_audio_status(self, device_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'audio_services': {
                ...
                'tts': 'elevenlabs' if ELEVENLABS_AVAILABLE else 'openai' if OPENAI_AVAILABLE else 'text_only',
