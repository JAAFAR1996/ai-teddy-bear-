...
class ILoadBalancer(ABC):
    ...
    @abstractmethod
    async def select_instance(self, service_name: str, strategy: LoadBalancingStrategy) -> Optional[ServiceInstance]:
...
class KubernetesServiceDiscovery(IServiceDiscovery):
    ...
    async def discover_services(self, service_name: str) -> List[ServiceInstance]:
        ...
        try:
            # Get service
            service = self.v1.read_namespaced_service(service_name, self.namespace)
            
            # Get endpoints
            endpoints = self.v1.read_namespaced_endpoints(service_name, self.namespace)
...
class LoadBalancer(ILoadBalancer):
    ...
    async def select_instance(self, service_name: str, strategy: LoadBalancingStrategy) -> Optional[ServiceInstance]:
        ...
        healthy_instances = [i for i in service.instances if i.status == ServiceStatus.HEALTHY]
    ...
    async def update_instance_status(self, instance_id: str, status: ServiceStatus) -> None:
        ...
        self.instance_states[instance_id]["last_updated"] = datetime.now(timezone.utc)
    ...
    def _round_robin(self, service_name: str, instances: List[ServiceInstance]) -> ServiceInstance:
        ...
        instance = instances[self.round_robin_indexes[service_name] % len(instances)]
    ...
    def _weighted_round_robin(self, service_name: str, instances: List[ServiceInstance]) -> ServiceInstance:
        ...
        instance = weighted_instances[self.round_robin_indexes[service_name] % len(weighted_instances)]
        self.round_robin_indexes[service_name] += 1
    ...
    async def _get_service_instances(self, service_name: str) -> Optional[ServiceDefinition]:
        ...
        async def monitor_health():
            while True:
                try:
                    ...
                    logger.error(f"❌ Health monitoring error for {service.name}: {e}")
                    await asyncio.sleep(service.health_check_interval)
...
class ServiceOrchestrator:
    ...
    async def get_service_health(self, service_name: str) -> Dict[str, Any]:
        ...
        health_status = {
            ...
            "healthy_instances": len([i for i in service.instances if i.status == ServiceStatus.HEALTHY]),
            "unhealthy_instances": len([i for i in service.instances if i.status == ServiceStatus.UNHEALTHY]),
...
# Factory functions
def create_consul_registry(consul_host: str = "localhost", consul_port: int = 8500) -> IServiceRegistry:
...
def create_service_orchestrator(
    ...
    return ServiceOrchestrator(registry, discovery, load_balancer, health_checker)
