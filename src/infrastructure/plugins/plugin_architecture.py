...
class PluginSandbox:
    ...
    def secure_exec(self, code: str, globals_dict: Dict[str, Any], locals_dict: Dict[str, Any]):
        """
        Secure code execution using AST parsing to block dangerous operations.
        WARNING: Never use with untrusted user input. Only safe, validated code should be executed.
        """
        # Parse code and block dangerous nodes
        dangerous_nodes = (ast.Import, ast.ImportFrom, ast.Exec, ast.Call)
        dangerous_names = {'eval', 'exec', 'open',
                           'file', 'system', 'subprocess', '__import__'}
        tree = ast.parse(code, mode='exec')
        for node in ast.walk(tree):
            if isinstance(node, dangerous_nodes):
                raise SecurityError(
                    f"Blocked dangerous AST node: {type(node).__name__}")
            if isinstance(node, ast.Call):
                ...
                if isinstance(node.func, ast.Attribute) and node.func.attr in dangerous_names:
                    raise SecurityError(
                        f"Blocked dangerous attribute call: {node.func.attr}")
        ...
        exec(compile(tree, filename="<secure_exec>", mode="exec"),
             restricted_globals, locals_dict)
