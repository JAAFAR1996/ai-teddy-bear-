...
from .safe_expression_parser import (
    create_safe_parser,
    create_safe_template_engine,
    safe_eval,
    SecurityLevel
)
...
from ..config.secure_configuration import (
...
class SecureApplicationService:
    ...
    async def initialize(self, environment: str = "development"):
        ...
        security_level = (
            SecurityLevel.STRICT if environment == "production"
    ...
    def _setup_exception_strategies(self):
        ...
        global_exception_handler.register_strategy(
            ChildSafetyException,
            FallbackStrategy(safety_fallback)
        )
    ...
    async def process_child_request(
        ...
        with CorrelationContext() as correlation_id:
            ...
            ai_key = await self.secrets_manager.get_secret(
                "openai_api_key",
            ...
            safe_response = await self._apply_safety_filters(
                response,
                safety_config
            )
