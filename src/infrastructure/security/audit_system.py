    ...
    async def _count_recent_events(self, identifier: str, 
        ...
        for event in self.event_buffer:
            if (event.timestamp >= cutoff_time and 
                ...
                (event.user_id == identifier or event.ip_address == identifier)):
                count += 1
...
async def log_child_interaction(user_id: str, device_id: str, 
...
async def log_audio_recording(user_id: str, device_id: str, 
...
async def log_login_attempt(user_id: str, username: str, ip_address: str,
                           result: str, **kwargs) -> str:
    """Log login attempt"""
    audit_logger = get_audit_logger()
    
    event_type = (SecurityEventType.LOGIN_SUCCESS if result == "success" 
