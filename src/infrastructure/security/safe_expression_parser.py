...
class SafeExpressionParser:
    ...
    SAFE_STRING_METHODS = {
        ...
        'isalnum', 'isspace', 'capitalize', 'title'
    }
...
class ASTValidator:
    ...
    def _validate_name(self, node: ast.Name):
        ...
        if node.id not in self.allowed_functions and \
           node.id not in self.context.variables and \
           node.id not in self.context.allowed_names:
