...
class SafeExpressionConfig(BaseModel):
    ...
    default_security_level: SecurityLevel = Field(default=SecurityLevel.RESTRICTED)
    max_complexity_score: int = Field(default=10, description="Maximum AST complexity")
    evaluation_timeout: float = Field(default=5.0, description="Evaluation timeout in seconds")
    enable_json_logic: bool = Field(default=True, description="Enable JSONLogic support")
    enable_templates: bool = Field(default=True, description="Enable template processing")
    strict_validation: bool = Field(default=True, description="Strict input validation")
...
class SecurityValidator:
    ...
    def __init__(self, config: SafeExpressionConfig):
        self.config = config
        self.forbidden_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in config.forbidden_patterns]
            ...
            if isinstance(child, ast.Call):
                ...
                if isinstance(child.func, ast.Attribute):
                    if isinstance(child.func.value, ast.Name):
                        if child.func.value.id == "__builtins__":
    ...
    def evaluate(self, expression: str, context: Optional[ExpressionContext] = None) -> Any:
        ...
        try:
            ...
            if not self.security_validator.validate_expression(expression):
                raise ExpressionSecurityError("Expression contains security risks")
            ...
            # Evaluate
            return self._evaluate_node(tree.body, context or ExpressionContext())
            
        except SyntaxError as e:
            raise ExpressionValidationError(f"Invalid syntax: {e}")
        except Exception as e:
            if isinstance(e, (ExpressionSecurityError, ExpressionValidationError)):
                raise
            raise ExpressionValidationError(f"Evaluation error: {e}")
    ...
    def _evaluate_node(self, node: ast.AST, context: ExpressionContext) -> Any:
        ...
        if isinstance(node, ast.Constant):
            ...
            if node.id in context.variables:
                ...
                raise ExpressionValidationError(f"Undefined variable: {node.id}")
            ...
            if type(node.op) in self.operators:
                ...
                raise ExpressionSecurityError(f"Unsafe operator: {type(node.op).__name__}")
            ...
            if type(node.op) in self.operators:
                ...
                raise ExpressionSecurityError(f"Unsafe unary operator: {type(node.op).__name__}")
            ...
            for op, comparator in zip(node.ops, node.comparators):
                ...
                if type(op) in self.operators:
                    ...
                    raise ExpressionSecurityError(f"Unsafe comparison operator: {type(op).__name__}")
            ...
            if isinstance(node.op, ast.And):
                ...
                raise ExpressionSecurityError(f"Unsafe boolean operator: {type(node.op).__name__}")
            ...
            if isinstance(node.func, ast.Name):
                ...
                if func_name in self.safe_functions:
                    ...
                    kwargs = {
                        kw.arg: self._evaluate_node(kw.value, context)
                    ...
                    return context.functions[func_name](*args, **kwargs)
                else:
                    raise ExpressionSecurityError(f"Unsafe function call: {func_name}")
            else:
                raise ExpressionSecurityError("Complex function calls not allowed")
        
        elif isinstance(node, ast.List):
            return [self._evaluate_node(elt, context) for elt in node.elts]
        
        elif isinstance(node, ast.Dict):
            keys = [self._evaluate_node(key, context) for key in node.keys]
            values = [self._evaluate_node(value, context) for value in node.values]
            ...
            raise ExpressionSecurityError(f"Unsafe node type: {type(node).__name__}")
...
class TemplateParser(IExpressionParser):
    ...
    def validate(self, expression: str) -> bool:
        ...
        for placeholder in placeholders:
            ...
            if not self.ast_evaluator.security_validator.validate_expression(expr):
                return False
        
        return True
    
    def evaluate(self, expression: str, context: Optional[ExpressionContext] = None) -> str:
...
def safe_json_logic(logic: str, variables: Optional[Dict[str, Any]] = None) -> Any:
