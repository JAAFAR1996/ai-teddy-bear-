...
class AgeAppropriatenessChecker:
    ...
    def _analyze_concept_appropriateness(self, content: str, age: int) -> float:
        ...
        inappropriate_concepts = [
            'موت', 'مرض خطير', 'حادث', 'طلاق',
            'فقر', 'حروب', 'سياسة', 'اقتصاد معقد'
        ]
        
        inappropriate_count = sum(1 for concept in inappropriate_concepts 
    ...
    def _analyze_emotional_complexity(self, content: str, age: int) -> float:
        ...
        complex_emotions = [
            'اكتئاب', 'قلق شديد', 'خوف مرضي', 'غضب شديد',
            'يأس', 'إحباط عميق', 'صدمة نفسية'
        ]
        
        complex_emotion_count = sum(1 for emotion in complex_emotions 
...
class AdvancedContentFilter:
    ...
    def _calculate_overall_risk(
        ...
        risk_levels = [RiskLevel.SAFE, RiskLevel.LOW_RISK, RiskLevel.MEDIUM_RISK, 
