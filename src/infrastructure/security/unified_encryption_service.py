...
class UnifiedEncryptionService:
    ...
    def __init__(
        self, master_key: Optional[str] = None, config: Optional[Dict[str, Any]] = None
    ):
    ...
    async def decrypt(self, encrypted_data: EncryptedData) -> bytes:
        ...
        if encrypted_data.algorithm == "fernet":
            ...
            raise ValueError(
                f"Unknown encryption algorithm: {encrypted_data.algorithm}"
            )

    async def encrypt_child_data(
            self, child_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        result = {
            k: v for k,
            v in child_data.items() if k not in sensitive_fields}
        result.update(encrypted_fields)

        return result

    async def decrypt_child_data(
        self, encrypted_child_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for key, value in encrypted_child_data.items():
            if key.startswith("encrypted_"):
                ...
                try:
                    decrypted_data[field_name] = json.loads(
    ...
    async def _encrypt_aes_gcm_with_rsa(
    ...
    async def _decrypt_fernet(self, encrypted_data: EncryptedData) -> bytes:
        ...
        key = self._generate_data_key(
            encrypted_data.key_id, encrypted_data.encryption_level
        )
        f = Fernet(key)
    ...
    async def _decrypt_aes_gcm_with_rsa(
            self, encrypted_data: EncryptedData) -> bytes:
    ...
    def rotate_keys(self) -> None:
        ...
        expired_keys = [
            key_id
            for key_id, (_, created_at) in self._key_cache.items()
...
def get_encryption_service(
    master_key: Optional[str] = None, config: Optional[Dict[str, Any]] = None
