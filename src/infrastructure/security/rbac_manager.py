...
class TeddyBearRBACManager:
    ...
    def _get_role_permission_config(self) -> Dict[UserRole, Set[Permission]]:
        """Returns the configuration for role permissions."""
        return {
            ...
            UserRole.GUEST: {Permission.CHILD_INTERACT},
        }

    def _initialize_role_permissions(self) -> Any:
        """Initialize default permissions for each role"""
        self.role_permissions = self._get_role_permission_config()
    ...
    async def _check_user_status(
        ...
        if not user:
            return AccessDecision(
                ...
                audit_log_id=await self._log_access_attempt(
        ...
        if not user.is_active:
            return AccessDecision(
                ...
                audit_log_id=await self._log_access_attempt(
                    request, False, "Inactive user"
                ),
            )
        return None

    async def _check_basic_permissions(
        self, user: UserProfile, request: AccessRequest
    ) -> Optional[AccessDecision]:
        """Checks basic role permissions."""
        if request.permission not in user.permissions:
            return AccessDecision(
                granted=False,
                reason="Permission not granted to user role",
                audit_log_id=await self._log_access_attempt(
                    request, False, "Permission denied"
                ),
            )
        return None
    ...
    async def check_access(self, request: AccessRequest) -> AccessDecision:
        ...
        user_status_decision = await self._check_user_status(request)
        if user_status_decision:
            return user_status_decision

        user = self.users.get(request.user_id)

        permission_decision = await self._check_basic_permissions(user, request)
        ...
        decision = AccessDecision(
            ...
            + resource_check.conditions
            + time_check.conditions,
            expires_at=self._calculate_access_expiry(user, request),
            audit_log_id=await self._log_access_attempt(
                request, True, "Access granted"
            ),
        )

        cache_key = (
            f"{request.user_id}:{request.permission.value}:{request.resource_id}"
        )
        self.access_cache[cache_key] = decision

        return decision

    def _get_context_handlers(self) -> Dict[AccessContext, Callable]:
        """Returns a dictionary of handlers for different access contexts."""
        return {
            AccessContext.DIRECT_INTERACTION: self._handle_direct_interaction,
            AccessContext.PARENTAL_SUPERVISION: self._handle_parental_supervision,
            AccessContext.EMERGENCY_OVERRIDE: self._handle_emergency_override,
            AccessContext.THERAPEUTIC_SESSION: self._handle_therapeutic_session,
            AccessContext.EDUCATIONAL_ACTIVITY: self._handle_educational_activity,
        }
    ...
    async def _check_context_permissions(
        ...
        handler = self._get_context_handlers().get(request.context)
        if handler:
    ...
    async def _handle_direct_interaction(
        ...
        if user.role == UserRole.CHILD:
            return AccessDecision(
                granted=True, reason="Child direct interaction allowed"
            )
        if user.role == UserRole.PARENT:
            return AccessDecision(granted=True, reason="Parent access allowed")
        return AccessDecision(
            granted=False,
            reason="Direct interaction not allowed for this role")

    async def _handle_parental_supervision(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        ...
        return AccessDecision(granted=True,
                              reason="Parental supervision allowed")

    async def _handle_emergency_override(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        if user.user_id in self.emergency_contacts.get(
                request.resource_id, []):
            return AccessDecision(
                granted=True,
        ...
        return AccessDecision(
            granted=False, reason="User not authorized for emergency override"
        )

    async def _handle_therapeutic_session(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        ...
        return AccessDecision(granted=True,
                              reason="Therapeutic session access allowed")

    async def _handle_educational_activity(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        if user.role not in [
                UserRole.EDUCATOR,
        ...
        return AccessDecision(
            granted=True, reason="Educational activity access allowed"
        )

    def _get_resource_handlers(self) -> Dict[str, Callable]:
        """Returns a dictionary of handlers for different resource types."""
        return {
            "child:": self._handle_child_resource,
            "family:": self._handle_family_resource,
        }
    ...
    async def _check_resource_access(
        ...
        if not request.resource_id:
            return AccessDecision(
                granted=True, reason="No specific resource check needed"
            )

        for prefix, handler in self._get_resource_handlers().items():
            if request.resource_id.startswith(prefix):
                return await handler(user, request)

        return AccessDecision(granted=True,
                              reason="Resource access check passed")

    async def _handle_child_resource(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        """Handles access checks for child resources."""
        child_id = request.resource_id.split(":")[1]

        if user.role == UserRole.PARENT and child_id in user.children_ids:
            return AccessDecision(
                granted=True, reason="Parent accessing own child")
        if user.role == UserRole.CHILD and child_id == user.user_id:
            return AccessDecision(
                granted=True, reason="Child accessing own data")
        if user.role == UserRole.GUARDIAN:
            return AccessDecision(
                granted=True, reason="Guardian access (needs DB verification)"
            )
    ...
    async def _handle_family_resource(
        self, user: UserProfile, request: AccessRequest
    ) -> AccessDecision:
        """Handles access checks for family resources."""
        family_id = request.resource_id.split(":")[1]
        if user.family_id == family_id:
            return AccessDecision(
                granted=True, reason="User is member of this family")
        return AccessDecision(
            granted=False, reason="User is not a member of this family"
        )
    ...
    async def _check_time_restrictions(
        ...
        if user.user_id in self.time_restrictions:
            ...
            if (
                "weekend_restricted" in restrictions
                and restrictions["weekend_restricted"]
            ):
    ...
    async def _log_access_attempt(
        self, request: AccessRequest, granted: bool, reason: str
    ) -> str:
    ...
    async def set_time_restrictions(
            self, user_id: str, restrictions: Dict[str, Any]):
    ...
    async def get_family_members(self, family_id: str) -> List[UserProfile]:
        ...
        return [user for user in self.users.values() if user.family_id ==
                family_id]
...
def require_permission(
        permission: Permission,
        context: AccessContext = AccessContext.DIRECT_INTERACTION) -> None:
...
def require_role(allowed_roles: List[UserRole]) -> None:
