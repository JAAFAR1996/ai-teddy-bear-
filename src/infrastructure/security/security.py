...
class RateLimiter:
    ...
    async def _check_redis_rate_limit(self, identifier: str, current_time: float) -> bool:
    ...
    def _check_local_rate_limit(self, identifier: str, current_time: float) -> bool:
...
def setup_security(Optional[redis.Redis] = None) -> None:
    ...
    app.add_middleware(SecurityMiddleware, config=security_config, redis_client=redis_client)
    
    logger.info("Security middleware configured", config=security_config.dict())
