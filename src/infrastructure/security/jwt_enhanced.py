...
class EnhancedJWTManager:
    ...
    async def create_token_pair(self, claims: TokenClaims,
    ...
    async def refresh_access_token(self, refresh_token: str,
                                  device_fingerprint: Optional[str] = None) -> Optional[TokenPair]:
    ...
    async def create_device_token(self, user_id: str, device_id: str, 
    ...
    async def _store_device_token(self, token: str, user_id: str, 
                                 device_id: str, device_info: Dict[str, Any]):
        ...
        if self.redis_client:
            ...
            await self.redis_client.expire(
                f"device_token:{token}", 
                int(self.device_token_expire.total_seconds())
            )
...
def get_jwt_manager(secret_key: Optional[str] = None,
                   redis_client: Optional[redis.Redis] = None) -> EnhancedJWTManager:
...
async def create_parent_tokens(user_id: str, username: str, email: str,
                              family_id: str, device_fingerprint: Optional[str] = None) -> TokenPair:
...
async def create_child_tokens(user_id: str, username: str, email: str,
                             family_id: str, parent_id: str, device_id: str) -> TokenPair:
