...
class ChildSafetyConfig(BaseModel):
    ...
    moderation_providers: List[str] = Field(
        ["perspective", "azure", "custom"],
        description="Moderation providers"
    )
...
class SecureConfigurationManager:
    ...
    async def load_configuration(self) -> ApplicationConfig:
        ...
        for section in ['database', 'redis', 'security', 'child_safety', 
                       'performance', 'logging', 'ai_services', 'features']:
        ...
        try:
            ...
            await self._audit("load_configuration", "system", "full_config", 
                            action="read", new_value="<config loaded>")
        ...
        if persist:
            for loader in self.loaders:
                if await loader.save(key, value):
                    await self._audit("set", "system", key, 
                                    action="write", new_value=value)
                    return True
