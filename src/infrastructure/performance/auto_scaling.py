...
# Prometheus metrics
SCALING_EVENTS_TOTAL = Counter('scaling_events_total', 'Total scaling events', ['direction', 'reason'])
CURRENT_REPLICAS = Gauge('current_replicas', 'Current number of replicas', ['deployment'])
TARGET_REPLICAS = Gauge('target_replicas', 'Target number of replicas', ['deployment'])
SCALING_DURATION = Histogram('scaling_duration_seconds', 'Time taken for scaling operation')
...
class ScalingPolicy:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.scale_up_threshold = config.get("scale_up_threshold", 1.2)  # 20% above target
        self.scale_down_threshold = config.get("scale_down_threshold", 0.8)  # 20% below target
...
class AutoScalingSystem:
    ...
    async def _setup_geographic_regions(self) -> None:
        ...
        region_configs = self.config.get("geographic_regions", [
            {"name": "us-east", "location": "US East", "latency_ms": 50, "capacity": 2000},
            {"name": "us-west", "location": "US West", "latency_ms": 80, "capacity": 1500},
            {"name": "eu-west", "location": "Europe West", "latency_ms": 120, "capacity": 1800},
            {"name": "asia-east", "location": "Asia East", "latency_ms": 150, "capacity": 1200},
        ])
    ...
    async def _monitor_metrics(self) -> None:
        ...
        while True:
            try:
                ...
                CURRENT_REPLICAS.labels(deployment="ai-teddy-api").set(self.current_replicas)
                TARGET_REPLICAS.labels(deployment="ai-teddy-api").set(self.target_replicas)
    ...
    async def _make_scaling_decision(self, metrics: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """Make scaling decision based on metrics"""
        current_time = time.time()
        
        # Check scale-up conditions
        scale_up_needed = (
            metrics["cpu_utilization"] > self.scaling_policy.target_cpu_utilization * self.scaling_policy.scale_up_threshold or
            metrics["memory_utilization"] > self.scaling_policy.target_memory_utilization * self.scaling_policy.scale_up_threshold or
            metrics["request_rate"] > self.scaling_policy.target_request_rate * self.scaling_policy.scale_up_threshold
        )
        
        # Check scale-down conditions
        scale_down_needed = (
            metrics["cpu_utilization"] < self.scaling_policy.target_cpu_utilization * self.scaling_policy.scale_down_threshold and
            metrics["memory_utilization"] < self.scaling_policy.target_memory_utilization * self.scaling_policy.scale_down_threshold and
            metrics["request_rate"] < self.scaling_policy.target_request_rate * self.scaling_policy.scale_down_threshold
        )
        
        # Check cooldown periods
        if scale_up_needed and self.current_replicas < self.scaling_policy.max_replicas:
            if (self.last_scale_up is None or 
                current_time - self.last_scale_up > self.scaling_policy.scale_up_cooldown_seconds):
                return {
                    "action": "scale_up",
                    "reason": "high_utilization",
                    "current_replicas": self.current_replicas,
                    "target_replicas": min(self.current_replicas + 1, self.scaling_policy.max_replicas),
                    "metrics": metrics
                }
        
        if scale_down_needed and self.current_replicas > self.scaling_policy.min_replicas:
            if (self.last_scale_down is None or 
                current_time - self.last_scale_down > self.scaling_policy.scale_down_cooldown_seconds):
                return {
                    "action": "scale_down",
                    "reason": "low_utilization",
                    "current_replicas": self.current_replicas,
                    "target_replicas": max(self.current_replicas - 1, self.scaling_policy.min_replicas),
                    "metrics": metrics
                }
    ...
    async def _execute_scaling_decision(self, decision: Dict[str, Any]) -> None:
        ...
        try:
            ...
            logger.info(f"🔄 Executing {action}: {self.current_replicas} -> {target_replicas} replicas")
    ...
    async def _update_kubernetes_deployment(self, target_replicas: int) -> None:
        """Update Kubernetes deployment replicas"""
        # Mock Kubernetes API call
        # In production, use kubernetes-asyncio to update deployment
        logger.info(f"🔄 Updating Kubernetes deployment to {target_replicas} replicas")
    ...
    async def _optimize_traffic_distribution(self) -> None:
        ...
        for region in healthy_regions:
            ...
            weight = (region.capacity / total_capacity) * (100 / region.latency_ms)
            region.traffic_weight = weight
        
        logger.info(f"✅ Traffic distribution optimized across {len(healthy_regions)} regions")
    ...
    async def manual_scale(self, target_replicas: int) -> bool:
        ...
        if not (self.scaling_policy.min_replicas <= target_replicas <= self.scaling_policy.max_replicas):
            logger.error(f"Target replicas {target_replicas} outside allowed range")
            return False
...
# Factory function
async def create_auto_scaling_system(config: Dict[str, Any]) -> AutoScalingSystem:
